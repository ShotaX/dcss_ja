...
bool set_level_flags(uint32_t flags, bool silent)
{
    ...
    {
        mpr(jtrans("You sense the appearance of a powerful magical force which warps space."), MSGCH_WARN);
    }

    if (could_map && !can_map && !silent)
    {
        mpr(jtrans("A powerful force appears that prevents you from remembering where you've been."), MSGCH_WARN);
    }
...
bool unset_level_flags(uint32_t flags, bool silent)
{
    ...
    if (!could_control && can_control && !silent)
    {
        ...
        // else to send it.
        mpr(jtrans("You sense the disappearance of a powerful magical force which warped space."), MSGCH_RECOVERY);
    }
    ...
    if (!could_map && can_map && !silent)
    {
        // Isn't really a "recovery", but I couldn't think of where
        // else to send it.
        mpr(jtrans("You sense the disappearance of the force that prevented you from remembering where you've been."), MSGCH_RECOVERY);
    }
...
static dungeon_feature_type _pick_temple_altar(vault_placement &place)
{
    if (_temple_altar_list.empty())
    {
        if (_current_temple_hash != NULL)
        {
            ...
            mpr(jtrans("Ran out of altars for temple!"), MSGCH_ERROR);
            return DNGN_FLOOR;
        }
