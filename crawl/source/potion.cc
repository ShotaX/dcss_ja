                   ...
                   bool from_fountain)
{
    ...
    case POT_BLOOD_COAGULATED:
        ...
        else
        {
            ...
            {
                // Likes it.
                mpr(jtrans("This tastes like blood."));
                lessen_hunger(value, true);
            }
            else
            {
                mpr(jtrans("Yuck - this tastes like blood."));
    ...
    case POT_BRILLIANCE:
    {
        ...
        mprf(MSGCH_DURATION, jtrans("You feel %s all of a sudden.").c_str(),
             were_brilliant ? jtrans("clever").c_str() : jtrans("more clever").c_str());
    ...
    case POT_AGILITY:
    {
        ...
        mprf(MSGCH_DURATION, jtrans("You feel %s all of a sudden.").c_str(),
             were_agile ? jtrans("agile").c_str() : jtrans("more agile").c_str());
    ...
    case POT_FLIGHT:
        ...
        {
            mprf(MSGCH_WARN,"%s",jtrans("Your roots keep you in place.").c_str());
            break;
        }

        if (you.liquefied_ground())
        {
            mprf(MSGCH_WARN,"%s",jtrans("This potion isn't strong enough to pull you from the ground!").c_str());
            break;
        }
    ...
    case POT_STRONG_POISON:
        ...
        {
            mprf(jtrans("You feel %s nauseous.").c_str(),
                 (pot_eff == POT_POISON) ? jtrans("slightly").c_str() : jtrans("quite").c_str());
            maybe_id_resist(BEAM_POISON);
        }
        else
        {
            mprf(MSGCH_WARN,
                 jtrans("That liquid tasted %s nasty...").c_str(),
                 (pot_eff == POT_POISON) ? jtrans("very").c_str() : jtrans("extremely").c_str());
    ...
    case POT_INVISIBILITY:
        if (you.haloed() || you.glows_naturally())
        {
            ...
            mpr(jtrans("You briefly turn translucent."));
            return true;
        }
        else if (you.backlit())
          {
...
//Fixme_ja
              mprf(MSGCH_DURATION,
                   jtrans("You become %stransparent, but the glow from your %s prevents you from becoming completely invisible.").c_str(),
                 you.duration[DUR_INVIS] ? jtrans("more").c_str() : "",
                 comma_separated_line(afflictions.begin(), afflictions.end()).c_str());
          }
          else
          {
            mpr(!you.duration[DUR_INVIS] ? jtrans("You fade into invisibility!")
                                         : jtrans("You fade further into invisibility."),
                MSGCH_DURATION);
        }
    ...
    case POT_RESTORE_ABILITIES:
    {
        ...
        {
            mpr(jtrans("You have got your breath back."), MSGCH_RECOVERY);
    ...
    case POT_RESISTANCE:
        mpr(jtrans("You feel protected."), MSGCH_DURATION);
