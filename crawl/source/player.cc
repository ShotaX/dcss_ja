...
static void _moveto_maybe_repel_stairs()
{
    ...
    if (x_chance_in_y(pct, 100))
    {
        if (slide_feature_over(you.pos(), coord_def(-1, -1), false))
        {
            ...
            mprf(jtrans("%s slides away as you move it!").c_str(), stair_str.c_str());
...
static bool _check_moveto_cloud(const coord_def& p, const string &move_verb)
{
    ...
    if (cloud != EMPTY_CLOUD && !you.confused())
    {
            ...
            && !crawl_state.disables[DIS_CONFIRMATIONS])
        {
            ...
            string prompt = make_stringf(jtrans("Really %s into that cloud of %s?").c_str(),cloud_name_at_index(cloud).c_str(),
                                         move_verb.c_str());
...
static bool _check_moveto_trap(const coord_def& p, const string &move_verb)
{
    ...
    if (trap->type == TRAP_ZOT && !crawl_state.disables[DIS_CONFIRMATIONS])
    {
        string prompt = make_stringf(
            jtrans("Do you really want to %s into the Zot trap").c_str(),
            move_verb.c_str());
    ...
    {
        string prompt = make_stringf(
            jtrans("Really %s %s that?").c_str(),feature_description_at(p, false, DESC_BASENAME, false).c_str(),
            move_verb.c_str());
                                    ...
                                    bool cling = true)
{
    ...
    if (msg != "")
        ...
        mpr(jtrans("You cannot swim in your current form."));
                                  ...
                                  const string &msg)
{
        ...
        && !crawl_state.disables[DIS_CONFIRMATIONS])
    {
        ...
        prompt += jtrans("Are you sure you want to").c_str();

        prompt += env.grid(p) == DNGN_DEEP_WATER ? jtrans("deep water").c_str() 
												 : jtrans("lava").c_str();

        if (you.ground_level())
            prompt += jtrans("into").c_str();
        else
            prompt += jtrans("over").c_str();

        prompt += move_verb + jtrans("shimasuka").c_str();

        prompt += need_expiration_warning(DUR_FLIGHT, p)
                      ? jtrans("while you are losing your buoyancy?").c_str()
                      : jtrans("while your transformation is expiring?").c_str();
...
static bool _check_moveto_exclusion(const coord_def& p, const string &move_verb)
{
    if (is_excluded(p)
        ...
        && !crawl_state.disables[DIS_CONFIRMATIONS])
    {
        string prompt = make_stringf(jtrans("Really %s into a travel-excluded area?").c_str(),
                                     move_verb.c_str());
    ...
    if (you.ground_level())
    {
        ...
        if (feat_is_water(new_grid) && !you.can_swim() && !beogh_water_walk())
        {
            ...
            if (!feat_is_water(old_feat))
            {
                ...
                {
                    mpr(jtrans("You slowly cross the shallow water and cling to the wall."));
                }
                else
                {
                    mprf(jtrans("You %s the %s water.").c_str(),
						 new_grid == DNGN_SHALLOW_WATER ? jtrans("shallow").c_str() : jtrans("deep").c_str(),
                         stepped ? jtrans("enter").c_str() : jtrans("fall into").c_str());
...
int burden_change(void)
{
    ...
    if (you.burden <= carrying_capacity(BS_UNENCUMBERED))
    {
        ...
        if (old_burdenstate != you.burden_state)
            mpr(jtrans("Your possessions no longer seem quite so burdensome."));
    }
    else if (you.burden <= carrying_capacity(BS_ENCUMBERED))
    {
        ...
        if (old_burdenstate != you.burden_state)
        {
            mpr(jtrans("You are being weighed down by all of your possessions."));
    ...
    else
    {
        ...
        if (old_burdenstate != you.burden_state)
        {
            mpr(jtrans("You are being crushed by all of your possessions."));
            learned_something_new(HINT_HEAVY_LOAD);
        }
    }
...
static void _remove_temp_mutations()
{
    ...
    if (num_remove >= you.attribute[ATTR_TEMP_MUTATIONS])
        mpr(jtrans("You feel the corruption within you wane completely."), MSGCH_DURATION);
    else
        mpr(jtrans("You feel the corruption within you wane somewhat."), MSGCH_DURATION);
...
void gain_exp(unsigned int exp_gained, unsigned int* actual_gain)
{
    ...
    if (!you.sage_skills.empty())
    {
        ...
        {
            mprf(jtrans("You feel less studious about %s.").c_str(), skill_name(skill));
...
static void _draconian_scale_colour_message()
{
    ...
    case SP_RED_DRACONIAN:
        mpr(jtrans("Your scales start taking on a fiery red colour."),
            MSGCH_INTRINSIC_GAIN);
        perma_mutate(MUT_HEAT_RESISTANCE, 1, "draconian maturity");
        break;

    case SP_WHITE_DRACONIAN:
        mpr(jtrans("Your scales start taking on an icy white colour."),
            MSGCH_INTRINSIC_GAIN);
        perma_mutate(MUT_COLD_RESISTANCE, 1, "draconian maturity");
        break;

    case SP_GREEN_DRACONIAN:
        mpr(jtrans("Your scales start taking on a lurid green colour."),
            MSGCH_INTRINSIC_GAIN);
        perma_mutate(MUT_POISON_RESISTANCE, 1, "draconian maturity");
        break;

    case SP_YELLOW_DRACONIAN:
        mpr(jtrans("Your scales start taking on a golden yellow colour."),
            MSGCH_INTRINSIC_GAIN);
        break;

    case SP_GREY_DRACONIAN:
        mpr(jtrans("Your scales start taking on a dull iron-grey colour."),
            MSGCH_INTRINSIC_GAIN);
        perma_mutate(MUT_UNBREATHING, 1, "draconian maturity");
        break;

    case SP_BLACK_DRACONIAN:
        mpr(jtrans("Your scales start taking on a glossy black colour."),
            MSGCH_INTRINSIC_GAIN);
        perma_mutate(MUT_SHOCK_RESISTANCE, 1, "draconian maturity");
        break;

    case SP_PURPLE_DRACONIAN:
        mpr(jtrans("Your scales start taking on a rich purple colour."),
            MSGCH_INTRINSIC_GAIN);
        break;

    case SP_MOTTLED_DRACONIAN:
        mpr(jtrans("Your scales start taking on a weird mottled pattern."),
            MSGCH_INTRINSIC_GAIN);
        break;

    case SP_PALE_DRACONIAN:
        mpr(jtrans("Your scales start fading to a pale cyan-grey colour."),
            MSGCH_INTRINSIC_GAIN);
        break;
...
static void _felid_extra_life()
{
    ...
    {
        you.lives++;
        mpr(jtrans("Extra life!"), MSGCH_INTRINSIC_GAIN);
           ...
           && you.experience >= exp_needed(you.experience_level + 1))
    {
        ...
        if (new_exp <= you.max_level)
        {
            mprf(MSGCH_INTRINSIC_GAIN,
                 jtrans("Welcome back to level %d!").c_str(), new_exp);
        ...
        else  // Character has gained a new level
        {
            ...
            if (new_exp == 27)
            {
                mpr(jtrans("You have reached level 27, the final one!"),
                    MSGCH_INTRINSIC_GAIN);
            }
            else
            {
                mprf(MSGCH_INTRINSIC_GAIN, jtrans("You have reached level %d!").c_str(),
                     new_exp);
            }
            ...
            case SP_DEEP_DWARF:
                ...
                {
                    mpr(jtrans("You feel somewhat more resistant."),
                        MSGCH_INTRINSIC_GAIN);
            ...
            case SP_MUMMY:
                ...
                {
                    mpr(jtrans("You feel more in touch with the powers of death."),
                        MSGCH_INTRINSIC_GAIN);
                }

                if (you.experience_level == 13)  // level 13 for now -- bwr
                {
                    mpr(jtrans("You can now infuse your body with magic to restore decomposition."), MSGCH_INTRINSIC_GAIN);
                }
                break;
            ...
            case SP_VAMPIRE:
                if (you.experience_level == 3)
                {
                    ...
                    {
                        mpr(jtrans("If you weren't so full you could now transform into a vampire bat."), MSGCH_INTRINSIC_GAIN);
                    }
                    else
                    {
                        mpr(jtrans("You can now transform into a vampire bat."),
                            MSGCH_INTRINSIC_GAIN);
                    }
                }
                else if (you.experience_level == 6)
                {
                    mpr(jtrans("You can now bottle potions of blood from corpses."),
                        MSGCH_INTRINSIC_GAIN);
                }
                break;
            ...
            case SP_NAGA:
                ...
                if (!(you.experience_level % 3))
                {
                    mpr(jtrans("Your skin feels tougher."), MSGCH_INTRINSIC_GAIN);
                    you.redraw_armour_class = true;
                }

                if (you.experience_level == 13)
                {
                    mpr(jtrans("Your tail grows strong enough to constrict your enemies."), MSGCH_INTRINSIC_GAIN);
                }
                break;
            ...
            case SP_PALE_DRACONIAN:
                if (!(you.experience_level % 3))
                {
                    mpr(jtrans("Your scales feel tougher."), MSGCH_INTRINSIC_GAIN);
                    you.redraw_armour_class = true;
                }
                ...
                if (you.experience_level == 14)
                {
                    ...
                    case SP_BLACK_DRACONIAN:
                        ...
                        mpr(jtrans("You can now fly continuously."), MSGCH_INTRINSIC_GAIN);
                        break;
                    default:
                        break;
                    }
            ...
            case SP_DEMONSPAWN:
            {
                ...
                for (unsigned i = 0; i < you.demonic_traits.size(); ++i)
                {
                    if (is_body_facet(you.demonic_traits[i].mutation))
                    {
                                ...
                                != first_body_facet)
                        {
                            ...
                            {
                                mpr(jtrans("You feel monstrous as your demonic heritage exerts itself."),
                                     MSGCH_MUTATION);
                ...
                for (unsigned i = 0; i < you.demonic_traits.size(); ++i)
                {
                        ...
                        == you.experience_level)
                    {
                        if (!gave_message)
                        {
                            mpr(jtrans("Your demonic ancestry asserts itself..."),
                                MSGCH_INTRINSIC_GAIN);
            ...
            case SP_TENGU:
                ...
                if (you.experience_level == 5)
                {
                    mpr(jtrans("You have gained the ability to fly."),
                        MSGCH_INTRINSIC_GAIN);
                }
...
static void _output_expiring_message(duration_type dur, const char* msg)
{
    if (you.duration[dur])
    {
        ...
        mprf(jtrans("%s%s").c_str(), expires ? jtrans("Expiring:").c_str() : "", msg);
    }
}
...
static void _display_movement_speed()
{
    ...
    mprf(jtrans("Your %s speed is %s%s%s.").c_str(),
          // order is important for these:
          (swim)    ? jtrans("swimming").c_str() :
          (water)   ? jtrans("wading").c_str() :
          (fly)     ? jtrans("flying").c_str()
                    : jtrans("movement").c_str(),

          (water && !swim)  ? jtrans("uncertain and").c_str() :
          (!water && swift) ? jtrans("aided by the wind").c_str() : "",

          (!water && swift) ? ((move_cost >= 10) ? jtrans(", but still").c_str()
                                                 : jtrans("and").c_str())
                            : "",

          (move_cost <   8) ? jtrans("very quick").c_str() :
          (move_cost <  10) ? jtrans("quick").c_str() :
          (move_cost == 10) ? jtrans("average").c_str() :
          (move_cost <  13) ? jtrans("slow").c_str()
                            : jtrans("very slow").c_str());
}
...
/*
    ...
    mprf(jtrans("%s given your current equipment.").c_str(),
         (to_hit <   1) ? jtrans("You are completely incapable of fighting").c_str() :
         (to_hit <   5) ? jtrans("Hitting even clumsy monsters is extremely awkward").c_str() :
         (to_hit <  10) ? jtrans("Hitting average monsters is awkward").c_str() :
         (to_hit <  15) ? jtrans("Hitting average monsters is difficult").c_str() :
         (to_hit <  20) ? jtrans("Hitting average monsters is hard").c_str() :
         (to_hit <  30) ? jtrans("Very agile monsters are a bit awkward to hit").c_str() :
         (to_hit <  45) ? jtrans("Very agile monsters are a bit difficult to hit").c_str() :
         (to_hit <  60) ? jtrans("Very agile monsters are a bit hard to hit").c_str() :
         (to_hit < 100) ? jtrans("You feel comfortable with your ability to fight").c_str()
                        : jtrans("You feel confident with your ability to fight").c_str());
*/
}

static string _attack_delay_desc(int attack_delay)
{
    return ((attack_delay >= 200) ? jtrans("extremely slow").c_str() :
            (attack_delay >= 155) ? jtrans("very slow").c_str() :
            (attack_delay >= 125) ? jtrans("quite slow").c_str() :
            (attack_delay >= 105) ? jtrans("below average").c_str() :
            (attack_delay >=  95) ? jtrans("average").c_str() :
            (attack_delay >=  75) ? jtrans("above average").c_str() :
            (attack_delay >=  55) ? jtrans("quite fast").c_str() :
            (attack_delay >=  45) ? jtrans("very fast").c_str() :
            (attack_delay >=  35) ? jtrans("extremely fast").c_str() :
                                    jtrans("blindingly fast").c_str());
}
...
static void _display_attack_delay()
{
    ...
    string msg = jtrans("Your attack speed is") + _attack_delay_desc(avg)
...
void display_char_status()
{
    ...
    else if (you.duration[DUR_DEATHS_DOOR])
    {
        _output_expiring_message(DUR_DEATHS_DOOR,
                                 jtrans("You are standing in death's doorway.").c_str());
    }
    else
    ...
    // magic resistance
    mprf(jtrans("You are %s to hostile enchantments.").c_str(),
         jtrans(magic_res_adjective(player_res_magic(false))).c_str());
    dprf("MR: %d", you.res_magic());

    // character evaluates their ability to sneak around:
    mprf(jtrans("You feel %s_stealth").c_str(), jtrans(stealth_desc(check_stealth())).c_str());
...
void flush_mp()
{
  if (Options.magic_point_warning
      ...
      {
          mpr(jtrans("* * * LOW MAGIC WARNING * * *"), MSGCH_DANGER);
      }
    ...
    if (you.hp < minimum + 1)
    {
        if (!suppress_msg)
        {
            mpr(you.species != SP_DJINNI ?
                jtrans("You haven't enough vitality at the moment.") :
                jtrans("You haven't enough essence at the moment."));
        }
    ...
    if (you.magic_points < minimum)
      {
          if (!suppress_msg)
        {
            if (get_real_mp(include_items) < minimum)
                mpr(jtrans("You haven't enough magic capacity."));
            else
                mpr(jtrans("You haven't enough magic at the moment."));
        }
          crawl_state.cancel_cmd_again();
          crawl_state.cancel_cmd_repeat();
        return false;
  ...
  bool enough_zp(int minimum, bool suppress_msg)
{
    ...
    {
        if (!suppress_msg)
            mpr(jtrans("You haven't enough Zot Points."));
...
{
    if (cont > 5)
        return jtrans("You are engulfed in a nimbus of crackling magics!").c_str();
    else if (cont == 5)
        return jtrans("Your entire body has taken on an eerie glow!").c_str();
    else if (cont > 1)
    {
        return (make_stringf(jtrans("You are %s with residual magics%s").c_str(),
                   (cont == 4) ? jtrans("practically glowing").c_str() :
                   (cont == 3) ? jtrans("heavily infused").c_str() :
                   (cont == 2) ? jtrans("contaminated").c_str()
                                    : jtrans("lightly contaminated").c_str(),
                   (cont == 4) ? "!" : ""));
    }
    else if (cont == 1)
        return jtrans("You are very lightly contaminated with residual magic.").c_str();
    else
        return "";
}
    ...
    else if (msg && new_level != old_level)
    {
        ...
        else
        {
            mprf((change > 0) ? MSGCH_WARN : MSGCH_RECOVERY,
                 jtrans("You feel %s contaminated with magical energies.").c_str(),
                 (change > 0) ? jtrans("more").c_str() : jtrans("less").c_str());
        }
        ...
        {
            mpr(jtrans("You fade completely from view now that you are no longer glowing from magical contamination."));
        }
    }
...
bool confuse_player(int amount, bool resistable)
{
    ...
    if (you.duration[DUR_CONF] > old_value)
    {
        ...
        mprf(MSGCH_WARN, jtrans("You are %sconfused.").c_str(),
             old_value > 0 ? jtrans("more").c_str() : "");
...
bool poison_player(int amount, string source, string source_aux, bool force)
{
    ...
    {
        mprf(MSGCH_WARN, jtrans("You are %spoisoned.").c_str(),
             old_value > 0 ? jtrans("more_player").c_str() : "");
...
void dec_poison_player()
{
    ...
    if (x_chance_in_y(you.duration[DUR_POISONING], 5))
    {
        ...
        if (you.hp < oldhp)
            mprf(channel, jtrans("You feel %ssick.").c_str(), jtrans(adj).c_str());
...
void reduce_poison_player(int amount)
{
    ...
    if (you.duration[DUR_POISONING] < old_value)
    {
        mprf(MSGCH_RECOVERY, jtrans("You feel %sbetter.").c_str(),
             you.duration[DUR_POISONING] > 0 ? jtrans("a little").c_str() : "");
    }
}
...
bool napalm_player(int amount, string source, string source_aux)
{
    ...
    if (you.duration[DUR_LIQUID_FLAMES] > old_value)
        mpr(jtrans("You are covered in liquid flames!"), MSGCH_WARN);
...
void dec_napalm_player(int delay)
{
    ...
    {
        if (you.ground_level())
            mpr(jtrans("The flames go out!"), MSGCH_WARN);
        else
            mpr(jtrans("You dip into the water, and the flames go out!"), MSGCH_WARN);
...
bool slow_player(int turns)
{
    ...
    else
    {
        if (you.duration[DUR_SLOW] == 0)
            mpr(jtrans("You feel yourself slow down."));
        else
...
void dec_slow_player(int delay)
{
    ...
    if (you.duration[DUR_SLOW] <= BASELINE_DELAY)
    {
        mpr(jtrans("You feel yourself speed up."), MSGCH_DURATION);
...
void dec_exhaust_player(int delay)
{
    ...
    if (you.duration[DUR_EXHAUSTED] <= BASELINE_DELAY)
    {
        mpr(jtrans("You feel less exhausted."), MSGCH_DURATION);
...
bool haste_player(int turns, bool rageext)
{
    ...
    if (!you.duration[DUR_HASTE])
        mpr(jtrans("You feel yourself more speed up."));
...
void dec_haste_player(int delay)
{
    ...
    if (you.duration[DUR_HASTE] > BASELINE_DELAY)
    {
        ...
        {
            mpr(jtrans("Your extra speed is starting to run out."), MSGCH_DURATION);
            if (coinflip())
    ...
    else if (you.duration[DUR_HASTE] <= BASELINE_DELAY)
    {
        if (!you.duration[DUR_BERSERK])
            mpr(jtrans("You feel yourself slow down."), MSGCH_DURATION);
...
void dec_disease_player(int delay)
{
    if (you.disease)
    {
        ...
        if (you.disease == 0)
            mpr(jtrans("You feel your health improve."), MSGCH_RECOVERY);
    }
}
...
void float_player()
{
    if (you.fishtail)
    {
        mprf("%s",jtrans("Your tail turns into legs as you fly out of the water.").c_str());
        merfolk_stop_swimming();
    }
...
{
    if (you.form == TRAN_TREE)
        return mpr(jtrans("Your roots keep you in place."));
    ...
    if (!already_flying)
        mprf(MSGCH_DURATION, jtrans("You feel %s buoyant.").c_str(), standing ? jtrans("very").c_str() : jtrans("more").c_str());
...
string player::shout_verb() const
{
    ...
    {
    case TRAN_DRAGON:
        return jtrans("roar").c_str();
    case TRAN_SPIDER:
        return jtrans("hiss").c_str();
    case TRAN_BAT:
    case TRAN_PORCUPINE:
        return jtrans("squeak").c_str();
    case TRAN_PIG:
        return coinflip() ? jtrans("squeal").c_str() : jtrans("oink").c_str();
    ...
    default:
        if (species == SP_FELID)
            return coinflip() ? jtrans("meow").c_str() : jtrans("yowl").c_str();
        // depends on SCREAM mutation
        int level = player_mutation_level(MUT_SCREAM);
        if (level <= 1)
            return jtrans("shout").c_str();
        else if (level == 2)
            return jtrans("yell").c_str();
        else // level == 3
            return jtrans("scream").c_str();
    }
}
...
bool player::rot(actor *who, int amount, int immediate, bool quiet)
{
    ...
    if (!quiet)
    {
        mprf(MSGCH_WARN, jtrans("You feel your flesh %s away!").c_str(),
             (rotting > 0 || immediate) ? jtrans("rotting").c_str() : jtrans("start to rot").c_str());
    }
...
void player::paralyse(actor *who, int str, string source)
{
    ...
    mprf(jtrans("You %s the ability to move!").c_str(),
         paralysis ? jtrans("still haven't").c_str() 
					 : jtrans("suddenly lose").c_str());
...
void player::petrify(actor *who, bool force)
  {
    ...
    redraw_evasion = true;
      mpr(jtrans("You are slowing down."), MSGCH_WARN);
  }
...
bool player::attempt_escape(int attempts)
{
    ...
    {
        mprf(jtrans("You escape %s's grasp.").c_str(), themonst->name(DESC_THE, true).c_str());
    ...
    {
        string emsg = jtrans("Your attempt to break free from").c_str();
        emsg += themonst->name(DESC_THE, true);
        emsg += jtrans("fails, but you feel that another attempt might succeed.").c_str();
        mpr(emsg);
        turn_is_over = true;
        return false;
    }
...
void player::sentinel_mark(bool trap)
{
    if (duration[DUR_SENTINEL_MARK])
    {
      ...
      {
          mpr(jtrans("A sentinel's mark forms upon you."), MSGCH_WARN);
...
void player::weaken(actor *attacker, int pow)
{
    if (!duration[DUR_WEAK])
        mpr(jtrans("You feel yourself grow feeble."), MSGCH_WARN);
    else
        mpr(jtrans("You feel as though you will be weak longer."), MSGCH_WARN);
...
static string _constriction_description()
{
    ...
    {
        cinfo += make_stringf(jtrans("You have %d tentacle%s available for constriction.").c_str(),
                              num_free_tentacles);
    }
    ...
    if (you.is_constricted())
    {
        ...
        cinfo += make_stringf(jtrans("You are being %s by %s.").c_str(),
						monster_by_mid(you.constricted_by)->name(DESC_A).c_str(),
                      you.held == HELD_MONSTER ? jtrans("held").c_str() : jtrans("constricted").c_str());
    }
    ...
    if (you.constricting && !you.constricting->empty())
    {
        ...
        cinfo += jtrans("You are constricting").c_str();
        cinfo += ".";
    }
