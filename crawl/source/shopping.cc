                            ...
                            int num_in_list, MenuFreeform* freeform)
{
    ...
    if (!viewing && _can_shoplist())
    {
        shop_list = "[$] ";
        if (num_selected > 0)
            shop_list += jtrans("selected -> shopping list");
        else if (num_in_list > 0)
            shop_list += jtrans("shopping list -> selected").c_str();
        else
            shop_list = "";
    }
    if (!shop_list.empty())
    ...
    fs = formatted_string::parse_string(make_stringf("%s",
            jtrans("[<w>x</w>/<w>Esc</w>/<w>R-Click</w>] exit").c_str()));
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_highlight_colour(WHITE);
    ...
    fs = formatted_string::parse_string(make_stringf(
            "[<w>!</w>] %s",
            (viewing ? jtrans("buy items").c_str() : jtrans("examine items").c_str())));
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_bg_colour(BLACK);
    ...
    {
        pkeys = "[" + pkeys + "]";
        pkeys += jtrans("select item to").c_str();
		pkeys += (viewing ? jtrans("examine").c_str() : jtrans("buy").c_str());
    }
    ...
    fs = formatted_string::parse_string(
            jtrans("[<w>Enter</w>] make purchase").c_str());
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_bg_colour(BLACK);
    ...
    // print formatted string
    fs = formatted_string::parse_string(jtrans("list known items").c_str());
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_bg_colour(BLACK);
    ...
    // print formatted string
    fs = formatted_string::parse_string(jtrans("[<w>?</w>] inventory").c_str());
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_bg_colour(BLACK);
    ...
    // print formatted string
    fs = formatted_string::parse_string(jtrans("[<w>*</w>] invert selection").c_str());
    // print menu item
    tmp = new TextItem();
    tmp->set_fg_colour(LIGHTGRAY);
    tmp->set_bg_colour(BLACK);
                            ...
                            int total_stock, int num_selected, int num_in_list)
{
    ...
    if (!viewing && _can_shoplist())
    {
        shop_list = "[<w>$</w>] ";
        if (num_selected > 0)
            shop_list += jtrans("selected -> shopping list").c_str();
        else if (num_in_list > 0)
            shop_list += jtrans("shopping list -> selected").c_str();
        else
            shop_list = "";
    ...
    if (!pkeys.empty())
    {
        pkeys = jtrans("[" + pkeys + "] select item to")
                + (viewing ? jtrans("examine") : jtrans("buy"));
    }
    fs = formatted_string::parse_string(make_stringf(
            "[<w>x</w>/<w>Esc</w>"
#ifdef USE_TILE
                                ...
                                )
{
    ...
    for (unsigned int i = 0; i < stock.size(); ++i)
    {
        ...
        if (shop_item_unknown(item))
            item_name += jtrans("(unknown)").c_str();

        const int cols = get_number_of_cols();

        cprintf(jtrans("%s%5d gold").c_str(), chop_string(item_name, cols-16).c_str(), gp_value);
...
static bool _in_a_shop(int shopidx, int &num_in_list)
{
    ...
    const string hello = jtrans("Welcome to") + shop_name(shop.pos) + "!";
    bool first = true;
    int total_cost = 0;
    ...
    while (true)
    {
        ...
        if (stock.empty())
        {
            _shop_print(jtrans("I'm sorry, my shop is empty now.").c_str(), 1);
            _shop_more();
        ...
        if (!total_cost)
        {
            snprintf(info, INFO_SIZE, jtrans("You have %d gold piece.").c_str(), you.gold);

            textcolor(YELLOW);
        }
        else if (total_cost > you.gold)
        {
            snprintf(info, INFO_SIZE, jtrans("You have %d gold piece. You are short %d gold piece for the purchase.").c_str(),
                     you.gold,
                     total_cost - you.gold);
        ...
        {
            snprintf(info, INFO_SIZE, jtrans("You have %d gold piece. After the purchase, you will have %d gold piece.").c_str(),
                     you.gold,
                     you.gold - total_cost);
        ...
        {
            first = false;
            snprintf(info, INFO_SIZE, jtrans("%s What would you like to do?").c_str(),
                      hello.c_str());
        }
        else
            snprintf(info, INFO_SIZE, "%s" ,jtrans("What would you like to do?").c_str());
...
#endif
        {
            ...
            {
                if (_shop_yesno(jtrans("Buy items on shopping list? (Y/n)").c_str(), 'y'))
                {
                    to_buy = in_list;
            ...
            {
                _shop_print(jtrans("I'm sorry, you don't seem to have enough money.").c_str(),
                            1);
            }
            ...
            {
                snprintf(info, INFO_SIZE, jtrans("Purchase for %d gold? (y/n)").c_str(),
                         total_purchase);
                ...
                if (_shop_yesno(info, 'n'))
                {
                    ...
                    if (outside_items)
                    {
                        mprf(jtrans("I'll put %s outside for you.").c_str(),
                              num_items == 1             ? jtrans("it").c_str() :
                              num_items == outside_items ? jtrans("them").c_str()
                                                         : jtrans("part of them").c_str());
                    }
        ...
        else if (key == '$')
        {
            ...
            {
                _shop_print(jtrans("Huh?").c_str(), 1);
                _shop_more();
                continue;
            }
        ...
        else
        {
            ...
            else
            {
                ...
                if (in_list[key])
                {
                    ...
                    {
                        if (_shop_yesno(jtrans("Remove from shopping list? (y/N)").c_str(),
                                         'n'))
                        {
                            shopping_list.del_thing(item);
                            in_list[key]  = false;
                    ...
                    {
                        if (_shop_yesno(jtrans("Remove item from shopping list and mark for purchase? (Y/n)").c_str(),  'y'))
                        {
                            shopping_list.del_thing(item);
                            in_list[key] = false;
...
void shop()
{
    ...
    if (_shop_get_stock(i).empty())
    {
        ...
        mprf(jtrans("%s appears to be closed.").c_str(), shop_name(shop.pos).c_str());
        _delete_shop(i);
        return;
    }
    ...
    if (bought_something)
        mprf(jtrans("Thank you for shopping at %s!").c_str(), shopname.c_str());
                             ...
                             const level_pos* _pos)
{
    ...
    {
        mprf(MSGCH_ERROR, jtrans("%s is already on the shopping list.").c_str(),
                             ...
                             const level_pos* _pos)
{
    ...
    if (idx == -1)
    {
        mprf(MSGCH_ERROR, jtrans("%s isn't on shopping list, can't delete it.").c_str(),
             item.name(DESC_THE).c_str());
        return false;
    }
                                                ...
                                                int cost)
{
    ...
    for (unsigned int i = 0; i < list->size(); i++)
    {
        ...
        if (cost != -1)
        {
            ...
            string prompt =
                make_stringf(jtrans("Shopping-list: replace %dgp %s with cheaper one? (Y/n)").c_str(), list_cost,
        ...
        if (do_prompt)
        {
            ...
            string prompt = make_stringf(jtrans("Shopping-list: remove %s? (Y/n)").c_str(),
        ...
        {
            string str = make_stringf(jtrans("Shopping-list: removing %s").c_str(),
                                      describe_thing(thing, DESC_A).c_str());
...
void ShoppingList::gold_changed(int old_amount, int new_amount)
{
    ...
    if (new_amount > old_amount && new_amount >= min_unbuyable_cost)
    {
        ...
        mpr_comma_separated_list(jtrans("You now have enough gold to"), descs,
                                 jtrans(", or"));
        mpr(jtrans("You can access your shopping list by pressing '$'."));
...
void ShoppingListMenu::draw_title()
{
    if (title)
    {
        ...
        fs.cprintf(jtrans("%d %s, total cost %d gp").c_str(),
...
//Fixme_ja
        draw_title_suffix(formatted_string::parse_string(make_stringf(
            jtrans("<lightgrey>  [<w>a-z</w>: %-8s <w>?</w>/<w>!</w>: change action]").c_str(),
            jtrans(verb).c_str())), false);
    }
}
...
void ShoppingList::display()
{
    ...
    string more_str = make_stringf(jtrans("<yellow>You have %d gp</yellow>").c_str(), you.gold);
    ...
    while (true)
    {
        ...
        if (shopmenu.menu_action == Menu::ACT_EXECUTE)
        {
            ...
            {
                string prompt =
                   make_stringf(jtrans("You cannot afford %s; travel there anyway? (y/N)").c_str(),
                                describe_thing(*thing, DESC_A).c_str());
                clrscr();
        ...
        else if (shopmenu.menu_action == Menu::ACT_EXAMINE)
        {
            ...
            else // not an item, so we only stored a description.
            {
                ...
                snprintf(info, INFO_SIZE,
                         jtrans("%s with an entry fee of %d gold pieces.").c_str(),
                         describe_thing(*thing, DESC_A).c_str(),
        ...
        {
            string prompt = make_stringf(jtrans("Delete %s from shopping list? (y/N)").c_str(),
                                         describe_thing(*thing, DESC_A).c_str());
            clrscr();
            if (!yesno(prompt.c_str(), true, 'n'))
                continue;
