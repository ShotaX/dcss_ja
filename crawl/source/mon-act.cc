...
static bool _swap_monsters(monster* mover, monster* moved)
{
    ...
    {
        mprf(jtrans("%s and %s swap places.").c_str(), mover->name(DESC_THE).c_str(),
             moved->name(DESC_THE).c_str());
    }
...
static bool _handle_scroll(monster* mons)
{
    ...
    case SCR_TELEPORTATION:
        if (!mons->has_ench(ENCH_TP) && !mons->no_tele(true, false))
        {
            ...
            {
                simple_monster_message(mons, jtrans("reads a scroll.").c_str());
                read = true;
                ident = ID_KNOWN_TYPE;
    ...
    case SCR_SUMMONING:
        ...
        {
            simple_monster_message(mons, jtrans("reads a scroll.").c_str());
            mprf(jtrans("Wisps of shadow swirl around %s.").c_str(), mons->name(DESC_THE).c_str());
            read = true;
...
{
    if (!simple_monster_message(mons, jtrans("zaps a wand.").c_str()))
    {
        if (!silenced(you.pos()))
            mpr(jtrans("You hear a zap."), MSGCH_SOUND);
    }
...
static void _rod_fired_pre(monster* mons)
{
    ...
    if (!simple_monster_message(mons, jtrans("zaps a rod.").c_str())
        && !silenced(you.pos()))
    {
                ...
                && !mons->is_habitable(*ai))
            {
                    ...
                    || mons->props["no_conf_move"].get_string() != where)
                {
                    ...
                    simple_monster_message(mons,
                        make_stringf(jtrans("stays still, afraid of the %s.").c_str(),
...
static bool _jelly_divide(monster* parent)
{
    ...
    *child = *parent;
    ...
    if (!simple_monster_message(parent, jtrans("splits in two!").c_str())
        && (player_can_hear(parent->pos()) || player_can_hear(child->pos())))
    {
        mpr(jtrans("You hear a squelching noise."), MSGCH_SOUND);
    }
...
static bool _monster_eat_item(monster* mons, bool nearby)
{
         ...
         si && eaten < max_eat && hps_changed < 50; ++si)
    {
        ...
        {
            mprf(MSGCH_SOUND, jtrans("You hear a%s slurping noise.").c_str(),
                 nearby ? "" : jtrans("distant").c_str());
            shown_msg = true;
        }
        ...
        if (you_worship(GOD_JIYVA))
        {
            ...
            if (gain > PIETY_NONE)
                simple_god_message(jtrans("appreciates your sacrifice.").c_str());
    ...
    if (eaten > 0)
    {
        ...
        if (death_ooze_ate_good)
            simple_monster_message(mons, jtrans("twists violently!").c_str());
        else if (eaten_net)
            simple_monster_message(mons, jtrans("devours the net!").c_str());
        else
            _jelly_divide(mons);
    }
                                       ...
                                       bool do_heal, bool nearby)
{
    ...
    if (nearby)
    {
        mprf(jtrans("%s eats %s.").c_str(), mons->name(DESC_THE).c_str(),
...
static void _mons_open_door(monster* mons, const coord_def &pos)
{
    ...
    if (was_seen)
    {
        ...
        if (!you.can_see(mons))
        {
            mprf(jtrans("Something unseen %s").c_str(), open_str.c_str());
            interrupt_activity(AI_FORCE_INTERRUPT);
        }
        else if (!you_are_delayed())
        {
            mprf(jtrans("%s %sopen").c_str(), mons->name(DESC_A).c_str(),
                 open_str.c_str());
        }
    }
...
static bool _do_move_monster(monster* mons, const coord_def& delta)
{
    ...
    if (mons->is_constricted())
    {
        if (mons->attempt_escape())
            simple_monster_message(mons, jtrans("escapes!").c_str());
        else
        {
            simple_monster_message(mons, jtrans("struggles to escape constriction.").c_str());
            _swim_or_move_energy(mons);
            return true;
        }
    }
        ...
        else if (mons_can_eat_door(mons, f))
        {
            ...
            if (you.see_cell(f))
            {
                ...
                if (!you.can_see(mons))
                {
                    ...
                    simple_monster_message(mons, jtrans("eats the door!").c_str());
            }
...
static bool _monster_move(monster* mons)
{
    ...
    if (mons->berserk_or_insane())
    {
        ...
        if (noise_level > 0)
        {
            if (you.can_see(mons) && mons->berserk())
            {
                if (one_chance_in(10))
                {
                    mprf(MSGCH_TALK_VISUAL, jtrans("%s rages.").c_str(),
    ...
    if (mons->has_ench(ENCH_AQUATIC_LAND))
    {
        ...
        if (adj_move.empty())
        {
            simple_monster_message(mons, jtrans("flops around on dry land!").c_str());
            return false;
        }
    ...
    if (burrows || flattens_trees || digs)
    {
            ...
            && !burrows && digs)
        {
            ...
            else if (_mons_can_zap_dig(mons))
            {
                ...
                simple_monster_message(mons, jtrans("falters for a moment.2").c_str());
                 ...
                 && good_move[mmov.x + 1][mmov.y + 1] == true)
        {
            ...
            if (flattens_trees)
            {
                ...
                if (you.see_cell(target))
                {
                    ...
                    mprf(jtrans("%s knocks down a tree!").c_str(),
                         actor_visible?
                         mons->name(DESC_THE).c_str() : jtrans("Something").c_str());
                    noisy(25, target);
                }
                else
            ...
            else if (player_can_hear(mons->pos() + mmov))
            {
                ...
                mpr((mons->type == MONS_BORING_BEETLE) ?
                    jtrans("You hear a grinding noise.") :
                    jtrans("You hear a sizzling sound."), MSGCH_SOUND);
            }
        }
    }
