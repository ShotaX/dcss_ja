...
void xom_tick()
{
    ...
    if (x_chance_in_y(1, 20))
    {
        ...
        {
            simple_god_message(jtrans("is getting BORED.").c_str());
            //updating piety status line
            you.redraw_title = true;
        }
    }
    ...
    if (you.faith() ? coinflip() : one_chance_in(3))
    {
                 ...
                 && x_chance_in_y(chance - 1, 80))
        {
            ...
            {
                if (interest < 25)
                    simple_god_message(jtrans("is interested.").c_str());
                else
                    simple_god_message(jtrans("is intrigued.").c_str());
...
static int _xom_detect_items(int sever, int tension, bool debug = false)
{
    ...
    if (detect_items(sever) == 0)
        ...
        mpr(jtrans("Xom1"));  //("You detect items!");
...
static int _xom_detect_creatures(int sever, int tension, bool debug = false)
{
    ...
    else if (num_creatures == prev_detected)
    {
        ...
        mpr(jtrans("Xom2")); //("You detect no further creatures.");
    }
    else
        mpr(jtrans("Xom3")); //("You detect creatures!");
...
static int _xom_confuse_monsters(int sever, bool debug = false)
{
    ...
    for (monster_iterator mi(you.get_los()); mi; ++mi)
    {
              ...
              &menv[ANON_FRIENDLY_MONSTER], random2(sever) * 10)))
        {
            ...
            simple_monster_message(*mi, jtrans("looks rather confused.").c_str());
            rc = true;
        }
    }
...
static void _confuse_monster(monster* mons, int sever)
{
    ...
    {
        if (was_confused)
            simple_monster_message(mons, jtrans("looks rather more confused.").c_str());
        else
            simple_monster_message(mons, jtrans("looks rather confused.").c_str());
    }
}
    ...
    if (num_mons > 1 && x_chance_in_y(sever, 70))
    {
        ...
        for (int i = 0; i < repeats; ++i)
        {
            ...
            if (_swap_monsters(mons[mon1], mons[mon2]))
            {
                if (!did_message)
                {
                    mpr(jtrans("Xom4")); //("Some monsters swap places.");
                    did_message = true;
                }
                if (one_chance_in(3))
...
static int _xom_give_mutations(bool good, bool debug = false)
{
    ...
    if (you.can_safely_mutate())
    {
        ...
        mpr(jtrans("Xom5")); //("Your body is suffused with distortional energy.");
...
static int _xom_throw_divine_lightning(bool debug = false)
{
    ...
    {
        mpr(jtrans("Xom6")); //("Your divine protection wanes.");
            ...
            for (monster_iterator mi(you.get_los()); mi; ++mi)
            {
                           ...
                           &menv[ANON_FRIENDLY_MONSTER], random2(sever) * 10)))
                {
                    simple_monster_message(*mi,
                                           jtrans("looks rather confused.").c_str());
                }
                mons_too = true;
            }
        }
...
static int _xom_repel_stairs(bool debug = false)
{
    ...
    {
        if (one_chance_in(8))
            mpr(jtrans("Xom9")); //("Nothing appears to happen... Ominous!");
        else
...
int xom_acts(bool niceness, int sever, int tension, bool debug)
{
        ...
        && crawl_state.which_god_acting() != GOD_XOM)
    {
        ...
        {
            niceness = false;
            simple_god_message(jtrans("asks Xom for help in punishing you, and Xom happily agrees.").c_str(), which_god);
        }
        else
        {
            niceness = true;
            simple_god_message(jtrans("calls in a favour from Xom.").c_str(), which_god);
        }
    }
