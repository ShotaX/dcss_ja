...
void canned_msg(canned_message_type which_message)
{
    ...
    case MSG_CANNOT_DO_YET:
        ...
        break;
    case MSG_OK:
        mpr(jtrans("Okay, then."), MSGCH_PROMPT);
        crawl_state.cancel_cmd_repeat();
        break;
    ...
    case MSG_SPELL_FIZZLES:
        ...
        break;
    case MSG_HUH:
        mpr(jtrans("Huh?"), MSGCH_EXAMINE_FILTER);
        crawl_state.cancel_cmd_repeat();
        break;
    ...
    {
        const char* when =
            (which_message == MSG_EMPTY_HANDED_ALREADY  ? "already"
														: "now");
        if (you.species == SP_FELID)
            mprf(jtrans("Your mouth is %s empty.").c_str(), jtrans(when).c_str());
        else if (you.has_usable_claws(true))
            mprf(jtrans("You are %s empty-clawed.").c_str(), jtrans(when).c_str());
        else if (you.has_usable_tentacles(true))
            mprf(jtrans("You are %s empty-tentacled.").c_str(), jtrans(when).c_str());
        else
            mprf(jtrans("You are %s empty-handed.").c_str(), jtrans(when).c_str());
        break;
    }
    case MSG_YOU_BLINK:
    ...
    case MSG_MANA_INCREASE:
        mpr(jtrans("You feel your mana capacity increase."));
        break;
    case MSG_MANA_DECREASE:
        mpr(jtrans("You feel your mana capacity decrease."));
        break;
    ...
    case MSG_EVOCATION_SUPPRESSED:
        mpr(jtrans("You may not evoke while suppressed!"));
        break;
    case MSG_BEING_WATCHED:
        mpr(jtrans("You feel you are being watched by something."));
        break;
...
const char* held_status(actor *act)
{
    if (get_trapping_net(act->pos(), true) != NON_ITEM)
        return jtrans("held in a net").c_str();
    else
        return jtrans("caught in a web").c_str();
}
