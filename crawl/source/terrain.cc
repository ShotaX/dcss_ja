                        ...
                        bool noisy)
{
    ...
    case DNGN_DEEP_WATER:
        if (noisy)
            mprf(MSGCH_SOUND, "%s", jtrans("You hear a splash.").c_str());
        return false;

    case DNGN_LAVA:
        if (noisy)
            mprf(MSGCH_SOUND, "%s", jtrans("You hear a sizzling splash.").c_str());
        return true;
                      ...
                      dungeon_feature_type terrain)
{
    ...
    mprf(jtrans("You fall into the %s!").c_str(),
         (terrain == DNGN_LAVA)       ? jtrans("lava").c_str() :
         (terrain == DNGN_DEEP_WATER) ? jtrans("water").c_str()
                                      : "programming rift");
    ...
    if (terrain == DNGN_LAVA)
    {
        ...
        else
        {
            ...
            if (damage > 100)
                mpr(jtrans("The lava roasts you!!"));
            else if (damage > 70)
                mpr(jtrans("The lava burns you!!"));
            else if (damage > 40)
                mpr(jtrans("The lava sears you!!"));
            else if (damage > 20)
                mpr(jtrans("The lava scorches you!"));
            else
                mpr(jtrans("The lava scalds you!"));
    ...
    else
    {
        if (you.form == TRAN_STATUE)
            ...
            mpr(jtrans("You try to escape, but your burden drags you down!"));
    }
    ...
    if (escape)
    {
        ...
        {
            mpr(jtrans("You manage to scramble free!"));
            if (!clinging)
