...
static void _surge_power(spell_type spell)
{
    ...
    {
        const string modifier = (enhanced  < -2) ? jtrans("extraordinarily").c_str() :
                                (enhanced == -2) ? jtrans("extremely").c_str() :
                                (enhanced ==  2) ? jtrans("strong").c_str() :
                                (enhanced  >  2) ? jtrans("huge").c_str()
                                                 : "";
        mprf(jtrans("You feel %s %s").c_str(),
             !modifier.length() ? ""
                                : article_a(modifier).c_str(),
             (enhanced < 0) ? jtrans("numb sensation.").c_str()
                            : jtrans("surge of power!").c_str());
    }
}
                ...
                const string &title, spell_selector selector)
{
    ...
    string more_str = jtrans("Press '!'").c_str();
    if (toggle_with_I)
    {
        spell_menu.add_toggle_key('I');
        more_str += jtrans("or 'I'").c_str();
    }
    if (!viewing)
        spell_menu.menu_action = Menu::ACT_EXECUTE;
    more_str += jtrans("to toggle spell view.").c_str();
...
bool cast_a_spell(bool check_range, spell_type spell)
{
        ...
        while (true)
        {
            ...
            if (keyin == 0)
            {
                ...
                if (you.last_cast_spell == SPELL_NO_SPELL)
                    mpr(jtrans("Cast which spell? (? or * to list)"), MSGCH_PROMPT);
                else
                {
                    mprf(MSGCH_PROMPT, jtrans("Casting: <w>%s</w>").c_str(),
                         jtrans(spell_title(you.last_cast_spell)).c_str());
                    mpr(jtrans("Confirm with . or Enter, or press ? or * to list all spells."), MSGCH_PROMPT);
                }
    ...
    if (check_range && spell_no_hostile_in_range(spell))
    {
        ...
        mpr(jtrans("There are no visible monsters within range! (Use <w>Z</w> to cast anyway.)"));
    ...
    if (/*god_hates_spell*/god_loathes_spell(spell, you.religion))
    {
        ...
        if (!yesno(god_loathes_spell(spell, you.religion) ?
            jtrans("<lightred>Casting this spell will cause instant excommunication!</lightred> Really cast?").c_str() :
            jtrans("Casting this spell will put you into penance. Really cast?").c_str(),
            true, 'n'))
        {
            canned_msg(MSG_OK);
            crawl_state.zero_turns_taken();
...
bool spell_is_uncastable(spell_type spell, string &msg)
{
    ...
    {
        msg = jtrans("You cannot cast that spell in your current form!");
        return true;
    }

    if (_vampire_cannot_cast(spell))
    {
        msg = jtrans("Your current blood level is not sufficient to cast that spell.");
        return true;
    }
                                  ...
                                  bool wiz_cast)
{
    ...
    {
        mpr(jtrans("The dungeon can only cope with one malign gateway at a time!"));
        return true;
    }
    ...
    {
        mpr(jtrans("You need to wait for the winds to calm down."));
        return true;
    }
                       ...
                       bool allow_fail, bool check_range)
{
    ...
    if ((flags & SPFLAG_TARGETTING_MASK) && spell != SPELL_PORTAL_PROJECTILE)
    {
        ...
        if (dir == DIR_DIR)
            mpr(prompt ? prompt : jtrans("Which direction?"), MSGCH_PROMPT);
        ...
        title += jtrans(spell_title(spell)).c_str();
        title += "</white>";
    ...
    else if (allow_fail)
    {
        ...
        {
            god_speaks(GOD_SIF_MUNA, jtrans("You feel a surge of divine spite.").c_str());
                 ...
                 && one_chance_in(20))
        {
            ...
            simple_god_message(jtrans("does not allow the disloyal to dabble in death!").c_str(), GOD_KIKUBAAQUDGHA);
                 ...
                 && one_chance_in(20))
        {
            ...
            simple_god_message(jtrans("does not allow the disloyal to dabble in destruction!").c_str(), GOD_VEHUMET);
    ...
    case SPRET_FAIL:
    {
        ...
        mprf(jtrans("You miscast %s.").c_str(), jtrans(spell_title(spell)).c_str());
        flush_input_buffer(FLUSH_ON_FAILURE);
