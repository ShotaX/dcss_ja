...
bool ugly_thing_mutate(monster* ugly, bool proximity)
{
    ...
    else if (one_chance_in(9))
    {
                           ...
                           + 1))
        {
            ...
            src  = proximity_type == 0 ? jtrans("you") :
                   proximity_type == 1 ? jtrans("its kin")
                                       : jtrans("its neighbour");
            src  += jtrans("from");
    ...
    if (success)
    {
        simple_monster_message(ugly,
            make_stringf(jtrans("basks in the mutagenic energy%s and changes!").c_str(),
                         src.c_str()).c_str());
...
static monster* _do_split(monster* thing, coord_def & target)
{
    ...
    if (you.can_see(thing))
        mprf(jtrans("%s splits.").c_str(), thing->name(DESC_A).c_str());
    ...
    if (you.can_see(merge_to))
    {
        ...
        if (you.can_see(crawlie))
        {
            if (crawlie->type == old_type)
				{
                mprf(jtrans("Two %s merge%s%s.").c_str(),
                     pluralise(crawlie->name(DESC_PLAIN)).c_str(),
					 changed ? merge_to->name(DESC_A).c_str() : "",
                     changed ? jtrans("to form").c_str() : "");
				}
            else
				{
                mprf(jtrans("%s merges with %s%s%s.").c_str(),
        ...
        {
            mprf(jtrans("%s suddenly becomes %s.").c_str(),
                 uppercase_first(old_name).c_str(),
                 merge_to->name(DESC_A).c_str());
        }
        else
            mprf(jtrans("%s twists grotesquely.").c_str(), merge_to->name(DESC_A).c_str());
    }
    else if (you.can_see(crawlie))
        mprf(jtrans("%s suddenly disappears!").c_str(), crawlie->name(DESC_A).c_str());
...
static bool _do_merge_slimes(monster* initial_slime, monster* merge_to)
{
    ...
    if (you.can_see(merge_to))
    {
        ...
        {
            mprf(jtrans("Two slime creatures merge to form %s.").c_str(),
                 merge_to->name(DESC_A).c_str());
        }
        else
        {
            mprf(jtrans("A slime creature suddenly becomes %s.").c_str(),
                 merge_to->name(DESC_A).c_str());
        }
...
static void _starcursed_scream(monster* mon, actor* target)
{
    ...
    if (n > 7)
    {
        message = jtrans("A cacophony of accursed wailing tears at your sanity!");
        if (coinflip())
            stun = 2;
    }
    else if (n > 4)
    {
        message = jtrans("A deafening chorus of shrieks assaults your mind!");
        if (x_chance_in_y(1,3))
            stun = 1;
    }
    else if (n > 1)
        message = jtrans("A chorus of shrieks assaults your mind.");
    else
        message = jtrans("The starcursed mass shrieks in your mind.");
    ...
    if (!target->is_player())
    {
        if (you.see_cell(target->pos()))
        {
            mprf(target->as_monster()->friendly() ? MSGCH_FRIEND_SPELL
                                                  : MSGCH_MONSTER_SPELL,
                 jtrans("%s writhes in pain as voices assail %s mind.").c_str(),
    ...
    if (!do_resist)
    {
        ...
        else
        {
            ...
            if (!do_resist)
            {
                ...
                mprf("%s",jtrans("The pull of her song draws you forwards.").c_str());

                if (swapping)
                {
                    if (monster_at(oldpos))
                    {
                        mprf(jtrans("Something prevents you from swapping places with %s.").c_str(),
                    ...
                    mprf(jtrans("You swap places with %s.").c_str(),
                         mon->name(DESC_THE).c_str());
                }
...
static bool _orange_statue_effects(monster* mons)
{
    ...
    if (foe && mons->can_see(foe) && !one_chance_in(3))
    {
        ...
        {
            if (foe->is_player())
                mprf(MSGCH_WARN, "%s",jtrans("A hostile presence attacks your mind!").c_str());
            else if (you.can_see(mons))
                mprf(MSGCH_WARN, jtrans("%s fixes %s piercing gaze on %s.").c_str(),
...
static int _battle_cry(monster* chief, battlecry_type type)
{
        ...
        && coinflip())
    {
        ...
        if (affected)
        {
            ...
            {
                mprf(MSGCH_SOUND, jtrans(messages[type][0]).c_str(),
            ...
            if (!seen_affected.empty())
            {
                ...
                if (seen_affected.size() == 1)
                {
                    ...
                    mprf(channel, jtrans(messages[type][1]).c_str(), who.c_str());
                }
                  else
                  {
                    bool generic = false;
					...
					//Fixme?:messages[type][2].c_str()//
                    mprf(channel, jtrans(messages[type][2]).c_str(),
                           chief->friendly() ? jtrans("Your").c_str() : jtrans("The").c_str(),
...
static bool _make_monster_angry(const monster* mon, monster* targ)
{
    ...
    if (you.can_see(mon))
    {
        ...
        {
            mprf(jtrans("%s calls on %s to defend %s!").c_str(),
                mon->name(DESC_THE).c_str(),
				mon->pronoun(PRONOUN_OBJECTIVE).c_str(),
                targ_name.c_str());
        }
        else
            mprf(jtrans("%s goads %s on!").c_str(), mon->name(DESC_THE).c_str(),
                 targ_name.c_str());
    }
...
static bool _seal_doors_and_stairs(const monster* warden)
{
    ...
    if (had_effect)
    {
        mprf(MSGCH_MONSTER_SPELL, jtrans("%s activates a sealing rune.").c_str(),
                (warden->visible_to(&you) ? warden->name(DESC_THE, true).c_str()
                                          : jtrans("Someone").c_str()));
        if (num_closed > 1)
...
void move_child_tentacles(monster* mons)
{
    ...
    for (unsigned i = 0; i < tentacles.size(); i++)
    {
        ...
        if (pull_constrictee)
        {
            if (you.can_see(tentacle))
            {
                mprf(jtrans("The tentacle pulls %s backwards!").c_str(),
                     constrictee->name(DESC_THE).c_str());
            }
...
bool mon_special_ability(monster* mons, bolt & beem)
{
    ...
    case MONS_LAVA_SNAKE:
        ...
        if (mons_should_fire(beem))
        {
            ...
            simple_monster_message(mons, jtrans("spits lava!").c_str());
            beem.fire();
            used = true;
        }
        break;
    ...
    case MONS_LIGHTNING_SPIRE:
        ...
        if (mons_should_fire(beem))
        {
            ...
            simple_monster_message(mons,
                                   jtrans("shoots out a bolt of electricity!").c_str());
            beem.fire();
            used = true;
        }
        break;
    ...
    case MONS_BOG_BODY:
        if (one_chance_in(8))
        {
            ...
            if (mons->visible_to(&you))
                simple_monster_message(mons, jtrans("begins to rapidly decay!").c_str());
        }
        break;
    ...
    case MONS_ALLIGATOR_SNAPPING_TURTLE:
            ...
            && !mons->has_ench(ENCH_WITHDRAWN))
        {
            ...
            simple_monster_message(mons, jtrans("withdraws into its shell!").c_str());
        }
        break;
    ...
    case MONS_BOULDER_BEETLE:
        ...
        if (!mons->has_ench(ENCH_ROLLING))
        {
            ...
            if (mons_is_fleeing(mons))
            {
                ...
                {
                //  behaviour_event(mons, ME_CORNERED);
                    simple_monster_message(mons, jtrans("curls into a ball and rolls away!").c_str());
            ...
            {
                simple_monster_message(mons, jtrans("curls into a ball and starts rolling!").c_str());
                boulder_start(mons, &beem);
            }
        }
        break;
    ...
    case MONS_MANTICORE:
        ...
        {
            make_mons_stop_fleeing(mons);
            simple_monster_message(mons, jtrans("flicks its tail!").c_str());
            beem.fire();
            used = true;
    ...
    case MONS_SIREN:
    {
            ...
            || mons->foe == MHITYOU && !already_mesmerised && coinflip())
        {
            ...
            {
                simple_monster_message(mons,
                    make_stringf(jtrans("chants %s song.").c_str(),
                    already_mesmerised ? jtrans("her luring").c_str() : jtrans("a haunting").c_str()).c_str(),
                    spl);
            ...
            else
            {
                ...
                if (already_mesmerised)
                    mpr(jtrans("You hear a luring song."), MSGCH_SOUND);
                else
                {
                    if (one_chance_in(4)) // reduce spamminess
                    {
                        if (coinflip())
                            mpr(jtrans("You hear a haunting song."), MSGCH_SOUND);
                        else
                            mpr(jtrans("You hear an eerie melody."), MSGCH_SOUND);
    ...
    case MONS_WRETCHED_STAR:
        ...
        if (one_chance_in(5))
        {
            if (cell_see_cell(you.pos(), mons->pos(), LOS_DEFAULT))
            {
                ...
                simple_monster_message(mons, jtrans("pulses with an eldritch light!").c_str());
    ...
    case MONS_VAULT_SENTINEL:
                ...
                && one_chance_in(4))
        {
            if (you.can_see(mons))
            {
                simple_monster_message(mons, jtrans("blows on a signal horn!").c_str());
                noisy(25, mons->pos());
            }
            else
                noisy(25, mons->pos(), jtrans("You hear a note blown loudly on a horn!").c_str());
...
void mon_nearby_ability(monster* mons)
{
    ...
    case MONS_GOLDEN_EYE:
        if (_eyeball_will_use_ability(mons))
        {
            ...
            if (can_see && you.can_see(foe))
                mprf(jtrans("%s blinks at %s.").c_str(),
                     mons->name(DESC_THE).c_str(),
                     foe->name(DESC_THE).c_str());
    ...
    case MONS_GIANT_EYEBALL:
        if (_eyeball_will_use_ability(mons))
        {
            const bool can_see = you.can_see(mons);
            if (can_see && you.can_see(foe))
                mprf(jtrans("%s stares at %s.").c_str(),
                     mons->name(DESC_THE).c_str(),
                     foe->name(DESC_THE).c_str());
    ...
    case MONS_GHOST_MOTH:
        ...
        {
            if (you.can_see(mons))
                simple_monster_message(mons, jtrans("stares at you.").c_str());
            else
                canned_msg(MSG_BEING_WATCHED);
            ...
            if (one_chance_in(4))
            {
                                                        ...
                                                        MG_FORCE_PLACE)))
                {
                    ...
                    if (you.can_see(plant))
                        mprf("%s",jtrans("A ballistomycete grows in the wake of the spore.").c_str());
                }
                              ...
                              bool player_kill)
{
    ...
    if (candidates.empty())
    {
            ...
            && mons->attitude == ATT_HOSTILE)
        {
            if (player_kill)
            {
                mpr(jtrans("Having destroyed the fungal colony, you feel a bit more experienced."));
                gain_exp(200);
            }
...
void ancient_zyme_sicken(monster* mons)
{
        ...
        && cell_see_cell(you.pos(), mons->pos(), LOS_SOLID_SEE))
    {
        if (!you.disease)
        {
            ...
            {
                mprf(MSGCH_WARN, jtrans("You feel yourself growing ill in the presence of %s.").c_str(),
                    mons->name(DESC_THE).c_str());
            }
...
void starcursed_merge(monster* mon, bool forced)
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        {
            if (forced)
                simple_monster_message(mon, jtrans("shudders and is absorbed by its neighbour.").c_str());
    ...
    for (distance_iterator di(mon->pos(), true, true, 8); di; ++di)
    {
            ...
            && mon->can_see(ally))
        {
            ...
            if (moved)
            {
                simple_monster_message(mon, jtrans("shudders and withdraws towards its neighbour.").c_str());
