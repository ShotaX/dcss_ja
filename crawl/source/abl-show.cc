...
bool activate_ability()
{
    ...
    while (selected < 0)
    {
        msg::streams(MSGCH_PROMPT) << jtrans("Use which ability? (? or * to list)").c_str()
                                   << endl;
...
static bool _do_ability(const ability_def& abil)
{
    ...
    case ABIL_ZIN_RECITE:
    {
        ...
        if (zin_check_recite_to_monsters(&prayertype))
        {
            ...
            mprf(MSGCH_PLAIN, jtrans("You clear your throat and prepare to recite %s.").c_str(),
    ...
    case ABIL_OKAWARU_HEROISM:
        mprf(MSGCH_DURATION, "%s", you.duration[DUR_HEROISM]
             ? jtrans("You feel more confident with your borrowed prowess.").c_str()
             : jtrans("You gain the combat prowess of a mighty hero.").c_str());
    ...
    case ABIL_OKAWARU_FINESSE:
        ...
        mprf(MSGCH_DURATION, "%s", you.duration[DUR_FINESSE]
             ? jtrans("Your hands get new energy.").c_str()
             : jtrans("You can now deal lightning-fast blows.").c_str());
    ...
    case ABIL_FEDHAS_SPAWN_SPORES:
    {
        ...
        {
            if (retval == 0)
                mprf("%s", jtrans("No corpses are in range.").c_str());
            else
                canned_msg(MSG_OK);
            return false;
        }
    ...
    case ABIL_JIYVA_SLIMIFY:
    {
        ...
        const string msg = (weapon) ? weapon->name(DESC_YOUR)
                                    : (jtrans("your").c_str() + you.hand_name(true));
