...
spret_type cast_sticks_to_snakes(int pow, god_type god, bool fail)
{
    if (!you.weapon())
    {
        mprf(jtrans("Your %s feel slithery!").c_str(), you.hand_name(true).c_str());
        return SPRET_ABORT;
    }

    const item_def& wpn = *you.weapon();
    const string abort_msg = make_stringf(jtrans("%s feel slithery for a moment!").c_str(),
    ...
    mpr((count > 1) ? jtrans("You create some snakes!") : jtrans("You create a snake!"));
    return SPRET_SUCCESS;
}
                                 ...
                                 int unfriendly, int horde_penalty, bool fail)
{
    ...
    mpr(jtrans("Summon from material in which direction?"), MSGCH_PROMPT);
    ...
    if (const monster* m = monster_at(targ))
    {
        if (you.can_see(m))
        {
            mpr(jtrans("There's something there already!"));
            return SPRET_ABORT;
        }
        else
        {
            fail_check();
            mpr(jtrans("Something seems to disrupt your summoning."));
            return SPRET_SUCCESS; // still losing a turn
        }
    }
    else if (smove.delta.origin())
    {
        mpr(jtrans("You can't summon an elemental from yourself!"));
        return SPRET_ABORT;
    }
    else if (!in_bounds(targ))
    {
        // XXX: Should this cost a turn?
        mpr(jtrans("That material won't yield to your beckoning."));
        return SPRET_ABORT;
    }
    ...
    mpr(jtrans("An elemental appears!"));

    if (!friendly)
        mpr(jtrans("It doesn't seem to appreciate being summoned."));
...
spret_type cast_summon_ugly_thing(int pow, god_type god, bool fail)
{
                      ...
                      0, god)))
    {
        ...
        mpr((mon == MONS_VERY_UGLY_THING) ? jtrans("A very ugly thing appears.")
                                          : jtrans("An ugly thing appears."));
    }
    else
        canned_msg(MSG_NOTHING_HAPPENS);
                              ...
                              bool fail)
{
    ...
    mprf(jtrans("%s dances into the air!").c_str(), wpn->name(DESC_YOUR).c_str());
    ...
    if (why)
    {
        simple_god_message(jtrans("booms: How dare you animate that foul thing!").c_str());
...
spret_type cast_call_imp(int pow, god_type god, bool fail)
{
    ...
    {
        mpr((mon == MONS_WHITE_IMP)  ? jtrans("A beastly little devil appears in a puff of frigid air.") :
            (mon == MONS_IRON_IMP)   ? jtrans("A metallic apparition takes form in the air.") :
            (mon == MONS_SHADOW_IMP) ? jtrans("A shadowy apparition takes form in the air.")
                                     : jtrans("A beastly little devil appears in a puff of flame."));
                                  ...
                                  bool charmed, bool quiet)
{
                      ...
                      dur, spell, you.pos(), MHITYOU, MG_FORCE_BEH, god)))
    {
        ...
        {
            mpr(charmed ? jtrans("You don't feel so good about this...")
                        : jtrans("It doesn't seem very happy."));
        }
        else if (friendly)
        {
...
spret_type cast_malign_gateway(actor * caster, int pow, god_type god, bool fail)
{
    ...
    if (success)
    {
        ...
        noisy(10, point);
        mpr(jtrans("The dungeon shakes, a horrible noise fills the air, and a portal to some otherworldly place is opened!"), MSGCH_WARN);
...
static void _display_undead_motions(int motions)
{
    ...
    if (motions & DEAD_ARE_WALKING)
        motions_list.push_back(jtrans("walking"));
    if (motions & DEAD_ARE_HOPPING)
        motions_list.push_back(jtrans("hopping"));
    if (motions & DEAD_ARE_SWIMMING)
        motions_list.push_back(jtrans("swimming"));
    if (motions & DEAD_ARE_FLYING)
        motions_list.push_back(jtrans("flying"));
    if (motions & DEAD_ARE_SLITHERING)
        motions_list.push_back(jtrans("slithering"));

    // Prevents the message from getting too long and spammy.
    if (motions_list.size() > 3)
        mpr(jtrans("The dead have arisen!"));
    else
        mpr(jtrans("The dead are") + comma_separated_line(motions_list.begin(),
            motions_list.end()) + "!");
}
                           ...
                           monster **raised, int* motions_r)
{
    ...
    if (zombie_type == MONS_HYDRA && number == 0)
    {
        if (you.see_cell(pos))
        {
            mprf(jtrans("The headless hydra %s sways and immediately collapses!").c_str(),
                 item.sub_type == CORPSE_BODY ? jtrans("hycorpse").c_str() : jtrans("hyskeleton").c_str());
        }
        return false;
    }
    ...
    for (stack_iterator si(a); si; ++si)
    {
                ...
                || si->sub_type == CORPSE_SKELETON))
        {
            ...
            if (actual && success)
            {
                ...
                {
                    mprf(jtrans("The corpse you are %s rises to %s!").c_str(),
                         was_draining ? jtrans("drinking from").c_str()
                                      : jtrans("butchering").c_str(),
                         beha == BEH_FRIENDLY ? jtrans("join your ranks").c_str()
                                              : jtrans("attack").c_str());
                }
...
spret_type cast_simulacrum(int pow, god_type god, bool fail)
{
    ...
    {
        mpr(jtrans("You need to wield a piece of raw flesh for this spell to be effective!"));
        return SPRET_ABORT;
    }
...
static void _apport_and_butcher(monster *caster, item_def &item)
{
    ...
    if (item.pos != caster->pos())
    {
        ...
        if (is_being_drained(item))
            theft = jtrans("you were drinking from");
        else if (is_being_butchered(item))
            theft = jtrans("you were butchering");

        if (you.can_see(caster))
        {
            mprf(jtrans("%s casts a spell and %s%s float close.").c_str(),
        ...
        {
            mprf(jtrans("%s%s suddenly rise and float away!").c_str(),
                 item_name.c_str(),
                 theft.c_str());
        }
        if (!theft.empty())
    ...
    {
        mprf(jtrans("%s picks up %s%s.").c_str(),
             caster->name(DESC_THE).c_str(),
             item.name(apported ? DESC_THE : DESC_A).c_str(),
             item.base_type == OBJ_CORPSES ? jtrans("and starts butchering it").c_str()
                                           : "");
    }
    ...
    if (caster->inv[MSLOT_MISCELLANY] != NON_ITEM)
    {
            ...
            && mons_class_can_be_zombified(item.mon_type))
        {
                                  ...
                                  sim_type)))
                {
                    if (!created++ && cast_visible)
                    {
                        simple_monster_message(caster,
                            jtrans("holds a chunk of flesh high, and a cloud of icy vapour forms.").c_str(),
            ...
            if (cast_visible)
            {
                ...
                {
                    mprf(MSGCH_WARN,
                         jtrans("The vapour coalesces into ice likenesses of %s.").c_str(),
                ...
                else if (seen == 1)
                {
                    ...
                    mprf(MSGCH_WARN,
                         jtrans("The vapour coalesces into an ice likeness of %s.").c_str(),
                          ...
                          unsigned short foe, god_type god, bool actual)
{
    ...
    if (num_lost + num_crawlies + num_masses == 0)
        ...
        if (seen_lost)
     {
          mprf(jtrans("%s corpses collapse into a pulpy mess!").c_str(),
              _count_article(seen_lost, seen_crawlies + seen_masses == 0));
     }
  
     if (seen_crawlies > 0)
     {
          mprf(jtrans("%s corpse begins to drag itself along the ground!").c_str(),
              _count_article(seen_crawlies, seen_lost + seen_masses == 0));
     }
  
     if (seen_masses > 0)
     {
          mprf(jtrans("%s corpses meld into an agglomeration of writhing flesh!").c_str(),
...
spret_type cast_twisted_resurrection(int pow, god_type god, bool fail)
{
    ...
    {
        mpr(jtrans("There are no corpses nearby!"));
        return SPRET_ABORT;
    }
}
...
spret_type cast_haunt(int pow, const coord_def& where, god_type god, bool fail)
{
    ...
    if (success > 1)
    {
        mpr(friendly ? jtrans("Insubstantial figures form in the air.")
                     : jtrans("You sense hostile presences."));
    }
    else if (success)
    {
        mpr(friendly ? jtrans("An insubstantial figure forms in the air.")
                     : jtrans("You sense a hostile presence."));
    }
    else
    {
        canned_msg(MSG_NOTHING_HAPPENS);
...
static int _abjuration(int pow, monster *mon)
{
    ...
    if (mon->is_summoned(&duration))
    {
        ...
        if (mons_is_god_gift(mon, GOD_SHINING_ONE))
        {
            ...
            if (sockage < duration)
            {
                simple_god_message(jtrans("protects a fellow warrior from your evil magic!").c_str(),
                                   GOD_SHINING_ONE);
                shielded = true;
            }
        }
        else if (mons_is_god_gift(mon, GOD_TROG))
        {
            ...
            if (sockage < duration)
            {
                simple_god_message(jtrans("shields an ally from your puny magic!").c_str(),
                                   GOD_TROG);
                shielded = true;
            }
        }
        ...
        if (!mon->lose_ench_duration(abj, sockage) && !shielded)
            simple_monster_message(mon, jtrans("shudders.").c_str());
    }
...
spret_type cast_battlesphere(actor* agent, int pow, god_type god, bool fail)
{
    ...
    if (agent->is_player() && (battlesphere = find_battlesphere(&you)))
    {
        ...
        if (recalled)
        {
            mpr(jtrans("You recall your battlesphere and imbue it with additional charge."));
        }
        else
            ...
            else
            {
                if (you.can_see(agent) && you.can_see(battlesphere))
                {
                    simple_monster_message(agent->as_monster(),
                                       jtrans("conjures a globe of magical energy!").c_str());
                }
                else if (you.can_see(battlesphere))
                    simple_monster_message(battlesphere, jtrans("appears!").c_str());
    ...
    if (!killed)
    {
        if (agent && agent->is_player())
        {
            if (you.can_see(mons))
            {
                if ((mons->number == 0))
				{
                    mpr(jtrans("Your battlesphere expends the last of its energy and dissipates."));
				}
                else
...
bool fire_battlesphere(monster* mons)
{
    ...
    if (mons->props.exists("firing") && mons->number > 0)
    {
        ...
        {
            mpr(jtrans("Battlesphere targetting itself? Fixing."), MSGCH_ERROR);
            mons->props.erase("firing");
                    ...
                    != beam.path_taken.end()))
        {
            ...
            simple_monster_message(mons, jtrans("fires!").c_str());
            beam.fire();
...
spret_type cast_fulminating_prism(int pow, const coord_def& where, bool fail)
{
    ...
    if (prism)
        mpr(jtrans("You conjure a prism of explosive energy!"));
    else
        canned_msg(MSG_NOTHING_HAPPENS);
