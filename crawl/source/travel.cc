...
static void _explore_find_target_square()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (!estatus)
            {
                mpr(jtrans("Done exploring.")); //("Done exploring.")
            ...
            else
            {
                ...
                mprf(jtrans("Partly explored, can't reach some %s.").c_str(),
...
void explore_pickup_event(int did_pickup, int tried_pickup)
{
    ...
    if (tried_pickup && you.running == RMODE_EXPLORE_GREEDY)
    {
        ...
        {
            const string prompt =
                make_stringf(jtrans("Could not pick up %s here; shall I ignore %s?").c_str(),
                             tried_pickup == 1? "" : "",
                             tried_pickup == 1? "" : "");
...
command_type travel()
{
    ...
    {
        mprf(jtrans("Key pressed, stopping %s.").c_str(), you.running.runmode_name().c_str());
        stop_running();
        return CMD_NO_CMD;
    }
    ...
    if (you.confused())
    {
        mprf(jtrans("You're confused, stopping %s.").c_str(),
    ...
    {
        mprf(jtrans("You're in a travel-excluded area, stopping %s.").c_str(),
             you.running.runmode_name().c_str());
        stop_running();
        return CMD_NO_CMD;
    }
...
static int _prompt_travel_branch(int prompt_flags, bool* to_entrance)
{
    ...
    while (true)
    {
        ...
        string shortcuts = "(";
        {
            ...
            {
                if (waypoint_list)
                    segs.push_back(jtrans("* - list branches"));
                else if (waycount)
                    segs.push_back(jtrans("* - list waypoints"));
            }
        ...
        mprf(MSGCH_PROMPT, "%s? %s",
             *to_entrance ? jtrans("Entrance to where").c_str() : jtrans("Where to").c_str(),
             shortcuts.c_str());
                                          ...
                                          bool already_entrance)
{
    ...
    {
        mesclr();
        mprf(MSGCH_PROMPT, jtrans("What level of %s? (default %s, ? - help)").c_str(),
        ...
        if (lev && lev->sacrificeable(you.pos()))
        {
            ...
            if ((Options.auto_sacrifice == AS_YES
                 ...
                 || Options.auto_sacrifice == AS_PROMPT
                    && yesno(jtrans("Do you want to sacrifice the items here?").c_str(), true, 'n'))
...
void TravelCache::delete_waypoint()
{
    ...
    while (get_waypoint_count())
    {
        ...
        list_waypoints();
        mpr(jtrans("Delete which waypoint? (* - delete all, Esc - exit)"),
            MSGCH_PROMPT);
...
void TravelCache::add_waypoint(int x, int y)
{
    ...
    mprf(MSGCH_PROMPT, jtrans("Assign waypoint to what number? (0-9%s)").c_str(),
         waypoints_exist? jtrans(", D - delete waypoint").c_str() : "");
    ...
    if (overwrite)
    {
        if (lid == old_lid) // same level
            mprf(jtrans("Waypoint %d re-assigned to your current position.").c_str(), waynum);
        else
        {
            mprf(jtrans("Waypoint %d re-assigned from %s to %s.").c_str(),
                 waynum, old_dest.c_str(), new_dest.c_str());
        }
    }
    else
        mprf(jtrans("Waypoint %d assigned to %s.").c_str(), waynum, new_dest.c_str());
...
bool runrest::run_should_stop() const
{
    ...
    if (is_excluded(targ) && !is_stair_exclusion(targ))
    {
        ...
        mprf(MSGCH_WARN, jtrans("Stopped running for exclusion.").c_str());
#endif
        return true;
    }
    ...
    const C &coll, const char *category) const
{
    ...
    {
        mprf(jtrans("Found %s %s.").c_str(), number_in_words(size).c_str(), category);
        return;
    }

    const string message = comma_separated_line(coll.begin(), coll.end()) + (jtrans("Found"));
...
bool explore_discoveries::prompt_stop() const
{
    ...
    for (unsigned int i = 0; i < marked_feats.size(); i++)
        mprf(jtrans("Found %s").c_str(), marked_feats[i].c_str());
