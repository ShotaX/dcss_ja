...
static void _maybe_melt_player_enchantments(beam_type flavour, int damage)
{
        ...
        || flavour == BEAM_STEAM)
    {
        ...
        {
            mpr(jtrans("Your icy envelope dissipates!"), MSGCH_DURATION);
                       ...
                       bolt *beam, bool doEffects)
{
    ...
    case BEAM_BOLT_OF_ZIN:
    {
        ...
        if (doEffects)
        {
            if (hurted <= 0)
                ...
                mpr(jtrans("The blast sears you terribly!"));
            else
                mpr(jtrans("The blast sears you!"));
    ...
    case BEAM_LIGHT:
        ...
        if (hurted > original && doEffects)
        {
            mpr(jtrans("The light scorches you terribly!"));
            xom_is_stimulated(200);
        }
        break;
...
void splash_with_acid(int acid_strength, int death_source, bool corrode_items, string hurt_message)
{
    ...
    if (post_res_dam > 0)
    {
        mpr(hurt_message.empty() ? jtrans("The acid burns!") : hurt_message);
...
static bool _expose_invent_to_element(beam_type flavour, int strength)
{
    ...
    {
        simple_god_message(jtrans("protects your food from the spores.").c_str(),
                           GOD_FEDHAS);
        return false;
    }
    ...
    for (int i = 0; i < ENDOFPACK; ++i)
    {
               ...
               && you.inv[i].base_type == OBJ_CORPSES)
        {
            ...
            if (num_dest > 0)
            {
                ...
                {
                case OBJ_SCROLLS:
                    mprf(jtrans("%s %s catch fire!").c_str(),
                         part_stack_string(num_dest, quantity).c_str(),
                         item_name.c_str());
                    break;

                case OBJ_POTIONS:
                    mprf(jtrans("%s %s freeze and shatter!").c_str(),
                         part_stack_string(num_dest, quantity).c_str(),
                         item_name.c_str());
                    break;
                ...
                case OBJ_FOOD:
                    mprf(jtrans("%s %s %s!").c_str(),
                         ...
                         (flavour == BEAM_DEVOUR_FOOD) ?
                             jtrans("devoured").c_str() : jtrans("covered with spores").c_str());
                    break;

                default:
                    mprf(jtrans("%s %s destroyed!").c_str(),
                         part_stack_string(num_dest, quantity).c_str(),
                         item_name.c_str());
                    break;
                }
    ...
    {
        simple_god_message(
            make_stringf(jtrans("shields %s delectables from destruction.").c_str(),
                         (total_dest > 0) ? jtrans("some of your").c_str() : jtrans("your").c_str()).c_str(),
            GOD_JIYVA);
    }
                             ...
                             int strength)
{
    ...
    if (you.see_cell(where))
    {
        ...
        {
        case OBJ_SCROLLS:
            mprf(jtrans("You see %s of smoke.").c_str(),
                 (num_dest > 1) ? jtrans("some puffs").c_str() : jtrans("a puff").c_str());
            break;

        case OBJ_POTIONS:
            mprf(jtrans("You see %s shatter.").c_str(),
                 (num_dest > 1) ? jtrans("some glass").c_str() : jtrans("glass").c_str());
            break;

        case OBJ_FOOD:
            mprf(jtrans("You see %s of spores.").c_str(),
                 (num_dest > 1) ? jtrans("some clouds").c_str() : jtrans("a cloud").c_str());
            break;

        default:
            mprf(jtrans("%s on the floor destroyed!").c_str(),
                 (num_dest > 1) ? jtrans("Some items").c_str() : jtrans("An item").c_str());
            break;
        }
    }
...
static void _lose_level_abilities()
{
        ...
        && !you.wearing_ego(EQ_ALL_ARMOUR, SPARM_FLYING))
    {
        ...
        mpr(jtrans("You feel your flight won't last long."), MSGCH_WARN);
    }
}
...
void lose_level(int death_source, const char *aux)
{
    ...
    mprf(MSGCH_WARN,
         jtrans("You are now level %d!").c_str(), you.experience_level);
                                  ...
                                  kill_method_type death_type, int death_source)
{
        ...
        && !torment)
    {
        ...
        if (how_many > 0)
        {
            ...
            if (count_created > 0)
            {
                mprf(jtrans("You shudder from the %s and a %s!").c_str(),
                     death_type == KILLED_BY_MONSTER ? jtrans("blow").c_str() : jtrans("blast").c_str(),
                     count_created > 1 ? jtrans("flood of jellies pours out from you").c_str()
                                       : jtrans("jelly pops out").c_str());
            }
        }
    }
}
          ...
          bool attacker_effects)
{
    ...
    if (dam != INSTANT_DEATH)
    {
        ...
        {
            simple_god_message(jtrans("protects you from harm!").c_str());
            return;
        }
        ...
        if (dam > 0 && you.hp_max <= dam * 2)
            mpr(jtrans("Ouch! That really hurt!"), MSGCH_DANGER);
        ...
        if (you.hp > 0)
        {
            ...
            {
                mpr(jtrans("* * * LOW HITPOINT WARNING * * *"), MSGCH_DANGER);
...
void _end_game(scorefile_entry &se)
{
    ...
    {
        mprnojoin(jtrans("You die..."));      // insert player name here? {dlb}
        xom_death_message((kill_method_type) se.get_death_type());
        ...
        case GOD_KIKUBAAQUDGHA:
        {
            ...
            {
                simple_god_message(jtrans("rasps: \"You have failed me! Welcome... oblivion!\"").c_str());
            }
            else
            {
                simple_god_message(jtrans("rasps: \"You have failed me! Welcome... death!\"").c_str());
            }
            break;
        }
        ...
        case GOD_YREDELEMNUL:
            if (you.is_undead)
                simple_god_message(jtrans("claims you as an undead slave.").c_str());
            else if (se.get_death_type() != KILLED_BY_DISINT
                     && se.get_death_type() != KILLED_BY_LAVA)
            {
                mpr(jtrans("Your body rises from the dead as a mindless zombie."),
                    MSGCH_GOD);
            }
    ...
    clrscr();
    cprintf(jtrans("Goodbye, %s.").c_str(), you.your_name.c_str());
