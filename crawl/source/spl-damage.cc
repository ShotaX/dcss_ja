...
spret_type cast_delayed_fireball(bool fail)
{
    ...
    {
        mpr(jtrans("You are already charged."));
        return SPRET_ABORT;
    }
...
spret_type cast_fire_storm(int pow, bolt &beam, bool fail)
{
    ...
    {
        mpr(jtrans("You can't place the storm on a wall."));
        return SPRET_ABORT;
    }
...
bool cast_hellfire_burst(int pow, bolt &beam)
{
    ...
    mpr(jtrans("You call forth a pillar of hellfire!"));
...
spret_type cast_chain_lightning(int pow, const actor *caster, bool fail)
{
         ...
         pow -= 8 + random2(13), source = target)
    {
        ...
        const char* msg = jtrans("You hear a mighty clap of thunder!").c_str();
        noisy(25, source, (first || !see_source) ? msg : NULL);
        first = false;

        if (see_source && !see_targ)
            mpr(jtrans("The lightning arcs out of your line of sight!"));
        else if (!see_source && see_targ)
            mpr(jtrans("The lightning arc suddenly appears!"));
    ...
    if (!affected_monsters.empty())
      {
        ...
        if (!seen_monsters.empty())
        {
            ...
            const string message =
                make_stringf(jtrans("%s frozen.").c_str(),
            ...
            if (strwidth(message) < get_number_of_cols() - 2)
            ...
            else
              {
                ...
                // affected.
                mprf(jtrans("The monsters around %s are frozen!").c_str(),
                               ...
                               bool actual, bool added_effects)
{
    ...
    if (actual && hurted > 0)
      {
        mpr(jtrans("You feel very cold.").c_str());
                          ...
                          bool actual, bool added_effects)
{
    if (actual)
    {
        if (agent->is_player())
        {
            mprf(jtrans("You draw life from %s.").c_str(),
    ...
    if (total_damage)
      {
        if (agent->is_player())
          {
            mpr(jtrans("You feel life flooding into your body.").c_str());
            inc_hp(total_damage);
        }
        else if (agent)
        {
            ...
            if (mons->heal(total_damage))
                simple_monster_message(mons, jtrans("is healed.").c_str());
          }
      }
}
    ...
    switch(spell)
      {
        case SPELL_OZOCUBUS_REFRIGERATION:
            player_msg = jtrans("The heat is drained from your surroundings.");
            global_msg = jtrans("Something drains the heat from around you.");
            mons_vis_msg = jtrans("drains the heat from the surrounding environment!");
            mons_invis_msg = jtrans("The ambient heat is drained!");
...
//Fixme_ja 
        case SPELL_DRAIN_LIFE:
            player_msg = jtrans("You draw life from your surroundings.");
            global_msg = jtrans("Something draws the life force from your surroundings.");
            mons_vis_msg = jtrans("draws from the surrounding life force!");
            mons_invis_msg = jtrans("The surrounding life force dissipates!");
...
void sonic_damage(bool scream)
{
    ...
    */
    ...
    {
        const string message =
            make_stringf(jtrans("%s hurt by the noise.").c_str(),
...
spret_type vampiric_drain(int pow, monster* mons, bool fail)
{
    ...
    {
        mpr(jtrans("Draining that being is not a good idea."));
        return SPRET_ABORT;
    }
...
spret_type cast_freeze(int pow, monster* mons, bool fail)
{
    ...
    if (!abort && !fail)
    {
        ...
        mprf(jtrans("You freeze %s.").c_str(), mons->name(DESC_THE).c_str());
...
spret_type cast_airstrike(int pow, const dist &beam, bool fail)
{
    ...
    if (mons->res_wind() > 0)
    {
        if (mons->observable())
        {
            mprf(jtrans("But air would do no harm to %s!").c_str(),
                 mons->name(DESC_THE).c_str());
            return SPRET_ABORT;
        }

        fail_check();
        mprf(jtrans("The air twists arounds and harmlessly tosses %s around.").c_str(),
             mons->name(DESC_THE).c_str());
    ...
    mprf(jtrans("The air twists around and strikes %s!").c_str(),
         mons->name(DESC_THE).c_str());
    noisy(4, beam.target);
...
static int _shatter_items(coord_def where, int pow, actor *)
{
    ...
    if (broke_stuff)
    {
        if (player_can_hear(where))
            mpr(jtrans("You hear glass break."), MSGCH_SOUND);
...
spret_type cast_shatter(int pow, bool fail)
{
    ...
    {
        noisy(30, you.pos());
        mpr(jtrans("The dungeon rumbles!"), MSGCH_SOUND);
    }
    ...
    if (dest && !silence)
        mpr(jtrans("Ka-crash!"), MSGCH_SOUND);
...
static int _shatter_player(int pow, actor *wielder, bool devastator = false)
{
    ...
    if (damage > 0)
    {
        mpr(damage > 15 ? jtrans("You shudder from the earth-shattering force.")
                        : jtrans("You shudder."));
        if (devastator)
...
bool mons_shatter(monster* caster, bool actual)
{
    ...
    {
        if (silence)
            mprf(jtrans("The dungeon shakes around %s!").c_str(),
                 caster->name(DESC_THE).c_str());
        else
        {
            noisy(30, caster->pos(), caster->mindex());
            mprf(MSGCH_SOUND, jtrans("The dungeon rumbles around %s!").c_str(),
                 caster->name(DESC_THE).c_str());
        }
    }
...
static int _ignite_poison_affect_item(item_def& item, bool in_inv)
{
        ...
        && get_weapon_brand(item) == SPWPN_VENOM)
    {
        ...
        {
            mprf(jtrans("%s bursts into flame!").c_str(),
    ...
    if (strength)
    {
        if (in_inv)
        {
            ...
            {
                mprf(jtrans("%s explode!").c_str(),
                     item.name(DESC_PLAIN).c_str());
            }
            else
            {
                mprf(jtrans("Your %s burn!").c_str(),
                     item.name(DESC_PLAIN).c_str());
            }
        }
...
static int _ignite_poison_monsters(coord_def where, int pow, int, actor *actor)
{
    ...
    if (damage > 0)
    {
        ...
        simple_monster_message(mon, jtrans("seems to burn from within!").c_str());
...
static int _ignite_poison_player(coord_def where, int pow, int, actor *actor)
{
    ...
    if (damage)
    {
        ...
        {
            mpr(jtrans("You feel that the poison has left your system."));
...
spret_type cast_ignite_poison(int pow, bool fail)
{
    ...
    mpr(jtrans("You ignite the poison in your surroundings!"));
...
static int _discharge_monsters(coord_def where, int pow, int, actor *)
{
    ...
    else
    {
        ...
        if (damage)
        {
            mprf(jtrans("%s is struck by lightning.").c_str(),
                 mons->name(DESC_THE).c_str());
...
spret_type cast_discharge(int pow, bool fail)
{
    ...
    if (dam == 0)
    {
        ...
        else
        {
            ...
            mprf(jtrans("%s blue arc ground harmlessly %s you.").c_str(),
                 plural ? jtrans("Some").c_str() : "",
                 plural ? jtrans("around").c_str() : (coinflip() ? jtrans("beside").c_str() :
                                      coinflip() ? jtrans("behind").c_str() : jtrans("before").c_str()));
        }
    }
    return SPRET_SUCCESS;
}
                              ...
                              const char **what, bool &destroy_wall, bool &hole)
{
  ...
  do_terrain:
                                ...
                                "veto_fragmentation") == "veto")
    {
        ...
        {
            mprf(jtrans("%s seems to be unnaturally hard.").c_str(),
                              ...
                              const coord_def target, bool fail)
{
    ...
    if (what != NULL) // Terrain explodes.
    {
        ...
        mprf(jtrans("The %s shatters!").c_str(), what);
        if (destroy_wall)
            nuke_wall(target);
    }
    ...
    {
        if (you.see_cell(target))
        mprf(jtrans("%s shatters!").c_str(), mon->name(DESC_THE).c_str());
