...
bool remove_sanctuary(bool did_attack)
{
    ...
    {
        if (seen_change)
            simple_god_message(jtrans("revokes the gift of sanctuary.").c_str(), GOD_ZIN);
...
void decrease_sanctuary_radius()
{
    ...
    {
        mpr(jtrans("The sanctuary starts shrinking."), MSGCH_DURATION);
        stop_running();
    }
    ...
    if (!size)
    {
        ...
        if (you.see_cell(env.sanctuary_pos))
            mpr(jtrans("The sanctuary disappears."), MSGCH_DURATION);
    }
}
...
void create_sanctuary(const coord_def& center, int time)
{
    ...
    if (trap_count > 0)
    {
        mpr(jtrans("By Zin's power, hidden traps are revealed to you."),
            MSGCH_GOD);
    }

    if (cloud_count == 1)
    {
        mpr(jtrans("By Zin's power, the foul cloud within the sanctuary is swept away."), MSGCH_GOD);
    }
    else if (cloud_count > 1)
    {
        mpr(jtrans("By Zin's power, all foul fumes within the sanctuary are swept away."), MSGCH_GOD);
    }

    if (blood_count > 0)
    {
        mpr(jtrans("By Zin's power, all blood is cleared from the sanctuary."),
            MSGCH_GOD);
    }

    if (scare_count == 1 && seen_mon != NULL)
        simple_monster_message(seen_mon, jtrans(" turns to flee the light!").c_str());
