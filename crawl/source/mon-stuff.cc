...
bool curse_an_item(bool quiet)
{
    ...
    if (you_worship(GOD_ASHENZARI))
    {
        if (!quiet)
        {
            mprf(MSGCH_GOD, jtrans("The curse is absorbed by %s.").c_str(),
                         ...
                         bool force)
{
    ...
    if (you.see_cell(mons->pos()))
    {
        ...
        {
            if (you.can_see(mons))
                simple_monster_message(mons, jtrans("turns back into a corpse!").c_str());
            else
            {
                mprf(jtrans("%s appears out of nowhere!").c_str(),
...
{
    return (killer == KILL_BANISHED ? jtrans("banished").c_str() :
            killer == KILL_PACIFIED ? jtrans("pacified").c_str() :
            killer == KILL_ENSLAVED ? jtrans("enslaved").c_str() : jtrans("killed").c_str());
}
...
static bool _ely_protect_ally(monster* mons, killer_type killer)
{
    ...
    snprintf(info, INFO_SIZE, jtrans("protects %s from harm!").c_str(),
...
static void _mummy_curse(monster* mons, killer_type killer, int index)
{
        ...
        && YOU_KILL(killer))
    {
        ...
        mpr(jtrans("You feel nervous for a moment..."), MSGCH_MONSTER_SPELL);
        curse_an_item();
    }
    else
    {
        if (index == NON_MONSTER)
        {
            mpr(jtrans("You feel extremely nervous for a moment..."),
                MSGCH_MONSTER_SPELL);
        }
        else if (you.can_see(target))
        {
            mprf(MSGCH_MONSTER_SPELL, jtrans("A malignant aura surrounds %s.").c_str(),
                             ...
                             int killer_index, bool pet_kill, bool wizard)
{
    ...
    if (type == MONS_GIANT_SPORE)
    {
        ...
        sanct_msg    = jtrans("By Zin's power, the giant spore's explosion is contained.").c_str();
    }
    else if (type == MONS_BALL_LIGHTNING)
    {
        _setup_lightning_explosion(beam, *mons);
        sanct_msg    = jtrans("By Zin's power, the ball lightning's explosion is contained.").c_str();
    }
    else if (type == MONS_LURKING_HORROR)
        sanct_msg = jtrans("The lurking horror fades away harmlessly.").c_str();
    else if (type == MONS_FULMINANT_PRISM)
    {
        _setup_prism_explosion(beam, *mons);
        sanct_msg = jtrans("By Zin's power, the prism's explosion is contained.").c_str();
    }
    else if (mons->has_ench(ENCH_INNER_FLAME))
    {
        ...
        sanct_msg       = jtrans("By Zin's power, the fiery explosion is contained.").c_str();
    ...
    if (you.can_see(mons))
    {
        ...
        if (is_sanctuary(mons->pos()))
            ...
            mprf(MSGCH_MONSTER_DAMAGE, MDAM_DEAD, jtrans("%s explodes!").c_str(),
...
static void _make_spectral_thing(monster* mons, bool quiet)
{
    if (mons->holiness() == MH_NATURAL && mons_can_be_zombified(mons))
    {
        ...
        {
            mprf("%s",jtrans("A glowing mist gathers momentarily, then fades.").c_str());
            return;
        }
                ...
                int killer_index, bool silent, bool wizard, bool fake)
{
    ...
    if (killer == KILL_YOU && you.berserk())
    {
            ...
            && !player_under_penance() && you.piety > random2(1000))
        {
            ...
            mpr(jtrans("You feel the power of Trog in you as your rage grows."),
                MSGCH_GOD, GOD_TROG);
        }
                 ...
                 && one_chance_in(30))
        {
            const int bonus = (2 + random2(4)) / 2;
            you.increase_duration(DUR_BERSERK, bonus);
            mpr(jtrans("Your amulet glows a violent red."));
        }
    }
    ...
    else if (mons->type == MONS_FULMINANT_PRISM && mons->number == 0)
    {
        ...
        {
            simple_monster_message(mons, jtrans("detonates feebly.").c_str(),
             ...
             || mons->type == MONS_TWISTER)
    {
        ...
        {
            simple_monster_message(mons, jtrans("dissipates!").c_str(),
                                   MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
            silent = true;
        }
    ...
    else if (mons->type == MONS_SIMULACRUM)
    {
        if (!silent && !mons_reset && !was_banished)
        {
            simple_monster_message(mons, jtrans("vapourises!").c_str(),
                                   MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
            silent = true;
        }
    ...
    else if (mons->type == MONS_DANCING_WEAPON)
    {
        ...
        if (!silent && !hard_reset && !was_banished)
        {
            if (!summoned_it)
            {
                simple_monster_message(mons, jtrans("falls from the air.").c_str(),
                                       MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
                silent = true;
            }
            else
        ...
        if (!silent && !mons_reset && !mons->has_ench(ENCH_SEVERED) && !was_banished)
        {
            if (you.can_see(mons))
            {
                mpr(silenced(mons->pos()) ?
                    jtrans("The tentacle is hauled back through the portal!") :
                    jtrans("With a roar, the tentacle is hauled back through the portal!"),
                    MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
            }
            silent = true;
        }
    ...
    switch (killer)
    {
        ...
        case KILL_YOU_CONF:     // You kill by confusion.
        {
            ...
            if (death_message)
            {
                ...
                {
                    mprf(MSGCH_MONSTER_DAMAGE, MDAM_DEAD, jtrans("%s is %s!").c_str(),
                         mons->name(DESC_THE).c_str(),
                         exploded                        ? jtrans("Killed-1").c_str() :
                         _wounded_damaged(targ_holy)     ? jtrans("Killed-2").c_str()
                                                         : jtrans("Killed-3").c_str());
                }
                else
                {
                    mprf(MSGCH_MONSTER_DAMAGE, MDAM_DEAD, jtrans("You %s %s!").c_str(),
						 mons->name(DESC_THE).c_str(),
                         exploded                        ? jtrans("Kill-1").c_str() :
                         _wounded_damaged(targ_holy)     ? jtrans("Kill-2").c_str()
                                                         : jtrans("Kill-3").c_str());
                }
            ...
            if (good_kill
                ...
                && random2(you.piety) >= piety_breakpoint(0))
              {
                ...
                if (hp_heal && you.hp < you.hp_max
                  ...
                  {
                    mprf("%s", jtrans("You feel a little better.").c_str());
                    inc_hp(hp_heal);
                }
            ...
            if (death_message)
            {
                const char* msg =
                    ...
                    _wounded_damaged(targ_holy)  ? "is destroyed!"
                                                 : "dies!";
                simple_monster_message(mons, jtrans(msg).c_str(), MSGCH_MONSTER_DAMAGE,
                                       MDAM_DEAD);
            }
                    ...
                    && !invalid_monster_index(killer_index))
                {
                        ...
                        && killer_mon->hit_points < killer_mon->max_hit_points)
                    {
                        simple_monster_message(killer_mon,
                                               jtrans("looks invigorated.").c_str());
        ...
        case KILL_MISCAST:
            if (death_message)
            {
                ...
                {
                    const char* msg =
                        exploded                     ? jtrans("MONKilled-1").c_str() :
                        _wounded_damaged(targ_holy)  ? jtrans("MONKilled-2").c_str()
                                                     : jtrans("MONKilled-3").c_str();
    ...
    {
        simple_monster_message(mons, jtrans("'s shape twists and changes as it dies.").c_str());
    }
                       ...
                       bool force_beh)
{
    ...
    if (source_tier == -1)
    {
        return simple_monster_message(mons,
            jtrans("'s appearance momentarily alters.").c_str());
    }
    relax = 1;
    ...
    if (targetc == RANDOM_MONSTER)
    {
        do
        {
            ...
            if (relax > 50)
                return simple_monster_message(mons, jtrans("shudders.").c_str());
        }
    ...
    if (!_valid_morph(mons, targetc))
        return simple_monster_message(mons, jtrans("looks momentarily different.").c_str());
    ...
    {
        mprf(jtrans("%s appears out of thin air!").c_str(), mons->name(DESC_A).c_str());
        autotoggle_autopickup(false);
    }
    else
...
void slimify_monster(monster* mon, bool hostile)
{
    ...
    {
        simple_monster_message(mon, jtrans("quivers momentarily.").c_str());
        return;
    }
...
void corrode_monster(monster* mons, const actor* evildoer)
{
             ...
             && mons->can_bleed() && !mons->res_acid())
    {
        ...
        if (you.can_see(mons))
        {
            mprf(jtrans("%s writhes in agony as %s flesh is eaten away!").c_str(),
                 mons->name(DESC_THE).c_str(),
...
bool swap_check(monster* mons, coord_def &loc, bool quiet)
{
    ...
    if (mons->caught())
    {
        if (!quiet)
        {
            simple_monster_message(mons,
                make_stringf(jtrans("is %s!").c_str(), held_status(mons)).c_str());
        }
        return false;
    }
    ...
    {
        if (!quiet)
            simple_monster_message(mons, jtrans("is being constricted!").c_str());
        return false;
    }
    ...
    if (!swap && !quiet)
    {
        ...
        simple_monster_message(mons, jtrans("cannot make way for you.").c_str());
...
string get_damage_level_string(mon_holy_type holi, mon_dam_level_type mdam)
{
    ...
    case MDAM_ALMOST_DEAD:
        ss << jtrans("ALMOST");
        ss << (_wounded_damaged(holi) ? jtrans("destroyed") : jtrans("dead"));
        return ss.str();
    ...
    ss << (_wounded_damaged(holi) ? jtrans("damaged") : jtrans("wounded"));
    return ss.str();
}
...
static void _mons_indicate_level_exit(const monster* mon)
{
    ...
    if (feat_is_gate(feat))
        simple_monster_message(mon, jtrans("passes through the gate.").c_str());
    else if (feat_is_travelable_stair(feat))
    {
        command_type dir = feat_stair_direction(feat);
        simple_monster_message(mon,
            make_stringf(jtrans("%s the %s.").c_str(),
                dir == CMD_GO_UPSTAIRS     ? jtrans("goes up").c_str() :
                dir == CMD_GO_DOWNSTAIRS   ? jtrans("goes down").c_str()
                                           : jtrans("takes").c_str(),
                feat_is_escape_hatch(feat) ? jtrans("escape hatch").c_str()
                                           : jtrans("stairs").c_str()).c_str());
    }
    else if (is_shaft)
    {
        simple_monster_message(mon,
            make_stringf(jtrans("%s the shaft.").c_str(),
                mons_flies(mon) ? jtrans("goes down").c_str()
                                : jtrans("jumps into").c_str()).c_str());
    }
}
                     ...
                     killer_type killer, int killnum)
{
        ...
        && !monster_habitable_grid(mons, grid))
    {
        ...
        {
            mprf(jtrans("%s falls into the %s!").c_str(),
                 mons->name(DESC_THE).c_str(),
        ...
        if (grid == DNGN_LAVA || mons->can_drown())
        {
            if (message)
            {
                if (grid == DNGN_LAVA)
                {
                    simple_monster_message(mons, jtrans("is incinerated.").c_str(),
                                           MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
                }
                else if (mons_genus(mons->type) == MONS_MUMMY)
                {
                    simple_monster_message(mons, jtrans("falls apart.").c_str(),
                                           MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
                }
                else
                {
                    simple_monster_message(mons, jtrans("drowns.").c_str(),
                                           MSGCH_MONSTER_DAMAGE, MDAM_DEAD);
                }
            }
...
void monster_teleport(monster* mons, bool instan, bool silent)
{
    ...
    if (!instan)
    {
        ...
        {
            if (!silent)
                simple_monster_message(mons, jtrans("seems more stable.").c_str());
        }
        else
        {
            if (!silent)
                simple_monster_message(mons, jtrans("looks slightly unstable.").c_str());
    ...
    if (!silent)
        simple_monster_message(mons, jtrans("disappears!").c_str());
    ...
    {
        if (was_seen)
            simple_monster_message(mons, jtrans("reappears nearby!").c_str());
        else
        {
            ...
            if (!interrupt_activity(AI_SEE_MONSTER, ai))
                simple_monster_message(mons, jtrans("appears out of thin air!").c_str());
        }
    }
...
static bool _mons_reaped(actor *killer, monster* victim)
{
    ...
    if (you.can_see(victim))
        mprf(jtrans("%s turns into a zombie!").c_str(), victim->name(DESC_THE).c_str());
    else if (you.can_see(zombie))
        mprf(jtrans("%s appears out of thin air!").c_str(), zombie->name(DESC_THE).c_str());
...
void debuff_monster(monster* mon)
{
    ...
    if (dispelled)
        simple_monster_message(mon, jtrans("s magical effects unravel!").c_str());
}
