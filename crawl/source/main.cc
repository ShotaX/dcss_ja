...
NORETURN static void _launch_game()
{
    ...
    {
        msg::stream << jtrans("Main-1") << (game_start? "" : jtrans("Main-2")) << "<yellow>, "
                    << you.your_name << " the "
    ...
    if (!crawl_state.game_is_tutorial())
        mpr(jtrans("Main-3")); //("Press <w>?</w> for a list of commands and other information.");
...
static void _take_starting_note()
{
    ...
    notestr << you.your_name << ", the "
            << species_name(you.species) << " "
            << you.class_name
            << jtrans(", began the quest for the Orb.");
...
static void _startup_hints_mode()
{
    ...
    msg::streams(MSGCH_TUTORIAL)
        << jtrans("Press any key to start the hints mode intro, or Escape to skip it.")
        << endl;
                             ...
                             bool known_shaft)
{
    ...
    if (you.beheld() && !you.confused())
    {
        ...
        mprf(jtrans("You cannot move away from %s!").c_str(),
    ...
    {
        mprf(jtrans("You can't do that while %s.").c_str(), held_status());
        return false;
    }
    ...
    {
        mpr(jtrans("You are carrying too much to climb upwards."));
        return false;
    }
...
static bool _prompt_dangerous_portal(dungeon_feature_type ftype)
{
    ...
    case DNGN_ENTER_ABYSS:
        return yesno(jtrans("If you enter this portal you will not be able to return immediately. Continue?").c_str(), false, 'n');

    case DNGN_MALIGN_GATEWAY:
        return yesno(jtrans("Are you sure you wish to approach this portal? There's no telling what its forces would wreak upon your fragile self.").c_str(), false, 'n');
...
{
    mprf(jtrans("You are a level %d %s %s.").c_str(),
    ...
    {
        mprf(jtrans("You are %d%% of the way to level %d.").c_str(), you.experience_level + 1, perc);
    }
    else
    {
        mpr(jtrans("I'm sorry, level 27 is as high as you can go."));
    ...
    if (you.species == SP_FELID)
    {
        ...
        perc = (nl - xl) * 100 - perc;
        mprf(you.lives < 2 ?
             jtrans("You'll get an extra life in %d.%02d levels' worth of XP.").c_str() :
             jtrans("If you died right now, you'd get an extra life in %d.%02d levels' worth of XP.").c_str(),
             perc / 100, perc % 100);
    }
...
static void _print_friendly_pickup_setting(bool was_changed)
{
    ...
    {
        mprf(jtrans("Your intelligent allies are %sforbidden to pick up anything at all.").c_str(),
             now.c_str());
    }
    else if (you.friendly_pickup == FRIENDLY_PICKUP_FRIEND)
    {
        mprf(jtrans("Your intelligent allies may %sonly pick up items dropped by allies.").c_str(),
             now.c_str());
    }
    else if (you.friendly_pickup == FRIENDLY_PICKUP_PLAYER)
    {
        mprf(jtrans("Your intelligent allies may %sonly pick up items dropped by you and your allies.").c_str()
             , now.c_str());
    }
    else if (you.friendly_pickup == FRIENDLY_PICKUP_ALL)
    {
        mprf(jtrans("Your intelligent allies may %spick up anything they need.").c_str(),
             now.c_str());
    }
    else
...
static void _do_remove_armour()
{
    ...
    int index = 0;
    if (armour_prompt(jtrans("Take off which item?").c_str(), &index, OPER_TAKEOFF))
...
static void _toggle_friendly_pickup()
{
    ...
    mpr(jtrans("Change to (d)efault, (n)othing, (f)riend-dropped, (p)layer, or (a)ll?").c_str(), MSGCH_PROMPT);
...
static void _do_display_map()
{
    ...
    mpr(jtrans("Move the cursor to view the level map, or type <w>?</w> for a list of commands."));
    flush_prev_message();
#endif
...
static void _do_list_gold()
{
    if (shopping_list.empty())
        mprf(jtrans("You have %d gold piece.").c_str(), you.gold);
    else
...
void process_command(command_type cmd)
{
    ...
    case CMD_TOGGLE_AUTOPICKUP:
        ...
        mprf(jtrans("Autopickup is now %s.").c_str(), Options.autopickup_on > 0 ? "on" : "off");
        break;
    ...
    case CMD_SAVE_GAME:
        if (yesno(jtrans("Save game and exit?").c_str(), true, 'n'))
            save_game(true);
        break;
    ...
    default:
        if (crawl_state.game_is_hints())
        {
            ...
            mpr(jtrans("Unknown command."), MSGCH_EXAMINE_FILTER);
...
static void _prep_input()
{
    ...
    if (you.seen_portals)
    {
        ...
        if (you.seen_portals == 1)
            mpr(jtrans("You have a vision of a gate."), MSGCH_GOD);
        else
            mpr(jtrans("You have a vision of multiple gates."), MSGCH_GOD);
    ...
    if (you.duration[dur] <= midpoint && old_dur > midpoint)
    {
        if (midmsg)
        {
            if (need_expiration_warning(dur))
                mprf(MSGCH_DANGER, jtrans("Careful! %s").c_str(), midmsg);
            else
                mpr(midmsg, chan);
        }
...
static void _decrement_paralysis(int delay)
{
    ...
    if (you.duration[DUR_PARALYSIS])
    {
        ...
        {
            mpr(jtrans("You can move again."), MSGCH_DURATION);
...
static void _decrement_petrification(int delay)
{
    ...
    {
        you.redraw_evasion = true;
        mprf(MSGCH_DURATION, jtrans("You turn to %s and can move again.").c_str(),
             you.form == TRAN_LICH ? jtrans("bone").c_str() :
             you.form == TRAN_ICE_BEAST ? jtrans("ice").c_str() :
             jtrans("flesh").c_str());
    }
...
static void _decrement_durations()
{
    ...
    if (you.duration[DUR_ICEMAIL_DEPLETED] > 0)
    {
        ...
        if (!you.duration[DUR_ICEMAIL_DEPLETED])
            mpr(jtrans("Your icy envelope is fully restored."), MSGCH_DURATION);
    ...
    if (_decrement_a_duration(DUR_ICY_ARMOUR, delay,
                              jtrans("Your icy armour evaporates.").c_str(), coinflip(),
                              you.props.exists("melt_armour") ? NULL
                                  : jtrans("Your icy armour starts to melt.").c_str()))
    {
        you.redraw_armour_class = true;
    }
    ...
    _decrement_a_duration(DUR_SILENCE, delay, jtrans("Your hearing returns.").c_str());

    _decrement_a_duration(DUR_REPEL_MISSILES, delay,
                          jtrans("You feel less protected from missiles.").c_str(),
                          coinflip(),
                          jtrans("Your repel missiles spell is about to expire...").c_str());

    _decrement_a_duration(DUR_DEFLECT_MISSILES, delay,
                          jtrans("You feel less protectedd from missiles.").c_str(),
                          coinflip(),
                          jtrans("Your deflect missiles spell is about to expire...").c_str());

    if (_decrement_a_duration(DUR_REGENERATION, delay,
                              NULL, coinflip(),
                              jtrans("Your skin is crawling a little less now.").c_str()))
    {
    ...
    _decrement_a_duration(DUR_JELLY_PRAYER, delay, jtrans("Your prayer is over.").c_str());
    ...
    if (you.duration[DUR_DIVINE_SHIELD] > 0)
    {
        if (you.duration[DUR_DIVINE_SHIELD] > 1)
        {
            ...
            if (you.duration[DUR_DIVINE_SHIELD] <= 1)
            {
                ...
                mpr(jtrans("Your divine shield starts to fade."), MSGCH_DURATION);
            }
        }
        ...
        if (you.duration[DUR_DIVINE_SHIELD] == 1 && !one_chance_in(3))
        {
            ...
            if (--you.attribute[ATTR_DIVINE_SHIELD] == 0)
            {
                ...
                mpr(jtrans("Your divine shield fades away."), MSGCH_DURATION);
            }
        }
    }
        ...
        if (you.duration[DUR_WEAPON_BRAND] <= 0)
        {
            ...
            case SPWPN_VORPAL:
                if (get_vorpal_type(weapon) == DVORP_SLICING)
                    msg += jtrans("seems blunter.");
                else
                    msg += jtrans("feels lighter.");
                break;
            case SPWPN_FLAME:
            case SPWPN_FLAMING:
                msg += jtrans("goes out.");
                break;
            case SPWPN_FREEZING:
                msg += jtrans("stops glowing.");
                break;
            case SPWPN_FROST:
                msg += jtrans("'s frost melts away.");
                break;
            case SPWPN_VENOM:
                msg += jtrans("stops dripping with poison.");
                break;
            case SPWPN_DRAINING:
                msg += jtrans("stops crackling.");
                break;
            case SPWPN_DISTORTION:
                msg += jtrans("seems straighter.");
                break;
            case SPWPN_PAIN:
                msg += jtrans("seems less pained.");
                break;
            case SPWPN_CHAOS:
                msg += jtrans("seems more stable.");
                break;
            case SPWPN_ELECTROCUTION:
                msg += jtrans("stops emitting sparks.");
                break;
            case SPWPN_HOLY_WRATH:
                msg += jtrans("'s light goes out.");
                break;
            case SPWPN_ANTIMAGIC:
                msg += jtrans("stops repelling magic.");
                calc_mp();
                break;
            default:
                msg += jtrans("seems inexplicably less special.");
                break;
            }
    ...
    _decrement_a_duration(DUR_BREATH_WEAPON, delay,
                          jtrans("You have got your breath back.").c_str(), 0, NULL,
                          MSGCH_RECOVERY);

    _decrement_a_duration(DUR_SWIFTNESS, delay,
                          jtrans("You feel sluggish.").c_str(), coinflip(),
                          jtrans("You start to feel a little slower.").c_str());
    _decrement_a_duration(DUR_RESISTANCE, delay,
                          jtrans("Your resistance to elements expires.").c_str(), coinflip(),
                          jtrans("You start to feel less resistant.").c_str());

    if (_decrement_a_duration(DUR_PHASE_SHIFT, delay,
                    jtrans("You are firmly grounded in the material plane once more.").c_str(),
                    coinflip(),
                    jtrans("You feel closer to the material plane.").c_str()))
    {
        you.redraw_evasion = true;
    }

    _decrement_a_duration(DUR_POWERED_BY_DEATH, delay,
                          jtrans("You feel less regenerative.").c_str());
    if (you.duration[DUR_POWERED_BY_DEATH] > 0)
        handle_pbd_corpses(true);

    _decrement_a_duration(DUR_TELEPATHY, delay, jtrans("You feel less empathic.").c_str());

    if (_decrement_a_duration(DUR_CONDENSATION_SHIELD, delay,
                              jtrans("Your icy shield evaporates.").c_str(),
                              coinflip(),
                              jtrans("Your icy shield starts to melt.").c_str()))
    {
        you.redraw_armour_class = true;
    }

    if (_decrement_a_duration(DUR_MAGIC_SHIELD, delay,
                              jtrans("Your magical shield disappears.").c_str()))
    {
        you.redraw_armour_class = true;
    }

    if (_decrement_a_duration(DUR_STONESKIN, delay, jtrans("Your skin feels tender.").c_str()))
        you.redraw_armour_class = true;
    ...
    _decrement_a_duration(DUR_CONTROL_TELEPORT, delay,
                          jtrans("You feel uncertain.").c_str(), coinflip(),
                          jtrans("You start to feel a little uncertain.").c_str());

    if (_decrement_a_duration(DUR_DEATH_CHANNEL, delay,
                              jtrans("Your unholy channel expires.").c_str(), coinflip(),
                              jtrans("Your unholy channel is weakening.").c_str()))
    {
        you.attribute[ATTR_DIVINE_DEATH_CHANNEL] = 0;
    }

    _decrement_a_duration(DUR_STEALTH, delay, jtrans("You feel less stealthy.").c_str());
    _decrement_a_duration(DUR_SLAYING, delay, jtrans("You feel less lethal.").c_str());

    if (_decrement_a_duration(DUR_INVIS, delay, jtrans("You flicker back into view.").c_str(),
                              coinflip(), jtrans("You flicker for a moment.").c_str()))
    {
        you.attribute[ATTR_INVIS_UNCANCELLABLE] = 0;
    }

    _decrement_a_duration(DUR_BARGAIN, delay, jtrans("You feel less charismatic.").c_str());
    _decrement_a_duration(DUR_CONF, delay, jtrans("You feel less confused.").c_str());
    _decrement_a_duration(DUR_LOWERED_MR, delay, jtrans("You feel less vulnerable to hostile enchantments.").c_str());
    _decrement_a_duration(DUR_SLIMIFY, delay, jtrans("You feel less slimy.").c_str(),
                          coinflip(), jtrans("Your slime is starting to congeal.").c_str());
    if (_decrement_a_duration(DUR_MISLED, delay,
                              jtrans("Your thoughts are your own once more.").c_str()))
    {
        end_mislead();
    }
    if (_decrement_a_duration(DUR_QUAD_DAMAGE, delay, NULL, 0,
                              jtrans("Quad Damage is wearing off.").c_str()))
    {
        invalidate_agrid(true);
    }
    _decrement_a_duration(DUR_MIRROR_DAMAGE, delay,
                          jtrans("Your dark mirror aura disappears.").c_str());
    if (_decrement_a_duration(DUR_HEROISM, delay,
                          jtrans("You feel like a meek peon again.").c_str()))
    {
            you.redraw_evasion      = true;
            you.redraw_armour_class = true;
    }
    _decrement_a_duration(DUR_FINESSE, delay, jtrans("Your hands slow down.").c_str());

    _decrement_a_duration(DUR_CONFUSING_TOUCH, delay,
                          ((string(jtrans("Your ").c_str()) + you.hand_name(true)) +
                          jtrans(" stop glowing.").c_str()).c_str());

    _decrement_a_duration(DUR_SURE_BLADE, delay,
                          jtrans("The bond with your blade fades away.").c_str());

    if (_decrement_a_duration(DUR_MESMERISED, delay,
                              jtrans("You break out of your daze.").c_str(),
    ...
    if (_decrement_a_duration(DUR_AFRAID, delay,
                              jtrans("Your fear fades away.").c_str(),
                              0, NULL, MSGCH_RECOVERY))
    {
    ...
    if (_decrement_a_duration(DUR_LIQUEFYING, delay,
                              jtrans("The ground is no longer liquid beneath you.").c_str()))
    {
        invalidate_agrid();
    }

    if (_decrement_a_duration(DUR_MIGHT, delay,
                              jtrans("You feel a little less mighty now.").c_str()))
    {
        notify_stat_change(STAT_STR, -5, true, jtrans("might running out").c_str());
    }

    if (_decrement_a_duration(DUR_AGILITY, delay,
                              jtrans("You feel a little less agile now.").c_str()))
    {
        notify_stat_change(STAT_DEX, -5, true, jtrans("agility running out").c_str());
    }

    if (_decrement_a_duration(DUR_BRILLIANCE, delay,
                              jtrans("You feel a little less clever now.").c_str()))
    {
        notify_stat_change(STAT_INT, -5, true, jtrans("brilliance running out").c_str());
    }
            ...
            || you.hunger + 100 <= HUNGER_STARVING + BERSERK_NUTRITION))
    {
            ...
            && one_chance_in(10 + player_mutation_level(MUT_BERSERK) * 25))
        {
            ...
            {
                mpr(jtrans("You pass out from exhaustion."), MSGCH_WARN);
        ...
        if (!you.duration[DUR_PARALYSIS] && !you.petrified())
            mpr(jtrans("You are exhausted."), MSGCH_WARN);
        ...
        notify_stat_change(STAT_STR, -5, true, jtrans("berserk running out").c_str());
    ...
    if (_decrement_a_duration(DUR_CORONA, delay) && !you.backlit())
        mpr(jtrans("You are no longer glowing."), MSGCH_DURATION);
    ...
    if (you.duration[DUR_DISJUNCTION])
    {
        ...
        _decrement_a_duration(DUR_DISJUNCTION, delay,
                              jtrans("The translocation energy dissipates.").c_str());
        if (!you.duration[DUR_DISJUNCTION])
            invalidate_agrid(true);
    }

    if (_decrement_a_duration(DUR_TORNADO_COOLDOWN, delay,
                              jtrans("The winds around you calm down.").c_str()))
    {
    ...
    if (you.duration[DUR_TORNADO])
    {
        ...
        _decrement_a_duration(DUR_TORNADO, delay,
                              jtrans("The winds around you start to calm down.").c_str());
        if (!you.duration[DUR_TORNADO])
    ...
    if (you.duration[DUR_FLIGHT])
    {
        if (!you.permanent_flight())
        {
            if (_decrement_a_duration(DUR_FLIGHT, delay,
                                      0,
                                      random2(6),
                                      jtrans("You are starting to lose your buoyancy.").c_str()))
            {
                land_player();
            }
        }
    ...
    if (you.rotting > 0)
    {
        ...
        {
            mpr(jtrans("You feel your flesh rotting away."), MSGCH_WARN);
            rot_hp(1);
            you.rotting--;
        }
    }
    ...
    if (you.duration[DUR_DEATHS_DOOR])
    {
        ...
        if (_decrement_a_duration(DUR_DEATHS_DOOR, delay,
                              jtrans("Your life is in your own hands again!").c_str(),
                              random2(6),
                              jtrans("Your time is quickly running out!").c_str()))
        {
    ...
    if (_decrement_a_duration(DUR_SCRYING, delay,
                              jtrans("Your astral sight fades away.").c_str()))
    {
        you.xray_vision = false;
    }

    _decrement_a_duration(DUR_LIFESAVING, delay,
                          jtrans("Your divine protection fades away.").c_str());

    if (_decrement_a_duration(DUR_DARKNESS, delay,
                          jtrans("The ambient light returns to normal.").c_str())
        || you.haloed())
    {
    ...
    _decrement_a_duration(DUR_SHROUD_OF_GOLUBRIA, delay,
                          jtrans("Your shroud unravels.").c_str(),
                          0,
                          jtrans("Your shroud begins to fray at the edges.").c_str());

    _decrement_a_duration(DUR_INFUSION, delay,
            jtrans("Your attacks are no longer magically infused.").c_str(),
            0,
            jtrans("You are feeling less magically infused.").c_str());

    _decrement_a_duration(DUR_SONG_OF_SLAYING, delay,
            jtrans("Your song has ended.").c_str(),
            0,
            jtrans("Your song is almost over.").c_str());

    _decrement_a_duration(DUR_SONG_OF_SHIELDING, delay,
            jtrans("Your magic is no longer protecting you.").c_str(),
            0,
            jtrans("You are feeling less protected by your magic.").c_str());

      _decrement_a_duration(DUR_SENTINEL_MARK, delay,
                            jtrans("The sentinel's mark upon you fades away.").c_str());
  
    _decrement_a_duration(DUR_WEAK, delay,
                          jtrans("Your attacks no longer feel as feeble.").c_str());

    _decrement_a_duration(DUR_DIMENSION_ANCHOR, delay,
                          jtrans("You are no longer firmly anchored in space.").c_str());

      _decrement_a_duration(DUR_SICKENING, delay);
  
    _decrement_a_duration(DUR_ANTIMAGIC, delay,
                          jtrans("You regain control over your magic.").c_str());
    ...
    _decrement_a_duration(DUR_RETCHING, delay, jtrans("Your fit of retching subsides.").c_str());
    ...
    _decrement_a_duration(DUR_FIRE_VULN, delay,
                          jtrans("You feel less vulnerable to fire.").c_str());
...
static void _check_banished()
{
    if (you.banished)
    {
        ...
        you.banished = false;
        if (!player_in_branch(BRANCH_ABYSS))
            mpr(jtrans("You are cast into the Abyss!"), MSGCH_BANISHMENT);
        else if (you.depth < brdepth[BRANCH_ABYSS])
            mpr(jtrans("You are cast deeper into the Abyss!"), MSGCH_BANISHMENT);
        else
            mpr(jtrans("The Abyss bends around you!"), MSGCH_BANISHMENT);
        more();
...
static void _player_reacts()
{
    ...
    if (one_chance_in(10))
    {
        ...
        {
            mpr(jtrans("You are suddenly pulled into a different region of the Abyss!"),
                MSGCH_BANISHMENT);
...
static void _player_reacts_to_monsters()
{
    ...
    if (what != "")
        mprf(MSGCH_DURATION, jtrans("The heat melts your icy %s.").c_str(), what.c_str());
        ...
        if (trap && trap->type == TRAP_GOLUBRIA)
        {
            if (--trap->ammo_qty <= 0)
            {
                if (you.see_cell(*it))
                    ...
                    mpr(jtrans("You hear a snapping sound."), MSGCH_SOUND);
                trap->destroy();
                noisy(8, *it);
            }
        }
...
void world_reacts()
{
    ...
    if (you.elapsed_time >= 2*1000*1000*1000)
    {
        ...
        mpr(jtrans("Sorry, but your quest for the Orb is now rather pointless. You quit..."));
...
static bool _untrap_target(const coord_def move, bool check_confused)
{
    ...
    if (find_trap(target) && grd(target) != DNGN_UNDISCOVERED_TRAP)
    {
        if (!you.confused())
        {
            if (!form_can_wield())
            {
                mpr(jtrans("You can't disarm traps in your present form."));
                return true;
            }
            ...
            {
                mpr(jtrans("You can't get to that trap right now."));
                return true;
            }
        }
...
static void _open_door(coord_def move, bool check_confused)
{
    ...
    if (move.origin())
    {
        ...
        {
            mpr(jtrans("Which direction?"), MSGCH_PROMPT);
    ...
    if (!(check_confused && you.confused()))
    {
        ...
        {
            string prompt = make_stringf(jtrans("This %s%s is marked as excluded! Open it anyway?").c_str(), adj, noun);
    ...
    else
    {
        ...
        mprf(jtrans(verb).c_str(), adj, noun);
    }
...
static void _close_door(coord_def move)
{
    ...
    if (you.attribute[ATTR_HELD])
    {
        mprf(jtrans("You can't close doors while %s.").c_str(), held_status());
        return;
    }
        ...
        if (you.berserk())
        {
            ...
            else
            {
                if (!berserk_close.empty())
                {
                    ...
                    mprf(MSGCH_SOUND, jtrans("You slam the %s%s shut with a bang!").c_str(), adj, noun);
                noisy(15, you.pos());
            }
        }
        else if (one_chance_in(skill) && !silenced(you.pos()))
...
static void _do_berserk_no_combat_penalty(void)
{
    ...
    if (you.berserk())
    {
        ...
        {
        case 2:
            mpr(jtrans("You feel a strong urge to attack something."), MSGCH_DURATION);
            break;
        case 4:
            mpr(jtrans("You feel your anger subside."), MSGCH_DURATION);
            break;
        case 6:
            mpr(jtrans("Your blood rage is quickly leaving you."), MSGCH_DURATION);
            break;
        }
...
static void _move_player(coord_def move)
{
    ...
    if (you.confused())
    {
        ...
        {
            string prompt = jtrans("Are you sure you want to stumble around while confused and next to").c_str();
            ...
            if (targ && !targ->wont_attack() && you.can_see(targ))
                prompt += jtrans("(Use ctrl+direction to attack without moving)").c_str();
    ...
    if (fedhas_passthrough(targ_monst))
    {
        ...
        if (!current || !fedhas_passthrough(current))
        {
            ...
            mprf(jtrans("You walk carefully through the %s.").c_str(),
                 mons_genus(targ_monst->type) == MONS_FUNGUS ? jtrans("fungus").c_str()
                                                             : jtrans("plants").c_str());
        }
        targ_monst = NULL;
    }
    ...
    {
        mprf(jtrans("You cannot move closer to %s!").c_str(),
...
static void _do_cmd_repeat()
{
    ...
    int ch = _get_num_and_char(jtrans("Number of times to repeat, then command key:").c_str(),
                               buf, sizeof(buf));
    ...
    if (ch == ' ' || ch == CK_ENTER)
    {
        mpr(jtrans("Enter command to be repeated:"), MSGCH_PROMPT);
...
static void _do_prev_cmd_again()
{
    ...
    {
        mpr(jtrans("Trying to re-do re-do command, aborting."), MSGCH_ERROR);
        crawl_state.cancel_cmd_all();
        return;
    }
