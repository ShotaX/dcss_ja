...
unsigned short mons_word_of_recall(monster* mons, unsigned short recall_target)
{
    ...
    for (unsigned int i = 0; i < mon_list.size(); ++i)
    {
            ...
            && mon_list[i]->move_to_pos(empty))
        {
            ...
            mon_list[i]->foe = foe;
            ++num_recalled;
            simple_monster_message(mon_list[i], jtrans("is recalled.").c_str());
        }
...
bool handle_mon_spell(monster* mons, bolt &beem)
{
    ...
    else
    {
            ...
            && spell_cast != draco_breath)
        {
            ...
            simple_monster_message(mons, jtrans("falters for a moment.").c_str());
                                  ...
                                  int wont_attack)
{
    ...
    if (you_worship(GOD_SHINING_ONE))
    {
        ...
        if (pow < duration)
        {
            simple_god_message(jtrans("protects your fellow warrior from evil magic!").c_str());
            shielded = true;
        }
    }
    else if (you_worship(GOD_TROG))
    {
        ...
        if (pow < duration)
        {
            simple_god_message(jtrans("shields your ally from puny magic!").c_str());
            shielded = true;
        }
    }
    ...
    {
        pow = 0;
        mpr(jtrans("Zin's power protects your fellow warrior from evil magic!"),
            MSGCH_GOD);
        shielded = true;
    }
    ...
    {
        if (!shielded)
            simple_monster_message(target, jtrans("shudders.").c_str());
        return 1;
    }
...
static bool _mons_vampiric_drain(monster *mons)
{
    ...
    if (!hp_cost)
    {
        simple_monster_message(mons,
                               jtrans("is infused with unholy energy, but nothing happens.").c_str(),
                               MSGCH_MONSTER_SPELL);
        return false;
    }
    ...
    {
        simple_monster_message(mons,
                               jtrans("is infused with unholy energy.").c_str(),
                               MSGCH_MONSTER_SPELL);
    }
    else
    ...
    if (target->is_player())
    {
        ...
        {
            simple_monster_message(mons,
                jtrans("draws life force from you and is healed!").c_str());
        }
    }
    else
    {
        ...
        {
            simple_monster_message(mons,
                make_stringf(jtrans("draws life force from %s and is healed!").c_str(),
...
static int _mons_mesmerise(monster* mons, bool actual)
{
    ...
    {
        simple_monster_message(mons, jtrans("attempts to bespell you!").c_str());
...
static int _mons_cause_fear(monster* mons, bool actual)
{
    ...
    {
        if (you.can_see(mons))
            simple_monster_message(mons, jtrans("radiates an aura of fear!").c_str());
        else
    ...
    for (actor_iterator ai(mons->get_los()); ai; ++ai)
    {
        ...
        else
        {
                ...
                && m->add_ench(mon_enchant(ENCH_FEAR, 0, mons)))
            {
                ...
                if (you.can_see(m))
                    simple_monster_message(m, jtrans("looks frightened!").c_str());
               ...
               bool do_noise, bool special_ability)
{
    ...
    case SPELL_INJURY_MIRROR:
        simple_monster_message(mons,
                               make_stringf(jtrans("offers %s to %s, and fills with unholy energy.").c_str(),
    ...
    {
        simple_monster_message(mons,
                               make_stringf(jtrans("invokes %s protection!").c_str(),
    ...
    case SPELL_SWIFTNESS:
        ...
        if (mons->type == MONS_ALLIGATOR)
        {
            ...
            simple_monster_message(mons, jtrans("puts on a burst of speed!").c_str());
        }
        else
            simple_monster_message(mons, jtrans("seems to move somewhat quicker.").c_str());
        return;

    case SPELL_STONESKIN:
    {
        if (you.can_see(mons))
            mprf(jtrans("%s skin hardens.").c_str(),
    ...
    case SPELL_SILENCE:
        ...
        simple_monster_message(mons, jtrans("'s surroundings become eerily quiet.").c_str());
        return;
    ...
    case SPELL_CALL_TIDE:
        if (player_in_branch(BRANCH_SHOALS))
        {
            ...
            if (simple_monster_message(
                    mons,
                    jtrans("sings a water chant to call the tide!").c_str()))
            {
    ...
    case SPELL_INK_CLOUD:
        ...
        simple_monster_message(
            mons,
            jtrans("squirts a massive cloud of ink into the water!").c_str());
        return;
    ...
    case SPELL_LEDAS_LIQUEFACTION:
        ...
        {
            mprf(jtrans("%s liquefies the ground around %s!").c_str(), mons->name(DESC_THE).c_str(),
                 mons->pronoun(PRONOUN_REFLEXIVE).c_str());
    ...
    case SPELL_CANTRIP:
    {
        ...
        else
        {
            ...
            const char* buff_msgs[] = { jtrans("glows brightly for a moment.").c_str(),
                                        jtrans("looks braver.").c_str(),
                                        jtrans("becomes somewhat translucent.").c_str(),
                                        jtrans("'s eyes start to glow.").c_str() };

            // Messages about the monster influencing you.
            const char* other_msgs[] = {
                jtrans("You feel troubled.").c_str(),
                jtrans("You feel a wave of unholy energy pass over you.").c_str()
            };
            ...
            {
                simple_monster_message(mons, jtrans("shimmers for a moment.").c_str(),
                                       channel);
            }
    ...
    case SPELL_BLINK_OTHER:
    {
        ...
        string msg = getSpeakString(mons->name(DESC_PLAIN) + jtrans("blink_other").c_str());
    ...
    case SPELL_BLINK_OTHER_CLOSE:
    {
        ...
        string msg = getSpeakString(mons->name(DESC_PLAIN)
                                    + jtrans("blink_other_close").c_str());
        if (!msg.empty() && msg != "__NONE")
        {
            mons_speaks_msg(mons, msg, MSGCH_TALK,
                            silenced(you.pos()) || silenced(mons->pos()));
    ...
    case SPELL_DEATHS_DOOR:
        if (!mons->has_ench(ENCH_DEATHS_DOOR))
        {
            ...
            simple_monster_message(mons,
                                   jtrans("stands defiantly in death's doorway!").c_str());
    ...
    {
        simple_monster_message(mons,
                               jtrans("'s wounds begin to heal before your eyes!").c_str());
        const int dur = BASELINE_DELAY
            * min(5 + roll_dice(2, (mons->hit_dice * 10) / 3 + 1), 100);
        mons->add_ench(mon_enchant(ENCH_REGENERATION, 0, mons, dur));
        return;
    ...
    case SPELL_OZOCUBUS_ARMOUR:
    {
        mprf(jtrans("A film of ice covers %s body!").c_str(),
        apostrophise(mons->name(DESC_THE)).c_str());
        const int power = (mons->hit_dice * 15) / 10;
    ...
    {
        simple_monster_message(mons,
            make_stringf(jtrans("begins to accept %s allies' injuries.").c_str(),
                                ...
                                bool innate, bool targeted)
{
    ...
    const string    spell_name = jtrans(spell_title(spell)).c_str();
