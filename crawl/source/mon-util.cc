...
void discover_mimic(const coord_def& pos, bool wake)
{
    ...
    if (act && !act->shove(name.c_str()))
    {
        ...
        mpr(jtrans("There is some commotion, and a hidden mimic gets squished!"));
        if (item)
    ...
    if (!mimic)
    {
        mpr(jtrans("Too many monsters on level, can't place mimic."), MSGCH_ERROR);
        if (item)
            destroy_item(*item, true);
        return;
    }
    ...
    if (mons_near(mimic))
    {
        mprf(MSGCH_WARN, jtrans("The %s is a mimic!").c_str(), name.c_str());
...
void mons_pacify(monster* mon, mon_attitude_type att, bool no_xp)
{
    ...
    if (mon->type == MONS_GERYON)
    {
        simple_monster_message(mon,
            make_stringf(jtrans("discards %s horn.").c_str(),
