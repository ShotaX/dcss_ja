...
static bool _tso_retribution()
{
    ...
    case 2: // summon holy warriors (3/7)
    {
        ...
        simple_god_message(success ? jtrans("sends the divine host to punish you for your evil ways!").c_str()
                                   : jtrans("'s divine host fails to appear.").c_str(), god);
    ...
    case 6: // either noisiness or silence (2/7)
        if (coinflip())
        {
            simple_god_message(jtrans("booms out: \"Take the path of righteousness! REPENT!\"").c_str(), god);
            noisy(25, you.pos()); // same as scroll of noise
        }
        else
        {
            god_speaks(god, jtrans("You feel the Shining One's silent rage upon you!").c_str());
            cast_silence(25);
        }
        break;
    }
...
static void _zin_remove_good_mutations()
{
    ...
    simple_god_message(jtrans("draws some chaos from your body!").c_str(), GOD_ZIN);
    ...
    {
        simple_god_message(jtrans("rids your body of chaos!").c_str(), GOD_ZIN);
...
static bool _zin_retribution()
{
    ...
    case 1:
    case 2: // recital
        simple_god_message(jtrans("recites the Axioms of Law to you!").c_str(), god);
        switch (random2(3))
        {
        case 0:
    ...
    case 3:
    case 4: // famine
        simple_god_message(jtrans("sends a famine down upon you!").c_str(), god);
        make_hungry(you.hunger / 2, false);
        break;
    case 5: // noisiness
        simple_god_message(jtrans("booms out: \"Turn to the light! REPENT!\"").c_str(), god);
        noisy(25, you.pos()); // same as scroll of noise
        break;
    case 6:
...
static void _ely_dull_inventory_weapons()
{
    ...
    if (num_dulled > 0)
    {
        ...
        simple_god_message(
            make_stringf(jtrans("dulls %syour weapons.").c_str(),
                         num_dulled == 1 ? "" : "").c_str(),
            GOD_ELYVILON);
    }
}
...
static bool _elyvilon_retribution()
{
    ...
    simple_god_message(jtrans(jtrans("'s displeasure finds you.").c_str()).c_str(), god);
    ...
    case 2: // mostly flavour messages
        MiscastEffect(&you, -god, SPTYP_POISON, one_chance_in(3) ? 1 : 0,
                      jtrans("the displeasure of Elyvilon").c_str());
        break;
...
static bool _cheibriados_retribution()
{
    ...
    case 4:
        simple_god_message(jtrans("adjusts the clock.").c_str(), god);
    ...
    case 2:
        ...
        {
            mpr(jtrans("You feel the world leave you behind!"), MSGCH_WARN);
    ...
    case 0:
        if (curse_an_item())
            simple_god_message(jtrans("makes up for lost time.").c_str(), god);
        else
            glammer = true;
    ...
    if (random2(you.experience_level) > 7 && !one_chance_in(5))
    {
        mgen_data temp =
            mgen_data::hostile_at(random_choose(MONS_EXECUTIONER, MONS_GREEN_DEATH,
                                  ...
                                  jtrans("the fury of Makhleb").c_str(),
        ...
        simple_god_message(success ? jtrans("sends a greater servant after you!").c_str()
                                   : jtrans("'s greater servant is unavoidably detained.").c_str(), god);
    }
    else
    {
        ...
        simple_god_message(count > 1 ? jtrans("sends minions to punish you.").c_str() :
                           count > 0 ? jtrans("sends a minion to punish you.").c_str()
                                     : jtrans("'s minions fail to arrive.").c_str(), god);
    }
...
static bool _kikubaaqudgha_retribution()
{
    ...
    god_speaks(god, coinflip() ? jtrans("You hear Kikubaaqudgha cackling.").c_str()
                               : jtrans("Kikubaaqudgha's malice focuses upon you.").c_str());
    ...
    if (coinflip())
    {
        ...
        do
        {
            MiscastEffect(&you, -god, SPTYP_NECROMANCY,
                          ...
                          random2avg(88, 3), jtrans("the malice of Kikubaaqudgha").c_str());
        }
    ...
    animate_dead(&you, 1 + random2(3), BEH_HOSTILE, MHITYOU, 0,
                 jtrans("the malice of Kikubaaqudgha").c_str(), GOD_KIKUBAAQUDGHA);
...
static bool _yredelemnul_retribution()
{
    ...
    if (random2(you.experience_level) > 4)
    {
        ...
        else
        {
            ...
            simple_god_message(count > 1 ? jtrans("sends servants to punish you.").c_str() :
                               count > 0 ? jtrans("sends a servant to punish you.").c_str()
                                         : jtrans("'s servants fail to arrive.").c_str(), god);
        }
    }
    else
    {
        simple_god_message(jtrans("'s anger turns toward you for a moment.").c_str(), god);
        MiscastEffect(&you, -god, SPTYP_NECROMANCY, 5 + you.experience_level,
                      random2avg(88, 3), jtrans("the anger of Yredelemnul").c_str());
    }
...
static bool _trog_retribution()
{
    ...
    if (coinflip())
    {
        ...
        simple_god_message(count > 1 ? jtrans("sends monsters to punish you.").c_str() :
                           count > 0 ? jtrans("sends a monster to punish you.").c_str()
                                     : jtrans("has no time to punish you... now.").c_str(),
                           god);
    }
    else if (!one_chance_in(3))
    {
        simple_god_message(jtrans("'s voice booms out, \"Feel my wrath!\"").c_str(), god);
        ...
        case 2:
            lose_stat(STAT_STR, 1 + random2(you.strength() / 5), true,
                      jtrans("divine retribution from Trog").c_str());
            break;
        ...
        case 3:
            ...
            {
                dec_penance(god, 3);
                mpr(jtrans("You suddenly pass out!"), MSGCH_WARN);
        ...
        case 5:
            ...
            {
                dec_penance(god, 1);
                mpr(jtrans("You suddenly feel exhausted!"), MSGCH_WARN);
                you.set_duration(DUR_EXHAUSTED, 200);
                slow_player(100);
            }
            break;
    ...
    else
    {
        ...
        //    fire magic. -- bwr
        dec_penance(god, 2);
        mpr(jtrans("You feel Trog's fiery rage upon you!"), MSGCH_WARN);
        MiscastEffect(&you, -god, SPTYP_FIRE, 8 + you.experience_level,
                      random2avg(98, 3), jtrans("the fiery rage of Trog").c_str());
    }
...
static bool _beogh_retribution()
{
    ...
    case 2: // send out one or two dancing weapons (12.5%)
    {
            ...
            if (monster *mon =
                create_monster(
                    mgen_data::hostile_at(MONS_DANCING_WEAPON,
                        jtrans("the wrath of Beogh").c_str(),
                        true, 0, 0, you.pos(), 0, god)))
            {
        ...
        {
            ostringstream msg;
            msg << jtrans("of electrocution at you.").c_str()
				<< jtrans("throwsno").c_str();
    ...
    default: // send orcs after you (3/8 to 5/8)
    {
        ...
        mgen_data temp = mgen_data::hostile_at(punisher, jtrans("the wrath of Beogh").c_str(),
                                               true, 0, 0, you.pos(),
                                               MG_PERMIT_BANDS, god);
        ...
        simple_god_message(
            mons ? jtrans("sends forth an army of orcs.").c_str()
                 : jtrans("is still gathering forces against you.").c_str(), god);
    }
    }
...
static bool _okawaru_retribution()
{
    ...
    simple_god_message(count > 0 ? jtrans("sends forces against you!").c_str()
                                 : jtrans("'s forces are busy with other wars.").c_str(), god);
...
static bool _sif_muna_retribution()
{
    ...
    simple_god_message(jtrans("'s wrath finds you.").c_str(), god);
    dec_penance(god, 1);
    ...
    case 1:
        lose_stat(STAT_INT, 1 + random2(you.intel() / 5), true,
                  jtrans("divine retribution from Sif Muna").c_str());
        break;
    ...
    case 7:
        if (!forget_spell())
            mpr(jtrans("You get a splitting headache."));
        break;
...
static bool _lugonu_retribution()
{
    ...
    {
        simple_god_message(jtrans("'s wrath finds you!").c_str(), god);
        MiscastEffect(&you, -god, SPTYP_TRANSLOCATION, 9, 90, jtrans("Lugonu's touch").c_str());
    ...
    simple_god_message(success ? jtrans("sends minions to punish you.").c_str()
                                : jtrans("'s minions fail to arrive.").c_str(), god);
...
static bool _vehumet_retribution()
{
    ...
    simple_god_message(jtrans("'s vengeance finds you.").c_str(), god);
    MiscastEffect(&you, -god, SPTYP_CONJURATION,
                   8 + you.experience_level, random2avg(98, 3),
                   jtrans("the wrath of Vehumet").c_str());
    return true;
}
...
static bool _jiyva_retribution()
{
    ...
    if (you.can_safely_mutate() && one_chance_in(7))
    {
        ...
        god_speaks(god, jtrans("You feel Jiyva alter your body.").c_str());

        for (int i = 0; i < mutat; ++i)
            mutate(RANDOM_BAD_MUTATION, jtrans("Jiyva's wrath").c_str(), true, false, true);
    }
    else if (there_are_monsters_nearby() && coinflip())
    {
        ...
        {
            simple_god_message(
                make_stringf(jtrans("'s putrescence saturates %s!").c_str(),
    ...
    {
            god_speaks(god, jtrans("Mutagenic energy floods into your body!").c_str());
    ...
    else
    {
        ...
        god_speaks(god, success ? jtrans("Some slimes ooze up out of the ground!").c_str()
                                : jtrans("The ground quivers slightly.").c_str());
    }
...
static bool _fedhas_retribution()
{
    ...
    case 0:
        ...
        {
            simple_god_message(jtrans("produces spores.").c_str(), GOD_FEDHAS);
            break;
        }
    ...
    case 1:
    {
        ...
        simple_god_message(jtrans("invokes the elements against you.").c_str(), GOD_FEDHAS);
    ...
    case 2:
    {
       ...
       temp.extra_flags |= (MF_NO_REWARD | MF_HARD_RESET);
        ...
        if (success)
        {
            god_speaks(god, jtrans("Plants grow around you in an ominous manner.").c_str());
            return false;
        }
...
bool divine_retribution(god_type god, bool no_bonus, bool force)
{
    ...
    if (do_more && one_chance_in(5) && you.experience_level < random2(37))
    {
        if (coinflip())
        {
            mpr(jtrans("The divine experience confuses you!"), MSGCH_WARN);
        ...
        else
        {
            if (you.duration[DUR_SLOW] < 180 * BASELINE_DELAY)
            {
                mpr(jtrans("The divine experience leaves you feeling exhausted!"),
                     MSGCH_WARN);
...
static string _get_beogh_speech(const string key)
{
    ...
    if (result.empty())
        return jtrans("Beogh is angry!").c_str();
...
static void _tso_blasts_cleansing_flame(const char *message)
{
        ...
        && !player_under_penance() && x_chance_in_y(you.piety, MAX_PIETY * 2))
    {
        god_speaks(you.religion,
                   make_stringf(jtrans("\"Mortal, I have averted the wrath of %s...this time.\"").c_str(),
    ...
    else
    {
        ...
        simple_god_message(jtrans("blasts you with cleansing flame!").c_str(),
                           GOD_SHINING_ONE);
...
static string _get_tso_speech(const string key)
{
    ...
    if (result.empty())
        return jtrans("The Shining One is angry!").c_str();
...
static bool _ely_holy_revenge(const monster *victim)
{
    ...
    if (msg.empty())
        msg = jtrans("Elyvilon is displeased.");
    ...
    mprf(jtrans("You %sare rebuked by divine will.").c_str(), targets.size() ? jtrans("and your allies").c_str()
                                                             : "");
                            ...
                            kill_method_type death_type)
{
    ...
    else
    {
        ...
        simple_god_message(jtrans("smites you!").c_str(), god);
