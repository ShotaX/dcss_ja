...
void monster::add_enchantment_effect(const mon_enchant &ench, bool quiet)
{
    ...
    case ENCH_SUBMERGED:
        ...
        if (!quiet && mons_near(this))
        {
            ...
            {
                mprf(jtrans("%s merges itself into the air.").c_str(),
                     name(DESC_A, true).c_str());
            }
            else if (type == MONS_TRAPDOOR_SPIDER)
            {
                mprf(jtrans("%s hides itself under the floor.").c_str(),
                     name(DESC_A, true).c_str());
            }
            else if (seen_context == SC_SURFACES)
            {
                ...
                mprf(jtrans("%s submerges.").c_str(), name(DESC_A, true).c_str());
        }
...
void monster::remove_enchantment_effect(const mon_enchant &me, bool quiet)
{
    ...
    case ENCH_HASTE:
        calc_speed();
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer moving quickly.").c_str());
        break;

    case ENCH_SWIFT:
        if (!quiet)
            if (type == MONS_ALLIGATOR)
                simple_monster_message(this, jtrans("slows down.").c_str());
            else
                simple_monster_message(this, jtrans("is no longer moving somewhat quickly.").c_str());
        break;
    ...
    case ENCH_SILENCE:
        ...
        if (!quiet && !silenced(pos()))
            if (alive())
                simple_monster_message(this, jtrans("becomes audible again.").c_str());
            else
                mprf(jtrans("As %s dies, the sound returns.").c_str(), name(DESC_THE).c_str());
        break;

    case ENCH_MIGHT:
        if (!quiet)
            simple_monster_message(this, jtrans("no longer looks unusually strong.").c_str());
        break;

    case ENCH_SLOW:
        if (!quiet && !liquefied_ground())
            simple_monster_message(this, jtrans("is no longer moving slowly.").c_str());
        calc_speed();
        break;
    ...
    case ENCH_STONESKIN:
        ...
        {
            mprf(jtrans("%s skin looks tender.").c_str(),
    ...
    case ENCH_OZOCUBUS_ARMOUR:
        ...
        if (!quiet && you.can_see(this))
        {
            mprf(jtrans("%s icy armour evaporates.").c_str(),
                 apostrophise(name(DESC_THE)).c_str());
        }
        break;
    ...
    case ENCH_PARALYSIS:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer paralysed.").c_str());
    ...
    case ENCH_PETRIFIED:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer petrified.").c_str());
    ...
    case ENCH_FEAR:
        if (holiness() == MH_NONLIVING || berserk_or_insane())
        {
            ...
            snprintf(info, INFO_SIZE,"%s", jtrans("stops retreating.").c_str());
        }
        else if (!mons_is_tentacle_or_tentacle_segment(type))
        {
            snprintf(info, INFO_SIZE, jtrans("seems to regain %s courage.").c_str(),
    ...
    case ENCH_CONFUSION:
        if (!quiet)
            simple_monster_message(this, jtrans("seems less confused.").c_str());
    ...
    case ENCH_INVIS:
        ...
        {
            if (!quiet)
                mprf(jtrans("%s appears from thin air!").c_str(), name(DESC_A, true).c_str());
    ...
    case ENCH_CHARM:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer charmed.").c_str());
    ...
    if (!quiet)
        {
            if (visible_to(&you))
                simple_monster_message(this, jtrans("stops glowing.").c_str());
            else if (has_ench(ENCH_INVIS) && mons_near(this))
            {
                mprf(jtrans("%s stops glowing and disappears.").c_str(),
    ...
    case ENCH_STICKY_FLAME:
        if (!quiet)
            simple_monster_message(this, jtrans("stops burning.").c_str());
        break;

    case ENCH_POISON:
        if (!quiet)
            simple_monster_message(this, jtrans("looks more healthy.").c_str());
        break;
    ...
    case ENCH_ROT:
        if (!quiet)
        {
            if (type == MONS_BOG_BODY)
                simple_monster_message(this, jtrans("'s decay slows.").c_str());
            else
                simple_monster_message(this, jtrans("is no longer rotting.").c_str());
        }
        break;
    ...
    case ENCH_HELD:
    {
        ...
        if (net != NON_ITEM)
        {
            ...
            if (!quiet)
                simple_monster_message(this, jtrans("breaks free.").c_str());
        }
        break;
    }
    ...
    case ENCH_ABJ:
        ...
        if (berserk())
            simple_monster_message(this, jtrans("is no longer berserk.").c_str());
    ...
    case ENCH_SUBMERGED:
        ...
        if (you.can_see(this))
        {
            ...
            else if (!quiet)
            {
                ...
                if (type == MONS_AIR_ELEMENTAL)
                {
                    mprf(channel, jtrans("%s forms itself from the air!").c_str(),
                                  name(DESC_A, true).c_str());
                }
                else if (type == MONS_TRAPDOOR_SPIDER)
                {
                    mprf(channel,
                         jtrans("%s leaps out from its hiding place under the floor!").c_str(),
                         name(DESC_A, true).c_str());
                }
                else if (type == MONS_LOST_SOUL)
                {
                    ...
                    mprf(jtrans("%s surfaces.").c_str(), name(DESC_A, true).c_str());
            }
        }
    ...
    case ENCH_SOUL_RIPE:
        if (!quiet)
        {
            simple_monster_message(this,
                                   jtrans("'s soul is no longer ripe for the taking.").c_str());
        }
        break;
    ...
    case ENCH_AWAKEN_FOREST:
        ...
        if (!quiet)
            forest_message(pos(), jtrans("The forest calms down.").c_str());
        break;

    case ENCH_BLEED:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer bleeding.").c_str());
        break;

    case ENCH_WITHDRAWN:
        if (!quiet)
            simple_monster_message(this, jtrans("emerges from its shell.").c_str());
        break;
    ...
    case ENCH_LIQUEFYING:
        ...
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer liquefying the ground.").c_str());
        break;
    ...
    case ENCH_DAZED:
        if (!quiet && alive())
                simple_monster_message(this, jtrans("is no longer dazed.").c_str());
        break;

    case ENCH_INNER_FLAME:
        if (!quiet && alive())
            simple_monster_message(this, jtrans("'s inner flame fades away.").c_str());
        break;

    case ENCH_ROLLING:
        calc_speed();
        if (!quiet && alive())
            simple_monster_message(this, jtrans("stops rolling.").c_str());
        break;
    ...
    case ENCH_MUTE:
        if (!quiet && alive())
                simple_monster_message(this, jtrans("is no longer mute.").c_str());
        break;

    case ENCH_BLIND:
        if (!quiet && alive())
            simple_monster_message(this, jtrans("is no longer blind.").c_str());
    ...
    case ENCH_DUMB:
        if (!quiet && alive())
            simple_monster_message(this, jtrans("is no longer stupefied.").c_str());
    ...
    case ENCH_MAD:
        if (!quiet && alive())
            simple_monster_message(this, jtrans("is no longer mad.").c_str());
    ...
    case ENCH_DEATHS_DOOR:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer invulnerable.").c_str());
        break;

    case ENCH_REGENERATION:
        if (!quiet)
            simple_monster_message(this, jtrans("is no longer regenerating.").c_str());
        break;

    case ENCH_WRETCHED:
        if (!quiet)
        {
            snprintf(info, INFO_SIZE, jtrans("seems to return to %s normal shape.").c_str(),
                     pronoun(PRONOUN_POSSESSIVE, true).c_str());
            simple_monster_message(this, info);
        }
        break;
...
void monster::apply_enchantment(const mon_enchant &me)
{
    ...
    case ENCH_INSANE:
        ...
        {
            simple_monster_message(this, jtrans("is no longer in an insane frenzy.").c_str());
    ...
    case ENCH_FATIGUE:
        if (decay_enchantment(en))
        {
            simple_monster_message(this, jtrans("looks more energetic.").c_str());
    ...
    case ENCH_MIRROR_DAMAGE:
        if (decay_enchantment(en))
            simple_monster_message(this, jtrans("'s dark mirror aura disappears.").c_str());
        break;
    ...
    case ENCH_HELD:
    {
        ...
        if (net == NON_ITEM)
        {
            ...
            if (trap && trap->type == TRAP_WEB)
            {
                if (coinflip())
                {
                    ...
                    {
                        simple_monster_message(this,
                            jtrans("struggles to get unstuck from the web.").c_str());
                    }
                    break;
                }
            ...
            && !berserk_or_insane() && type != MONS_DANCING_WEAPON)
        {
            if (mons_near(this) && !visible_to(&you))
                ...
                simple_monster_message(this, jtrans("struggles to escape the net.").c_str());
        ...
        {    // e.g. ogre, large zombie (large); centaur, naga, hydra (big).
            ...
            if (mons_near(this) && !visible_to(&you))
                ...
                simple_monster_message(this, jtrans("struggles against the net.").c_str());
            ...
            if (mitm[net].plus < -7)
            {
                if (mons_near(this))
                {
                    if (visible_to(&you))
                    {
                        mprf(jtrans("The net rips apart, and %s comes free!").c_str(),
                             name(DESC_THE).c_str());
                    }
                    else
    ...
    case ENCH_STICKY_FLAME:
    {
              ...
              || mons_intel(this) >= I_NORMAL && flight_mode()))
        {
            if (mons_near(this) && visible_to(&you))
            {
                mprf(ground_level() ? jtrans("The flames covering %s go out.").c_str()
                     : jtrans("%s dips into the water, and the flames go out.").c_str(),
        ...
        if (dam > 0)
        {
            simple_monster_message(this, jtrans("burns!").c_str());
                ...
                for (adjacent_iterator ai(pos()); ai; ++ai)
                {
                        ...
                        && coinflip())
                    {
                        ...
                        mon->add_ench(mon_enchant(ENCH_FEAR, dur + random2(20),
                                                  me.agent()));
                        if (visible_to(&you))
                            mprf(jtrans("%s catches fire!").c_str(), mon->name(DESC_A).c_str());
        ...
        if (decay_enchantment(en))
        {
            if (you.can_see(this))
            {
                if (type == MONS_PILLAR_OF_SALT)
                    mprf(jtrans("%s crumbles away.").c_str(), name(DESC_THE, false).c_str());
                else
                {
                    mprf(jtrans("A nearby %s withers and dies.").c_str(),
                                        ...
                                        && !actor_at(adjacent))
                {
                                                            ...
                                                            MG_FORCE_PLACE)))
                    {
                        ...
                        if (you.see_cell(adjacent) && you.see_cell(pos()))
                            mpr(jtrans("A ballistomycete spawns a giant spore."));
    ...
    case ENCH_PORTAL_TIMER:
    {
        if (decay_enchantment(en))
        {
            ...
            if (you.see_cell(base_position))
                mprf(jtrans("The portal closes; %s is severed.").c_str(), name(DESC_THE).c_str());
    ...
    case ENCH_PORTAL_PACIFIED:
    {
        if (decay_enchantment(en))
        {
            ...
            {
                if (you.can_see(this))
                    simple_monster_message(this, jtrans("suddenly becomes enraged!").c_str());
                else
    ...
    case ENCH_SEVERED:
    {
        simple_monster_message(this, jtrans("writhes!").c_str());
        coord_def base_position = props["base_position"].get_coord();
        maybe_bloodify_square(base_position);
        hurt(me.agent(), 20);
    }
    ...
    case ENCH_SILVER_CORONA:
        if (is_chaotic())
        {
            ...
            if (newdam > 0)
            {
                string msg = mons_has_flesh(this) ? jtrans("'s flesh").c_str() : "";
                msg += (dam < newdam) ? jtrans("is horribly charred!").c_str()
    ...
    case ENCH_WORD_OF_RECALL:
            ...
            || confused() || asleep() || has_ench(ENCH_FEAR))
        {
            ...
            if (you.can_see(this))
            {
                mprf(jtrans("%s word of recall is interrupted.").c_str(),
                     name(DESC_ITS).c_str());
            }
            break;
        }
