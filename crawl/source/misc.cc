...
bool maybe_coagulate_blood_potions_inv(item_def &blood)
{
    ...
    mprf(jtrans("You can't carry %s right now.").c_str(), coag_count > 1 ? "them" : "it");
...
void search_around()
{
    ...
    for (radius_iterator ri(you.pos(), max_dist); ri; ++ri)
    {
        ...
        {
            ptrap->reveal();
            mprf(jtrans("You found %s trap!").c_str(),
...
void merfolk_start_swimming(bool stepped)
{
    ...
    if (stepped)
        mpr(jtrans("Your legs become a tail as you enter the water."));
    else
        mpr(jtrans("Your legs become a tail as you dive into the water."));
...
bool go_berserk(bool intentional, bool potion)
{
    ...
    if (you.duration[DUR_FINESSE] > 0)
    {
        ...
        mpr(jtrans("Finesse? Hah! Time to rip out guts!"));
    }
    ...
    if (you_worship(GOD_CHEIBRIADOS))
    {
        ...
        if (intentional)
        {
            ...
            simple_god_message(jtrans("forces you to slow down.").c_str());
        }
        else
                 ...
                 bool check_dist, int range)
{
    if (!just_monsters)
    {
        ...
        if (in_bounds(you.pos()) && env.cgrid(you.pos()) != EMPTY_CLOUD)
        {
            ...
            if (is_damaging_cloud(type, want_move))
            {
                if (announce)
                {
                    mprf(MSGCH_WARN, jtrans("You're standing in a cloud of %s!").c_str(),
    ...
    if (visible.size() == 1)
    {
        ...
        const string monname = mons_is_mimic(m.type) ? "A mimic"
                                                     : m.name(DESC_A);
        msg = make_stringf(jtrans("%s is nearby!").c_str(), jtrans(monname).c_str());
    }
    else if (visible.size() > 1)
        msg = jtrans("There are monsters nearby!");
    else if (_exposed_monsters_nearby(want_move))
        msg = jtrans("There is a strange disturbance nearby!");
    else
        return true;
...
static void _drop_tomb(const coord_def& pos, bool premature, bool zin)
{
    ...
    {
        if (seen_change && !zin)
            mprf(jtrans("The walls disappear%s!").c_str(), premature ? jtrans(" prematurely").c_str() : "");
        else if (seen_change && zin)
        {
            mprf(jtrans("Zin %s %s %s.").c_str(),
                 (mon) ? mon->name(DESC_THE).c_str()
                       : jtrans("the silver walls,").c_str(),
                 (mon) ? jtrans("releases").c_str()
                       : jtrans("dismisses").c_str(),
                 (mon) ? make_stringf(jtrans("from %s prison").c_str(),
                             mon->pronoun(PRONOUN_POSSESSIVE).c_str()).c_str()
                       : jtrans("but there is nothing inside them").c_str());
        }
        else
        {
            if (!silenced(you.pos()))
                mpr(jtrans("You hear a deep rumble."), MSGCH_SOUND);
            else
...
void swap_with_monster(monster* mon_to_swap)
{
    ...
    mprf(jtrans("You swap places with %s.").c_str(), mon.name(DESC_THE).c_str());
    ...
    if (mon_caught)
    {
        if (you.body_size(PSIZE_BODY) >= SIZE_GIANT)
        {
            ...
            if (net != NON_ITEM)
                destroy_item(net);
            mprf(jtrans("The %s rips apart!").c_str(), (net == NON_ITEM) ? "web" : "net");
            you.attribute[ATTR_HELD] = 0;
            you.redraw_quiver = true;
        }
        else
...
void wear_id_type(item_def &item)
{
    ...
    mprf(jtrans("You are wearing: %s").c_str(),
...
void entered_malign_portal(actor* act)
{
    if (you.can_see(act))
    {
        mprf(jtrans("The portal repels %s, its terrible forces doing untold damage!").c_str(),
