...
int identify(int power, int item_slot, string *pre_msg)
{
    ...
    do
    {
        ...
        {
            mpr(jtrans("Choose an unidentified item, or Esc to abort."));
                          ...
                          targ_mode_type mode)
{
        ...
        && can_pacify <= 0)
    {
        if (can_pacify == 0)
        {
            mprf(jtrans("The light of Elyvilon fails to reach %s.").c_str(),
                 mons->name(DESC_THE).c_str());
            return 0;
        }
        else if (can_pacify == -3)
        {
            mprf(jtrans("The light of Elyvilon almost touches upon %s.").c_str(),
                 mons->name(DESC_THE).c_str());
            return 0;
        }
        else if (can_pacify == -4)
        {
            mprf(jtrans("%s is completely unfazed by your meager offer of peace.").c_str(),
                 mons->name(DESC_THE).c_str());
            return 0;
        }
        else
        {
            if (can_pacify == -2)
            {
                mprf(jtrans("You cannot pacify this monster while %s is sleeping!").c_str(),
        ...
        && is_hostile)
    {
        ...
        if (random2(1 + pgain))
            simple_god_message(jtrans("approves of your offer of peace.").c_str());
        else
            mpr(jtrans("Elyvilon supports your offer of peace."));

        if (is_holy)
            good_god_holy_attitude_change(mons);
        else
        {
            simple_monster_message(mons, jtrans("turns neutral.").c_str());
    ...
    {
        did_something = true;
        mprf(jtrans("You heal %s.").c_str(), mons->name(DESC_THE).c_str());

        if (mons->hit_points == mons->max_hit_points)
            simple_monster_message(mons, jtrans("is completely healed.").c_str());
        else
            print_wounds(mons);
    }
...
void antimagic()
{
    ...
    if (you.duration[DUR_PETRIFYING] > 0)
    {
        ...
        mpr(jtrans("You feel limber!"), MSGCH_DURATION);
    }

    if (you.attribute[ATTR_DELAYED_FIREBALL])
    {
        you.attribute[ATTR_DELAYED_FIREBALL] = 0;
        mpr(jtrans("Your charged fireball dissipates."), MSGCH_DURATION);
    }
    ...
    if (need_msg)
    {
        mprf(danger ? MSGCH_DANGER : MSGCH_WARN,
             jtrans("%sYour magical effects are unravelling.").c_str(),
             danger ? jtrans("Careful!").c_str() : "");
    }
...
bool remove_curse(bool alreadyknown, string *pre_msg)
{
    ...
    if (success)
    {
        ...
        mpr(jtrans("None of your equipped items are cursed."), MSGCH_PROMPT);
    else
    {
        if (pre_msg)
            mpr(*pre_msg);
...
static bool _selectively_curse_item(bool armour, string *pre_msg)
{
    while (1)
    {
        ...
        {
            mprf(jtrans("Choose an uncursed equipped piece of %s, or Esc to abort.").c_str(),
                 armour ? jtrans("armour").c_str() : jtrans("jewellery").c_str());
            if (Options.auto_list)
                more();
            continue;
        }
...
bool curse_item(bool armour, bool alreadyknown, string *pre_msg)
{
    ...
    if (affected == EQ_WEAPON)
    {
        if (you_worship(GOD_ASHENZARI) && alreadyknown)
        {
            mprf(MSGCH_PROMPT, jtrans("You aren't wearing any piece of uncursed %s.").c_str(),
                 armour ? jtrans("armour").c_str() : jtrans("jewellery").c_str());
        }
        else
        {
...
static bool _do_imprison(int pow, const coord_def& where, bool zin)
{
    ...
    if (zin)
    {
        ...
        if (!success)
        {
            mprf(none_vis ? jtrans("You briefly glimpse something next to %s.").c_str()
                        : jtrans("You need more space to imprison %s.").c_str(),
                targname.c_str());
            return false;
        }
    }
    ...
    if (number_built > 0)
    {
        if (zin)
        {
            mprf(jtrans("Zin imprisons %s with walls of pure silver!").c_str(),
                 targname.c_str());
        }
        else
...
bool cast_smiting(int pow, monster* mons)
{
    ...
    if (success)
    {
        ...
        mprf(jtrans("You smite %s!").c_str(), mons->name(DESC_THE).c_str());
