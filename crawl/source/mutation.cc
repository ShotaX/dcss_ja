...
string describe_mutations(bool center_title)
{
    string result;
    bool have_any = false;
    const char *mut_title = jtrans("Innate Abilities, Weirdness & Mutations").c_str();
    ...
    case SP_MERFOLK:
        result += _annotate_form_based(
            jtrans("You revert to your normal form in water.").c_str(),
            form_changed_physiology());
        result += _annotate_form_based(
            jtrans("You are very nimble and swift while swimming.").c_str(),
            form_changed_physiology());
        have_any = true;
        break;
    ...
    case SP_MINOTAUR:
        result += _annotate_form_based(
            jtrans("You reflexively headbutt those who attack you in melee.").c_str(),
    ...
    case SP_NAGA:
        ...
        {
            result += _annotate_form_based(
                jtrans("You can use your snake-like lower body to constrict enemies.").c_str(),
                !form_keeps_mutations());
        }
        ...
        if (you.experience_level > 2)
        {
            ...
            const string acstr = jtrans("Your serpentine skin is tough.") + "(AC +"+ num.str() + ")";
    ...
    case SP_GHOUL:
        result += jtrans("Your body is rotting away.\n").c_str();
        have_any = true;
        break;
    ...
    case SP_TENGU:
        ...
        {
            string msg = jtrans("You can fly").c_str();
            if (you.experience_level > 14)
                msg += jtrans("continuously").c_str();
            msg += ".\n";
    ...
    case SP_MUMMY:
        result += jtrans("You do not eat or drink.\n").c_str();
        result += jtrans("Your flesh is vulnerable to fire.\n").c_str();
        if (you.experience_level > 12)
        {
            result += jtrans("You are").c_str();
            if (you.experience_level > 25)
                result += jtrans("strongly").c_str();

            result += jtrans("in touch with the powers of death.\n").c_str();
            result +=
                jtrans("You can restore your body by infusing magical energy.\n").c_str();
        }
        have_any = true;
        break;

    case SP_GREEN_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe blasts of noxious fumes.").c_str());
        have_any = true;
        scale_type = "lurid green";
        break;

    case SP_GREY_DRACONIAN:
        result += jtrans("You can walk through water.\n").c_str();
        have_any = true;
        scale_type = "dull iron-grey";
        break;

    case SP_RED_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe blasts of fire.").c_str());
        have_any = true;
        scale_type = "fiery red";
        break;

    case SP_WHITE_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe waves of freezing cold.").c_str());
        result += _dragon_abil(jtrans("You can buffet flying creatures when you breathe cold.").c_str());
        scale_type = "icy white";
        have_any = true;
        break;

    case SP_BLACK_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe wild blasts of lightning.").c_str());
        if (you.experience_level >= 14)
            result += jtrans("You can fly continuously.\n").c_str();
        scale_type = "glossy black";
        have_any = true;
        break;

    case SP_YELLOW_DRACONIAN:
        result += _dragon_abil(jtrans("You can spit globs of acid.").c_str());
        result += _dragon_abil(jtrans("You can corrode armour when you spit acid.").c_str());
        result += _annotate_form_based(jtrans("You are resistant to acid.").c_str(),
    ...
    case SP_PURPLE_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe bolts of energy.").c_str());
        result += _dragon_abil(jtrans("You can dispel enchantments when you breathe energy.").c_str());
        scale_type = "rich purple";
        have_any = true;
        break;

    case SP_MOTTLED_DRACONIAN:
        result += _dragon_abil(jtrans("You can spit globs of burning liquid.").c_str());
        result += _dragon_abil(jtrans("You can ignite nearby creatures when you spit burning liquid.").c_str());
        scale_type = "weird mottled";
        have_any = true;
        break;

    case SP_PALE_DRACONIAN:
        result += _dragon_abil(jtrans("You can breathe blasts of scalding steam.").c_str());
        scale_type = "pale cyan-grey";
        have_any = true;
        break;

    case SP_KOBOLD:
        result += jtrans("You recuperate from illness quickly.\n").c_str();
        have_any = true;
        break;
    ...
    case SP_VAMPIRE:
        ...
        if (you.hunger_state == HS_STARVING)
            result += jtrans("<green>You do not heal naturally.</green>\n").c_str();
        else if (you.hunger_state == HS_ENGORGED)
            result += jtrans("<green>Your natural rate of healing is extremely fast.</green>\n").c_str();
        else if (you.hunger_state < HS_SATIATED)
            result += jtrans("<green>You heal slowly.</green>\n").c_str();
        else if (you.hunger_state >= HS_FULL)
            result += jtrans("<green>Your natural rate of healing is unusually fast.</green>\n").c_str();
        else
            have_any = false;

        if (you.experience_level >= 6)
        {
            result += jtrans("You can bottle blood from corpses.\n").c_str();
            have_any = true;
        }
        break;

    case SP_DEEP_DWARF:
        result += jtrans("You are resistant to damage.\n").c_str();
        result += jtrans("You can recharge devices by infusing magical energy.\n").c_str();
        have_any = true;
        break;

    case SP_FELID:
        result += jtrans("You cannot wear armour.\n").c_str();
        result += jtrans("You are incapable of any advanced item manipulation.\n").c_str();
        result += _annotate_form_based(jtrans("Your paws have sharp claws.").c_str(),
            !form_keeps_mutations() || you.form == TRAN_BLADE_HANDS);
        have_any = true;
        break;

    case SP_OCTOPODE:
        result += jtrans("You cannot wear most types of armour.\n").c_str();
        result += jtrans("You are amphibious.\n").c_str();
        result += _annotate_form_based(
            jtrans("You can wear up to eight rings at the same time.").c_str(),
            !form_keeps_mutations() && you.form != TRAN_SPIDER);
        result += _annotate_form_based(
            jtrans("You can use your tentacles to constrict many enemies at once.").c_str(),
            !form_keeps_mutations());
        have_any = true;
        break;
...
//Fixme_ja
    ...
    case SIZE_LITTLE:
        if (you.species == SP_FELID)
            break;
        result += jtrans("You are tiny and cannot use many weapons and most armour.\n").c_str();
        have_any = true;
        break;
    case SIZE_SMALL:
        result += jtrans("You are small and have problems with some larger weapons.\n").c_str();
        have_any = true;
        break;
    case SIZE_LARGE:
        result += jtrans("You are too large for most types of armour.\n").c_str();
        have_any = true;
        break;
    default:
        break;
    ...
    if (!have_any)
        result +=  jtrans("You are rather mundane.") +"\n";
            ...
            bool demonspawn, bool no_rot, bool temporary)
{
    ...
    if (!force_mutation)
    {
        ...
        if (!god_gift)
        {
            if ((you.rmut_from_item()
              ...
              {
                  if (failMsg)
                      mpr(jtrans("You feel odd for a moment."), MSGCH_MUTATION);
        ...
        if (you_worship(GOD_ZIN)
          ...
          {
              simple_god_message(jtrans("protects your body from mutation!").c_str());
              return false;
          }
      }
    ...
    if (undead_mutation_rot(beneficial) && !demonspawn)
      {
        ...
        {
            mpr(jtrans("Your body decomposes!"), MSGCH_MUTATION);
    ...
    case MUT_WEAK:   case MUT_CLUMSY: case MUT_DOPEY:
        mprf(MSGCH_MUTATION, jtrans("You feel %s.").c_str(), _stat_mut_desc(mutat, true));
        gain_msg = false;
        break;
    ...
    if (gain_msg)
        mpr(jtrans(mdef.gain[you.mutation[mutat]-1]).c_str(), MSGCH_MUTATION);
    ...
    case MUT_DEMONIC_GUARDIAN:
        ...
        {
            mpr(jtrans("Your demonic guardian will not assist you as long as you worship Okawaru."), MSGCH_MUTATION);
        }
        break;
                                          ...
                                          bool transient = false)
{
    ...
    case MUT_STRONG: case MUT_AGILE:  case MUT_CLEVER:
    case MUT_WEAK:   case MUT_CLUMSY: case MUT_DOPEY:
        mprf(MSGCH_MUTATION, jtrans("You feel %s.").c_str(), _stat_mut_desc(mutat, false));
        lose_msg = false;
        break;
    ...
    if (lose_msg)
        mpr(jtrans(mdef.lose[you.mutation[mutat]]).c_str(), MSGCH_MUTATION);
                     ...
                     bool disallow_mismatch)
{
    ...
    if (!force_mutation)
    {
        if (!god_gift)
        {
            ...
            {
                if (failMsg)
                    mpr(jtrans("You feel rather odd for a moment."), MSGCH_MUTATION);
                return false;
            }
        }
...
string mutation_desc(mutation_type mut, int level, bool colour)
{
    ...
    if (colour)
    {
        ...
        // Build the result
        ostringstream ostr;
        ostr << '<' << colourname << '>' << jtrans(result).c_str()
             << "</" << colourname << '>';
        result = ostr.str();
    }
...
bool perma_mutate(mutation_type which_mut, int how_much, const string &reason)
{
    ...
    dprf("Perma Mutate: %d, %d, %d", cap, you.mutation[which_mut], you.innate_mutations[which_mut]);
            ...
            && you.innate_mutations[which_mut] == cap-1)
        {
            ...
            mpr(jtrans("Your mutations feel more permanent."), MSGCH_MUTATION);
        }
