...
void quiver_item(int slot)
{
    ...
    mprf(jtrans("Quivering %s for %s.").c_str(), you.inv[slot].name(DESC_INVENTORY).c_str(),
         t == AMMO_THROW    ? jtrans("throwing").c_str() :
         t == AMMO_BLOWGUN  ? jtrans("blowguns").c_str() :
         t == AMMO_SLING    ? jtrans("slings").c_str() :
         t == AMMO_BOW      ? jtrans("bows").c_str() :
                              jtrans("crossbows").c_str());
}
...
void choose_item_for_quiver()
{
    ...
    int slot = prompt_invent_item(jtrans("Quiver which item? (- for none, * to show all)").c_str(),
                                  MT_INVLIST,
    ...
    if (slot == PROMPT_GOT_SPECIAL)  // '-' or empty quiver
    {
        ...
        mprf(jtrans("Reset %s quiver to default.").c_str(),
             t == AMMO_THROW    ? jtrans("throwing").c_str() :
             t == AMMO_BLOWGUN  ? jtrans("blowgun").c_str() :
             t == AMMO_SLING    ? jtrans("sling").c_str() :
             t == AMMO_BOW      ? jtrans("bow").c_str() :
                                  jtrans("crossbow").c_str());
        return;
    }
    else if (_wielded_slot_no_quiver(slot))
    {
        ...
        mpr(jtrans("You can't quiver wielded weapons."));
        return;
    }
    else
    {
