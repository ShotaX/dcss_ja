...
void adjust(void)
{
    mpr(jtrans("Adjust (i)tems, (s)pells, or (a)bilities?"), MSGCH_PROMPT);
...
static void _adjust_spell(void)
{
    ...
    mpr(jtrans("Adjust which spell?"), MSGCH_PROMPT);
    ...
    mprf_nocap("%c - %s", keyin, jtrans(spell_title(spell)).c_str());

    // Select target slot.
    keyin = 0;
    while (!isaalpha(keyin))
    {
        mpr(jtrans("Adjust to which letter?"), MSGCH_PROMPT);
        keyin = get_ch();
    ...
    if (spell != SPELL_NO_SPELL)
        mprf_nocap("%c - %s", input_1, jtrans(spell_title(spell)).c_str());
}
...
static void _adjust_ability(void)
{
    ...
    int selected = -1;
    mpr(jtrans("Adjust which ability?"), MSGCH_PROMPT);
    ...
    if (index1 == index2)
    {
        mpr(jtrans("That would be singularly pointless."));
        return;
    }
...
static void _find_description(bool *again, string *error_inout)
{
    *again = true;
    ...
    mpr(jtrans("Describe a Monster"), MSGCH_PROMPT);
    int ch;
    {
    ...
    if (want_regex)
    {
        ...
        mpr(jtrans("Describe what?"), MSGCH_PROMPT);
        char buf[80];
...
static void _add_formatted_keyhelp(column_composer &cols)
{
    ...
    _add_command(cols, 0, CMD_WAIT, jtrans("wait a turn (also <w>.</w>, <w>Del</w>)"), 2);
    _add_command(cols, 0, CMD_REST, jtrans("rest and long wait; stops when"), 2);
    string text2 = jtrans("Health or Magic become full,") + "\n"
				 + jtrans("100turn") + "\n";
    ...
    string text3 = jtrans("<h>Extended Movement:") + "\n";
...
#endif
    _add_command(cols, 0, CMD_READ_MESSAGES, jtrans("read messages (online play only)"), 2);
#endif
    ...
    _add_command(cols, 1, CMD_TOGGLE_FRIENDLY_PICKUP, jtrans("change ally pickup behaviour"));
    ...
    {
        string interact = (you.species == SP_VAMPIRE ? jtrans("Drain corpses")
                                                     : jtrans("Eat food"));
        interact += jtrans("(tries floor first)") + "\n";
    ...
    {
        string interact = (you.species == SP_VAMPIRE ? jtrans("Drain corpses on")
                                                     : jtrans("Eat food from"));
        interact += jtrans("");
        _add_command(cols, 1, CMD_EAT, interact, 2);
    }
