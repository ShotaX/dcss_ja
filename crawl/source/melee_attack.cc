        ...
        && (!crawl_state.game_is_zotdef() || !one_chance_in(20)))
    {
        ...
        if (attacker->is_player())
        {
            if (you.can_see(defender))
            {
                ...
                mprf(jtrans("You hit the %s.").c_str(), feat_name.c_str());
        }
        else
        {
            if (!mons_near(defender->as_monster()))
            {
                simple_monster_message(attacker->as_monster(),
                                       jtrans("hits something.").c_str());
            }
            else if (you.can_see(attacker))
            {
                mprf(jtrans("%s tries to hit %s, but is blocked by the %s.").c_str(),
...
bool melee_attack::handle_phase_dodged()
{
    ...
    if (ev_margin + (ev - ev_nophase) > 0)
    {
        ...
        {
            mprf(jtrans("%s momentarily out as %s attack passes through%s").c_str(),
                 jtrans(defender->name(DESC_THE)).c_str(),
                 jtrans(atk_name(DESC_ITS)).c_str(),
                 jtrans(attack_strength_punctuation()).c_str());
        }
    }
    else
    {
        if (needs_message)
        {
            ...
            {
                mprf(jtrans("%s%s misses %s%s").c_str(),
                     jtrans(atk_name(DESC_THE)).c_str(),
                     evasion_margin_adverb().c_str(),
                     jtrans(defender_name()).c_str(),
...
bool melee_attack::handle_phase_hit()
{
    ...
    else if (needs_message)
    {
        ...
        // TODO: Clean this up if possible, checking atype for do / does is ugly
                mprf(jtrans("%s %s %s but no damage.").c_str(),
             attacker->name(DESC_THE).c_str(),
             defender->name(DESC_THE).c_str(),
             attack_verb.c_str());
    }
...
bool melee_attack::handle_phase_damaged()
{
        ...
        you.duration[DUR_SHROUD_OF_GOLUBRIA] && !one_chance_in(3))
    {
        ...
        else
        {
            if (needs_message)
            {
                mprf(jtrans("Your shroud bends %s attack away%s").c_str(),
                     jtrans(atk_name(DESC_ITS)).c_str(),
                     attack_strength_punctuation().c_str());
            }
            did_hit = false;
    ...
    if (shroud_broken)
        mpr(jtrans("Your shroud falls apart!"), MSGCH_WARN);
...
bool melee_attack::attack()
{
    ...
    if (attacker->is_player() && attacker != defender)
    {
        ...
        if (player_under_penance(GOD_ELYVILON)
            ...
            && one_chance_in(20))
        {
            simple_god_message(jtrans("blocks your attack.").c_str(), GOD_ELYVILON);
            handle_phase_end();
            return false;
        }
...
void melee_attack::player_aux_setup(unarmed_attack_type atk)
{
    ...
    {
    case UNAT_CONSTRICT:
        aux_attack = aux_verb = jtrans("grab");
        aux_damage = 0;
        noise_factor = 10; // extremely quiet?
        break;

    case UNAT_KICK:
        aux_attack = aux_verb = jtrans("kick");
        aux_damage = 5;
    ...
    case UNAT_PECK:
        aux_damage = 6;
        noise_factor = 75;
        aux_attack = aux_verb = jtrans("peck");
        break;

    case UNAT_HEADBUTT:
        aux_damage = 5 + player_mutation_level(MUT_HORNS) * 3;
        aux_attack = aux_verb = jtrans("headbutt");
        break;

    case UNAT_TAILSLAP:
        aux_attack = aux_verb = jtrans("tail-slap");
    ...
    case UNAT_PUNCH:
        ...
        {
            aux_attack = aux_verb = jtrans("tentacle-slap");
            noise_factor = 125;
        }
    ...
    case UNAT_BITE:
        aux_attack = aux_verb = jtrans("bite");
    ...
    case UNAT_PSEUDOPODS:
        aux_attack = aux_verb = jtrans("bludgeon");
    ...
    case UNAT_TENTACLES:
        aux_attack = aux_verb = jtrans("squeeze");
...
bool melee_attack::player_aux_test_hit()
{
    ...
    {
        mprf(jtrans("Your %s passes through %s as momentarily phases out.").c_str(),
            aux_attack.c_str(),
            defender->name(DESC_THE).c_str(),
    ...
    {
        mprf(jtrans("Your %s misses %s.").c_str(), aux_attack.c_str(),
             defender->name(DESC_THE).c_str());
    }
...
bool melee_attack::player_aux_apply(unarmed_attack_type atk)
{
    ...
    if (damage_done > 0 || atk == UNAT_CONSTRICT)
    {
        ...
        {
            mprf(jtrans("%s is splashed with acid.").c_str(),
                 defender->name(DESC_THE).c_str());
    ...
    else // no damage was done
    {
        mprf(jtrans("You %s %s%s.").c_str(),
             defender->name(DESC_THE).c_str(),
             aux_verb.c_str(),
             you.can_see(defender) ? jtrans(", but do no damage").c_str() : "");
    }
...
string melee_attack::player_why_missed()
{
    ...
    if (to_hit < ev && to_hit + combined_penalty >= ev)
    {
        ...
        if (armour_miss && !shield_miss)
            return jtrans("Your") + armour_name + jtrans("prevents you from hitting");
        else if (shield_miss && !armour_miss)
            return (jtrans("Melee_Att-134")); //"Your shield prevents you from hitting "
        else
            return (jtrans("Melee_Att-135") + armour_name
                    + jtrans("Melee_Att-136"));
    }

    return (jtrans("Youev") + evasion_margin_adverb() + jtrans("missev"));
}
...
void melee_attack::set_attack_verb()
{
        ...
        && you.species != SP_FELID)
    {
        if (weap_type != WPN_UNKNOWN)
            ...
            attack_verb = jtrans("clumsily bash").c_str();
        return;
    }
    ...
    case DAM_PIERCE:
        if (damage_done < HIT_MED)
            attack_verb = jtrans("puncture").c_str();
        else if (damage_done < HIT_STRONG)
            attack_verb = jtrans("impale").c_str();
        else
        {
            ...
            {
                attack_verb = jtrans("spit").c_str();
                verb_degree = jtrans("like the proverbial pig").c_str();
            }
            else
            {
    ...
    case DAM_SLICE:
        if (damage_done < HIT_MED)
            attack_verb = jtrans("slash").c_str();
        else if (damage_done < HIT_STRONG)
            attack_verb = jtrans("slice").c_str();
        else if (defender_genus == MONS_OGRE)
        {
            attack_verb = jtrans("dice").c_str();
            verb_degree = jtrans("like an onion").c_str();
        }
        else if (defender_genus == MONS_SKELETON)
        {
            attack_verb = jtrans("fracture").c_str();
            verb_degree = jtrans("into splinters").c_str();
        }
        else if (defender_genus == MONS_HOG)
        {
            attack_verb = jtrans("carve").c_str();
            verb_degree = jtrans("like a proverbial ham").c_str();
        }
        else
        {
            const char* pierce_desc[][2] = {{"like a pillowcase",   "open"},
                                            {"like a ripe choko",   "slice"},
                                            {"into ribbons",  	    "cut"},
                                            {"carve",   "like a ham"},
                                            {"chop",    "into pieces"}};
            const int choice = random2(ARRAYSZ(pierce_desc));
            attack_verb = jtrans(pierce_desc[choice][0]);
            verb_degree = jtrans(pierce_desc[choice][1]);
        }
        break;

    case DAM_BLUDGEON:
        if (damage_done < HIT_MED)
            attack_verb = one_chance_in(4) ? jtrans("thump").c_str() 
											 : jtrans("sock").c_str();
        else if (damage_done < HIT_STRONG)
            attack_verb = jtrans("bludgeon").c_str();
        else if (defender_genus == MONS_SKELETON)
        {
            attack_verb = jtrans("shatter").c_str();
            verb_degree = jtrans("into splinters").c_str();
        }
        else
        {

            const char* pierce_desc[][2] = {{"like a grape",   "crush"},
                                            {"like a drum",    "beat"},
                                            {"like a gong",    "hammer_att"},
                                            {"like an anvil",  "pound"},
                                            {"like a pancake", "flatten"}};
            const int choice = random2(ARRAYSZ(pierce_desc));
            attack_verb = pierce_desc[choice][0];
            verb_degree = pierce_desc[choice][1];
        }
    ...
    case DAM_WHIP:
        if (damage_done < HIT_MED)
            attack_verb = jtrans("whack").c_str();
        else if (damage_done < HIT_STRONG)
            attack_verb = jtrans("thrash").c_str();
        else
        {
            ...
            case MH_NATURAL:
            case MH_DEMONIC:
                attack_verb = jtrans("punish").c_str();
                verb_degree = jtrans("causing immense pain").c_str();
                break;
            default:
                attack_verb = jtrans("devastate").c_str();
            }
        }
        break;
    ...
    case -1: // unarmed
        ...
        case TRAN_PORCUPINE:
            if (damage_done < HIT_WEAK)
                ...
                attack_verb = jtrans("bite").c_str();
            else
                attack_verb = jtrans("maul").c_str();
            break;
        case TRAN_BLADE_HANDS:
            if (damage_done < HIT_WEAK)
                ...
                attack_verb = jtrans("shred").c_str();
            break;
        case TRAN_TREE:
            if (damage_done < HIT_WEAK)
                attack_verb = jtrans("hit").c_str();
            else if (damage_done < HIT_MED)
                attack_verb = jtrans("smack").c_str();
            else if (damage_done < HIT_STRONG)
                attack_verb = jtrans("pummel").c_str();
            else
                attack_verb = jtrans("thrash").c_str();
            break;
        case TRAN_DRAGON:
            if (damage_done < HIT_WEAK)
                ...
                attack_verb = jtrans("claw").c_str();
            else if (damage_done < HIT_STRONG)
                attack_verb = jtrans("bite").c_str();
            else
                attack_verb = coinflip() ? jtrans("maul").c_str()
										 : jtrans("trample").c_str();
            break;
        case TRAN_WISP:
            if (damage_done < HIT_WEAK)
                attack_verb = jtrans("touch").c_str();
            else if (damage_done < HIT_MED)
                attack_verb = jtrans("hit").c_str();
            else
                attack_verb = jtrans("engulf").c_str();
            break;
            break;
        case TRAN_STATUE:
        case TRAN_LICH:
            ...
            {
                if (damage_done < HIT_WEAK)
                    attack_verb = jtrans("scratch").c_str();
                else if (damage_done < HIT_MED)
                    attack_verb = jtrans("claw").c_str();
                else if (damage_done < HIT_STRONG)
                    attack_verb = jtrans("mangle").c_str();
                else
                    attack_verb = jtrans("eviscerate").c_str();
                break;
            }
            else if (you.has_usable_tentacles())
            {
                if (damage_done < HIT_WEAK)
                    attack_verb = jtrans("tentacle-slap").c_str();
                else if (damage_done < HIT_MED)
                    attack_verb = jtrans("bludgeon").c_str();
                else if (damage_done < HIT_STRONG)
                    attack_verb = jtrans("batter").c_str();
                else
                    attack_verb = jtrans("thrash").c_str();
                break;
            }
            ...
            else
            {
                if (damage_done < HIT_WEAK)
                    attack_verb = jtrans("hit").c_str();
                      ...
                      attack_verb = jtrans("punch").c_str();
                else if (damage_done < HIT_STRONG)
                      attack_verb = jtrans("pummel").c_str();
                // XXX: detect this better
                         ...
                         && (get_mon_shape(defender->as_monster()->type)
                             ...
                             || get_mon_shape(defender->as_monster()->type)
                                == MON_SHAPE_ARACHNID))
                {
                    attack_verb = jtrans("squash").c_str();
                    verb_degree = jtrans("like a proverbial bug").c_str();
                }
                else
                {
                    ...
                    {
                        attack_verb = jtrans("beat").c_str();
                        verb_degree = jtrans("into a bloody pulp").c_str();
                    }
                    else
                    {
...
bool melee_attack::player_monattk_hit_effects()
{
        ...
        && mons_class_is_confusable(defender->type))
    {
        ...
        {
            mprf(jtrans("%s is stunned!").c_str(), defender->name(DESC_THE).c_str());
        }
    }
...
void melee_attack::rot_defender(int amount, int immediate)
{
    if (defender->rot(attacker, amount, immediate, true))
    {
        ...
        {
            special_damage_message =
                make_stringf(jtrans("You feel your flesh %s away!").c_str(),
                             immediate > 0 ? jtrans("rotting").c_str() : jtrans("start to rot").c_str());
        }
        else if (defender->is_monster() && defender_visible)
        {
            special_damage_message =
                make_stringf(
                    jtrans("%s %s!").c_str(),
                    jtrans(def_name(DESC_THE)).c_str(),
                    amount > 0 ? jtrans("rots").c_str() : jtrans("looks less resilient").c_str());
        }
    }
}
...
bool melee_attack::distortion_affects_defender()
{
        ...
        && mons_genus(defender->as_monster()->type) == MONS_BLINK_FROG)
    {
        if (one_chance_in(5))
        {
            ...
            {
                special_damage_message =
                    make_stringf(jtrans("%s bask in the distortional energy.").c_str(),
                                 jtrans(def_name(DESC_THE)).c_str());
            }
    ...
    {
        special_damage_message =
            make_stringf(jtrans("Space bends around %s.").c_str(),
            jtrans(def_name(DESC_THE)).c_str());
    ...
    {
        special_damage_message =
            make_stringf(jtrans("Space warps horribly around %s!").c_str(),
                         jtrans(def_name(DESC_THE)).c_str());
...
void melee_attack::antimagic_affects_defender()
{
    if (defender->is_player())
    {
        ...
        if (!mp_loss)
            return;
        mpr(jtrans("You feel your power leaking away."), MSGCH_WARN);
        drain_mp(mp_loss);
        obvious_effect = true;
    }
             ...
             && !mons_class_flag(defender->type, M_FAKE_SPELLS))
    {
        ...
        special_damage_message =
                    ...
                    + jtrans("magic leaks into the air.").c_str();
        obvious_effect = true;
    }
}
...
void melee_attack::pain_affects_defender()
{
    ...
    if (!one_chance_in(attacker->skill_rdiv(SK_NECROMANCY) + 1))
    {
        ...
        {
            special_damage_message =
                make_stringf(jtrans("%s in agony.").c_str(),
                             defender->name(DESC_THE).c_str());
        }
...
void melee_attack::chaos_affects_defender()
{
        ...
        if (!skip_chaos_message && you.see_cell(defender->pos()))
        {
            ...
            else if (you.can_see(defender))
            {
                simple_monster_message(defender->as_monster(),
                                       jtrans("gives off a flash of multicoloured light!").c_str());
            }
            else
    ...
    case CHAOS_CLONE:
    {
                                        ...
                                        &obvious_effect))
        {
            ...
            {
                special_damage_message =
                    make_stringf(jtrans("%s is duplicated!").c_str(),
                                 jtrans(def_name(DESC_THE)).c_str());
            }
    ...
    if (beam.flavour != BEAM_NONE)
    {
        ...
        if (weapon && you.can_see(attacker))
        {
            ...
            beam.name = jtrans(atk_name(DESC_THE));
...
void melee_attack::chaos_affects_attacker()
{
    ...
    {
        mprf(jtrans("Smoke pours forth from %s!").c_str(), wep_name(DESC_YOUR).c_str());
...
void melee_attack::do_miscast()
{
    ...
    string cause = jtrans(atk_name(DESC_THE));
    int    source;
...
bool melee_attack::apply_damage_brand()
{
    ...
    case SPWPN_FLAMING:
        calc_elemental_brand_damage(BEAM_FIRE, defender->res_fire(),
                                    defender->is_icy() ? jtrans("melt_mon").c_str() : jtrans("burn_mon").c_str());
        defender->expose_to_element(BEAM_FIRE);
        noise_factor += 400 / max(1, damage_done);
        break;

    case SPWPN_FREEZING:
        calc_elemental_brand_damage(BEAM_COLD, defender->res_cold(), jtrans("freeze_mon").c_str());
    ...
    case SPWPN_ELECTROCUTION:
        ...
        else if (one_chance_in(3))
        {
            special_damage_message =
                defender->is_player()?
                   jtrans("You are electrocuted!").c_str()
                :  jtrans("There is a sudden explosion of sparks!").c_str();
    ...
    case SPWPN_VAMPIRICISM:
    {
        ...
        else if (attacker_visible)
        {
            if (defender->is_player())
            {
                mprf(jtrans("%s draws strength from your injuries!").c_str(),
                     attacker->name(DESC_THE).c_str());
            }
            else
            {
                mprf(jtrans("%s is healed.").c_str(),
                     attacker->name(DESC_THE).c_str());
            }
        }
                                   ...
                                   brand_type wpn_brand)
{
    ...
    if (dam_type == DVORP_CLAWING)
    {
        ...
        verb = jtrans(RANDOM_ELEMENT(claw_verbs)).c_str();
    }
    else
    {
        ...
        verb = jtrans(RANDOM_ELEMENT(slice_verbs)).c_str();
    }
    ...
    if (defender->as_monster()->number == 1) // will be zero afterwards
    {
        if (defender_visible)
        {
            mprf(jtrans("%s %s %s's last head off!").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(def_name(DESC_THE)).c_str(),
    ...
    else
    {
        if (defender_visible)
        {
            mprf(jtrans("%s %s one of %s's heads off!").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(def_name(DESC_THE)).c_str(),
                 attacker->conj_verb(verb).c_str());
        }
...
void melee_attack::attacker_sustain_passive_damage()
{
        ...
        || defender->is_player() && you.form == TRAN_JELLY)
    {
        ...
        if (rA < 3)
        {
            ...
            {
                mprf(jtrans("Your %s burn!").c_str(), you.hand_name(true).c_str());
            ...
            {
                simple_monster_message(attacker->as_monster(),
                                       jtrans("is burned by acid!").c_str());
...
void melee_attack::apply_staff_damage()
{
    ...
    case STAFF_AIR:
        ...
        {
            special_damage_message =
                make_stringf(jtrans("%s electrocuted!").c_str(),
                             defender->name(DESC_THE).c_str());
            special_damage_flavour = BEAM_ELECTRICITY;
        }
    ...
    case STAFF_DEATH:
        ...
        if (special_damage)
        {
            special_damage_message =
                make_stringf(
                    jtrans("%s convulse in agony!").c_str(),
                    defender->name(DESC_THE).c_str());
...
bool melee_attack::attack_warded_off()
{
        ...
        && attacker->as_monster()->check_res_magic(WARDING_CHECK) <= 0)
    {
        if (needs_message)
        {
            mprf(jtrans("%s tries to attack %s, but flinches away.").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(defender_name()).c_str());
        }
...
bool melee_attack::attack_shield_blocked(bool verbose)
{
    ...
    if (pro_block >= con_block)
    {
            ...
            && attacker->as_monster()->type == MONS_PHANTASMAL_WARRIOR)
        {
            if (needs_message && verbose)
            {
                mprf("%s blade passes through %s shield.",
                    jtrans(atk_name(DESC_ITS)).c_str(),
                    jtrans(def_name(DESC_ITS)).c_str());
                return false;
            }
        }
        ...
        if (needs_message && verbose)
        {
            mprf(jtrans("%s %s block attack.").c_str(),
                 jtrans(def_name(DESC_THE)).c_str(),
                 jtrans(atk_name(DESC_ITS)).c_str());
        }
...
string melee_attack::mons_attack_verb()
{
    ...
    return jtrans(attack_types[attk_type]);
}
...
void melee_attack::announce_hit()
{
    ...
    else
    {
        ...
        mprf(jtrans("You %s %s%s%s%s").c_str(),
             jtrans(defender->name(DESC_THE)).c_str(),
             attack_verb.c_str(),
...
void melee_attack::mons_do_poison()
{
            ...
            && one_chance_in(attk_flavour == AF_POISON ? 4 : 3)))
    {
        ...
        if (needs_message)
        {
            mprf(jtrans("%s poisons %s!").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(defender_name()).c_str());
            if (force)
            {
                mprf(jtrans("%s partially resist.").c_str(),
                    jtrans(defender_name()).c_str());
            }
        }
    }
...
void melee_attack::mons_do_napalm()
{
    ...
    if (one_chance_in(20) || (damage_done > 2 && one_chance_in(3)))
    {
        if (needs_message)
        {
            mprf(jtrans("%s are covered in liquid flames%s").c_str(),
                 jtrans(def_name(DESC_THE)).c_str(),
                 special_attack_punctuation().c_str());
        }

        if (defender->is_player())
            napalm_player(random2avg(7, 3) + 1, jtrans(atk_name(DESC_A)));
        else
        {
            napalm_monster(
                defender->as_monster(),
...
void melee_attack::splash_defender_with_acid(int strength)
{
    ...
    else
    {
        ...
        if (defender_visible)
            mprf(jtrans("%s is splashed with acid.").c_str(), defender->name(DESC_THE).c_str());
...
void melee_attack::mons_apply_attack_flavour()
{
    ...
    case AF_DISEASE:
      ...
      case AF_COLD:
        ...
        if (needs_message && base_damage)
          {
            mprf(jtrans("%s %s freeze%s").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(defender_name()).c_str(),
                 special_attack_punctuation().c_str());
    ...
    case AF_VAMPIRIC:
        ...
        if (defender->stat_hp() < defender->stat_maxhp())
        {
            if (attacker->heal(1 + random2(damage_done), coinflip())
                && needs_message)
            {
                mprf(jtrans("%s %s strength from draw injuries!").c_str(),
                     jtrans(atk_name(DESC_THE)).c_str(),
                     jtrans(def_name(DESC_ITS)).c_str());
            }
        }
    ...
    case AF_CONFUSE:
        if (attk_type == AT_SPORE)
        {
            ...
            if (defender_visible)
            {
                mprf(jtrans("%s are engulfed in a cloud of spores!").c_str(),
                     defender->name(DESC_THE).c_str());
            }
        }
    ...
    case AF_RAGE:
        ...
        if (needs_message)
        {
            mprf(jtrans("%s %s infuriate!").c_str(),
                 jtrans(atk_name(DESC_THE)).c_str(),
                 jtrans(defender_name()).c_str());
        }
    ...
    case AF_STEAL_FOOD:
    {
        ...
        if (needs_message)
            mprf(jtrans("%s lunges at you hungrily!").c_str(), jtrans(atk_name(DESC_THE)).c_str());
        ...
        if (needs_message && ground)
            mpr(jtrans("Some of the food beneath you is devoured!"));
        break;
    }
...
void melee_attack::do_passive_freeze()
{
        ...
        && adjacent(you.pos(), attacker->as_monster()->pos()))
    {
        ...
        simple_monster_message(mon, jtrans("is very cold.").c_str());
...
void melee_attack::mons_do_eyeball_confusion()
{
        ...
        && x_chance_in_y(player_mutation_level(MUT_EYEBALLS), 20))
    {
        ...
        {
            mprf(jtrans("The eyeballs on your body gaze at %s.").c_str(),
    ...
    if (defender->is_player())
    {
        ...
        if (mut && attacker->alive() && one_chance_in(evp / 3 + 1)
            && x_chance_in_y(2, 13 - (mut * 2)))
          {
            ...
            {
                simple_monster_message(attacker->as_monster(),
                                       jtrans("is struck by your spines.").c_str());
            }
        ...
        if (attacker->alive() && (x_chance_in_y(2, 5)
            || random2(div_rand_round(attacker->armour_class(), 2)) < degree))
          {
            ...
            int hurt = attacker->apply_ac(dmg, AC_HALF);
              ...
              {
                mprf(jtrans("%s struck by %s %s.").c_str(), attacker->name(DESC_THE).c_str(),
                     defender->name(DESC_ITS).c_str(),
                     defender->as_monster()->type == MONS_BRIAR_PATCH ? jtrans("thorns").c_str()
                                                                      : jtrans("spines").c_str());
            }
            if (attacker->is_player())
...
void melee_attack::do_minotaur_retaliation()
{
    ...
    if (!defender->is_player())
    {
        ...
        if (you.see_cell(defender->pos()))
        {
            ...
            mprf(jtrans("%s furiously retaliates!").c_str(), defname.c_str());
            if (hurt <= 0)
            {
                mprf(jtrans("%s headbutts %s, but does no damage.").c_str(), defname.c_str(),
                     attacker->name(DESC_THE).c_str());
            }
            else
            {
                mprf(jtrans("%s headbutts %s%s").c_str(), defname.c_str(),
                     attacker->name(DESC_THE).c_str(),
    ...
    if (5 * you.strength() + 7 * you.dex() > random2(600))
    {
        ...
        if (hurt <= 0)
        {
            mprf(jtrans("You headbutt %s, but do no damage.").c_str(),
                 attacker->name(DESC_THE).c_str());
            return;
        }
        else
        {
            mprf(jtrans("You headbutt %s").c_str(),
                 attacker->name(DESC_THE).c_str());
            attacker->hurt(&you, hurt);
        }
    }
...
bool melee_attack::do_knockback(bool trample)
{
    do
    {
        ...
        if (needs_message)
        {
            mprf(jtrans("%s stumble backwards!").c_str(),
                 jtrans(def_name(DESC_THE)).c_str());
        }
    ...
    if (needs_message)
    {
        mprf(jtrans("%s hold ground!").c_str(),
             jtrans(def_name(DESC_THE)).c_str());
    }
                                               ...
                                               bool needs_bite_msg, int reduction)
{
    ...
    {
        mprf(jtrans("You bite %s, and draw %s blood!").c_str(),
             mon->name(DESC_THE, true).c_str(),
             mon->pronoun(PRONOUN_POSSESSIVE).c_str());
    }
    else
    {
        mprf(jtrans("You draw %s blood!").c_str(),
    ...
    if (you.hp < you.hp_max)
    {
        ...
        {
            inc_hp(heal);
            mprf(jtrans("You feel %sbetter.").c_str(), (you.hp == you.hp_max) ? jtrans("much").c_str() : "");
        }
    }
...
void melee_attack::stab_message()
{
    ...
    case 6:     // big melee, monster surrounded/not paying attention
        if (coinflip())
        {
            mprf(jtrans("You %s %s from a blind spot!").c_str(),
                  defender->name(DESC_THE).c_str(),
                  (you.species == SP_FELID) ? jtrans("pounce on").c_str() : jtrans("strike").c_str());
        }
        else
        {
            mprf(jtrans("You catch %s momentarily off-guard.").c_str(),
                  defender->name(DESC_THE).c_str());
        }
        break;
    case 4:     // confused/fleeing
        if (!one_chance_in(3))
        {
            mprf(jtrans("You catch %s completely off-guard!").c_str(),
                  defender->name(DESC_THE).c_str());
        }
        else
        {
            mprf(jtrans("You %s %s from behind!").c_str(),
                  (you.species == SP_FELID) ? jtrans("pounce on").c_str() : jtrans("strike").c_str(),
                  defender->name(DESC_THE).c_str());
        }
        break;
    ...
    case 1:
        ...
        {
            mprf(jtrans("You pounce on the unaware %s!").c_str(),
                 defender->name(DESC_PLAIN).c_str());
            break;
        }
        mprf(jtrans("%s fails to defend %s.").c_str(),
              defender->name(DESC_THE).c_str(),
