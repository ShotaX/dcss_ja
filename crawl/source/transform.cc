                              ...
                              bool meld = true, bool mutation = false)
{
    ...
    for (iter = removed.begin(); iter != removed.end(); ++iter)
    {
        ...
        mprf(jtrans("%s %s %s").c_str(), equip->name(DESC_YOUR).c_str(),
             unequip ? "" : jtrans("into your body.").c_str(),
             unequip ? jtrans("fallaway!").c_str() : jtrans("meld").c_str());
...
static void _unmeld_equipment_type(equipment_type e)
{
    ...
    if (force_remove)
    {
        mprf(jtrans("%s is pushed off your body!").c_str(), item.name(DESC_YOUR).c_str());
        unequip_item(e);
    }
    else
    {
        // if (item.base_type != OBJ_JEWELLERY)
        mprf(jtrans("%s unmelds from your body.").c_str(), item.name(DESC_YOUR).c_str());
        unmeld_slot(e);
    }
}
                                    ...
                                    dungeon_feature_type feat, bool quiet)
{
    ...
    if (!quiet)
    {
        mprf(jtrans("You would %s in your new form.").c_str(),
             feat == DNGN_DEEP_WATER ? jtrans("drown").c_str()
									 : jtrans("burn").c_str());
    }
    return false;
}
               ...
               bool just_check)
{
    ...
    {
        simple_god_message(jtrans("protects your body from unnatural transformation!").c_str());
        return false;
    }
    ...
    if (you.transform_uncancellable)
    {
        ...
        // other uses.
        if (!just_check)
            mpr(jtrans("You are stuck in your current form!"));
        return false;
    }
    ...
    if (previous_trans == which_trans)
    {
        ...
        if (you.duration[DUR_TRANSFORMATION] < dur * BASELINE_DELAY)
        {
            ...
            if (which_trans == TRAN_PIG)
                mpr(jtrans("You feel you'll be a pig longer."));
            else
    ...
    if (which_trans == TRAN_LICH && you.duration[DUR_DEATHS_DOOR])
    {
        if (!involuntary)
        {
            mpr(jtrans("The transformation conflicts with an enchantment already in effect."));
        }
        return _abort_or_fizzle(just_check);
    }
    ...
    case TRAN_APPENDAGE:
    {
        ...
        {
            mpr(jtrans("You have no appropriate body parts free."));
            return false;
        }
    ...
    case TRAN_STATUE:
        if (you.duration[DUR_STONESKIN])
            mpr(jtrans("Your new body merges with your stone armour."));
        else if (you.species == SP_LAVA_ORC)
            mpr(jtrans("Your new body is particularly stony."));
        if (you.duration[DUR_ICY_ARMOUR])
        {
            mpr(jtrans("Your new body cracks your icy armour."), MSGCH_DURATION);
    ...
    case TRAN_ICE_BEAST:
        if (you.duration[DUR_ICY_ARMOUR])
            mpr(jtrans("Your new body merges with your icy armour."));
        break;
    ...
    case TRAN_SPIDER:
        if (you.attribute[ATTR_HELD])
        {
            ...
            if (trap && trap->type == TRAP_WEB)
                mpr(jtrans("You wish you had such spider senses a moment ago."));
        }
        break;
    ...
    case TRAN_TREE:
        if (you_worship(GOD_FEDHAS) && !player_under_penance())
            simple_god_message(jtrans("makes you hardy against extreme temperatures.").c_str());
        // ignore hunger_state (but don't reset hunger)
        you.hunger_state = HS_SATIATED;
        set_redraw_status(REDRAW_HUNGER);
    ...
    case TRAN_LICH:
        ...
        {
            mpr(jtrans("You stop regenerating."), MSGCH_DURATION);
...
void untransform(bool skip_wielding, bool skip_move)
{
    ...
    {
    case TRAN_SPIDER:
        mpr(jtrans("Your transformation has ended."), MSGCH_DURATION);
    ...
    case TRAN_BLADE_HANDS:
        mprf(MSGCH_DURATION, jtrans("Your %s revert to their normal proportions.").c_str(),
             blade_parts().c_str());
        you.wield_change = true;
        break;
    ...
    case TRAN_STATUE:
        ...
        {
            mpr(jtrans("You revert to a slightly less stony form."), MSGCH_DURATION);
        }
        else if (you.species != SP_LAVA_ORC)
            mpr(jtrans("You revert to your normal fleshy form."), MSGCH_DURATION);
    ...
    case TRAN_ICE_BEAST:
        if (you.species == SP_LAVA_ORC && !temperature_effect(LORC_STONESKIN))
            mpr(jtrans("Your icy form melts away into molten rock."), MSGCH_DURATION);
        else
            mpr(jtrans("You warm up again."), MSGCH_DURATION);
    ...
    case TRAN_APPENDAGE:
        {
            ...
            mprf(MSGCH_DURATION, jtrans("Your %s disappear.").c_str(), appendage_name(app));
        }
        break;

    case TRAN_FUNGUS:
        mpr(jtrans("You stop sporulating."), MSGCH_DURATION);
        you.set_duration(DUR_CONFUSING_TOUCH, 0);
        break;
    case TRAN_TREE:
        mpr(jtrans("You feel less woody."), MSGCH_DURATION);
            ...
            && you.experience_level >= 5)
        {
            ...
            mpr(jtrans("You frantically escape the water."));
        }
        notify_stat_change(STAT_STR, -10, true,
    ...
    {
        you.attribute[ATTR_PERM_FLIGHT] = 1;
        mpr(jtrans("You frantically enable flight."));
    }
        ...
        && !player_effectively_in_light_armour())
    {
        ...
        mprf(MSGCH_DURATION, jtrans("%s cracks your icy armour.").c_str(),
