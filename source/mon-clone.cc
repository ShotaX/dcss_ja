    ...
    if (monster *clone = clone_mons(foe, true, &cloning_visible))
    {
        ...
        if (cloning_visible)
        {
            if (!you.can_see(caster))
                mprf(jtrans("%s seems to step out of %s!").c_str(),
                     foe->name(DESC_THE).c_str(),
                     foe->pronoun(PRONOUN_REFLEXIVE).c_str());
            else
                mprf(jtrans("%s seems to draw %s out of %s!").c_str(),
                               ...
                               spell_type spell_cast)
{
    if (foe->is_player())
    {
        ...
        {
            mpr(jtrans("There is a horrible, sudden wrenching feeling in your soul!"),
                MSGCH_WARN);
                    ...
                    coord_def pos)
{
    ...
    *obvious = false;
    ...
    {
        if (!quiet)
            simple_monster_message(orig, jtrans("is duplicated!").c_str());
        *obvious = true;
    }
