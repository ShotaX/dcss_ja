...
void fire_target_behaviour::set_prompt()
{
    ...
    else
    {
        ...
        switch (projected)
        {
        case LRET_FUMBLED:  msg << jtrans("Awkwardly throwing"); break;
    ...
    // And a key hint.
    msg << (no_other_items ? jtrans("(i - inventory)")
                           : jtrans("(i - inventory. (,) - cycle)"))
        << ": ";
...
static int _fire_prompt_for_item()
{
    ...
    int slot = prompt_invent_item(jtrans("Fire/throw which item? (* to show all)").c_str(),
                                   MT_INVLIST,
...
bool fire_warn_if_impossible(bool silent)
{
    ...
    if (you.attribute[ATTR_HELD])
    {
        ...
        {
            if (!silent)
                mprf(jtrans("You cannot throw anything while %s.").c_str(), held_status());
            return true;
        }
        else if (weapon->sub_type != WPN_BLOWGUN)
        {
            if (!silent)
            {
                mprf(jtrans("You cannot shoot with your %s while %s.").c_str(),
...
static bool _dispersal_hit_victim(bolt& beam, actor* victim, int dmg)
{
    ...
    else
    {
        ...
        if (was_seen && seen)
            mprf(jtrans("%s blinks!").c_str(), name.c_str());
        else if (was_seen && !seen)
            mprf(jtrans("%s vanishes!").c_str(), name.c_str());
    }
                           ...
                           bool message = true)
{
    if (victim->holiness() == MH_UNDEAD || victim->holiness() == MH_NONLIVING)
    {
        if (victim->is_monster())
            simple_monster_message(victim->as_monster(), jtrans("is unaffected.").c_str());
        else
    ...
    {
        if (victim->is_monster())
            simple_monster_message(victim->as_monster(), jtrans("resists.").c_str());
        else
              ...
              dist *target)
{
    ...
    // Create message.
    mprf(jtrans("%s %s%s %s2.").c_str(),
          teleport  ? jtrans("Magically, you").c_str() : jtrans("You").c_str(),
          projected ? "" : jtrans("awkwardly").c_str(),
          ammo_name.c_str(),
          projected == LRET_LAUNCHED ? jtrans("shoot").c_str() : jtrans("throw").c_str());
