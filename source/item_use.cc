...
#define SAY(x) {if (say_reason) { x; }}
    ...
    if (!ignore_temporary_disability
        ...
        && you.weapon()->cursed())
    {
        SAY(mprf(jtrans("You can't unwield your weapon%s!").c_str(),
                 !unwield ? jtrans("to draw a new one").c_str() : ""));
        return false;
    }
    ...
    {
        SAY(mpr(jtrans("You can't use weapons.")));
        return false;
    }
    ...
    {
        SAY(mpr(jtrans("That's too large and heavy for you to wield.")));
        return false;
    }
    ...
    {
        SAY(mpr(jtrans("That's too large for you to wield.")));
        return false;
    }
        ...
        && (item_type_known(*weapon) || !only_known))
    {
        if (say_reason)
        {
            mpr(jtrans("As you grasp it, you feel a great hunger. Being not satiated, you stop."));
            // If it's a standard weapon, you know its ego now.
            if (!is_artefact(*weapon) && !is_blessed(*weapon)
                && !item_type_known(*weapon))
            {
        ...
        && (!auto_wield || !you.inv[item_slot].defined() || !good_swap))
    {
        if (!auto_wield)
        {
            item_slot = prompt_invent_item(
                            jtrans("Wield which item (- for none, * to show all)?").c_str(),
...
static void _warn_launcher_shield_slowdown(const item_def &launcher)
{
    ...
    if (slowspeed > normspeed)
    {
        ...
        {
            mprf(MSGCH_WARN,
                    jtrans("Your %s %sslows your rate of fire.").c_str(),
...
void wear_armour(int slot) // slot is for tiles
{
    ...
    if (slot != -1)
        armour_wear_2 = slot;
    else if (!armour_prompt(jtrans("Wear which item?").c_str(), &armour_wear_2, OPER_WEAR))
        return;
...
bool can_wear_armour(const item_def &item, bool verbose, bool ignore_temporary)
{
    ...
    if (player_genus(GENPC_DRACONIAN) && slot == EQ_BODY_ARMOUR)
    {
        if (verbose)
        {
            mprf(jtrans("Your wings%s won't fit in that.").c_str(), you.mutation[MUT_BIG_WINGS]
                 ? "" : jtrans(", even vestigial as they are,").c_str());
        }
        return false;
    }
    ...
    {
        if (verbose)
            mprf(jtrans("This armour is too %s for you!").c_str(),
                 (bad_size > 0) ? jtrans("toobig").c_str() : jtrans("toosmall").c_str());
    ...
    if (sub_type == ARM_GLOVES)
    {
        ...
        {
            if (verbose)
                mpr(jtrans("You can't wear gloves with your huge claws!"));
            return false;
        }
    }
...
bool do_wear_armour(int item, bool quiet)
{
        ...
        && is_shield_incompatible(*you.weapon(), &invitem))
    {
        if (!quiet)
        {
            if (you.species == SP_OCTOPODE)
                ...
                mprf(jtrans("You'd need three %s to do that!").c_str(), you.hand_name(true).c_str());
        }
        return false;
    }
        ...
        && you.equip[EQ_CLOAK] != -1 && !cloak_is_being_removed())
    {
            ...
            && you.inv[you.equip[EQ_BODY_ARMOUR]].cursed())
        {
            if (!quiet)
            {
                mprf(jtrans("%s is stuck to your body!").c_str(),
        ...
        {
            if (!quiet)
                mpr(jtrans("Your cloak prevents you from wearing the armour."));
            return false;
        }
    }
...
bool takeoff_armour(int item)
{
    ...
    {
        mprf(jtrans("%s is melded into your body!").c_str(),
    ...
    if (invitem.cursed())
    {
        mprf(jtrans("%s is stuck to your body!").c_str(), invitem.name(DESC_YOUR).c_str());
        return false;
    }
    ...
    if (slot == EQ_BODY_ARMOUR)
    {
        if (you.equip[EQ_CLOAK] != -1 && !cloak_is_being_removed())
        {
            ...
            {
                mpr(jtrans("Your cloak prevents you from removing the armour."));
                return false;
            }
        }
    }
...
static int _prompt_ring_to_remove(int new_ring)
{
    ...
    mesclr();
    mprf(jtrans("Wearing %s.").c_str(), you.inv[new_ring].name(DESC_A).c_str());

    const char lslot = index_to_letter(left->link);
    const char rslot = index_to_letter(right->link);

#ifdef TOUCH_UI
    string prompt = jtrans("You're wearing two rings. Remove which one?").c_str();
...
#else
    mprf(MSGCH_PROMPT,
         jtrans("You're wearing two rings. Remove which one? (%c/%c/<</>/Esc)").c_str(),
         lslot, rslot);
...
//    mprf(jtrans("Wearing %s.").c_str(), you.inv[new_ring].name(DESC_A).c_str());
    ...
    mprf(MSGCH_PROMPT,"%s",
         jtrans("You're wearing all the rings you can. Remove which one?").c_str());
...
//         one_slot, two_slot, three_slot, four_slot, five_slot, six_slot, seven_slot, eight_slot);
    mprf(MSGCH_PROMPT, "%s",jtrans("(<w>?</w> for menu, <w>Esc</w> to cancel)").c_str());
...
static bool _swap_rings(int ring_slot)
{
    ...
    {
        mprf("%s",jtrans("You're already wearing two cursed rings!").c_str());
        return false;
    }
...
static bool _swap_rings_octopode(int ring_slot)
{
    ...
    {
        mprf(jtrans("You're already wearing %s cursed rings!%s").c_str(),
             number_in_words(cursed).c_str(),
             (cursed == 8 ? jtrans("Isn't that enough for you?").c_str() : ""));
        return false;
    }
...
static bool _puton_item(int item_slot)
{
    ...
    if (!is_amulet)     // i.e. it's a ring
    {
        ...
        {
            mpr(jtrans("You can't take your gloves off to put on a ring!"));
            return false;
        }
...
bool puton_ring(int slot)
{
    ...
    {
        item_slot = prompt_invent_item(jtrans("Put on which piece of jewellery?").c_str(),
...
bool remove_ring(int slot, bool announce)
{
    ...
    {
        mpr(jtrans("You can't take your gloves off to remove any rings!"));
        return false;
    }
    ...
    {
        const int equipn =
            (slot == -1)? prompt_invent_item(jtrans("Remove which piece of jewellery?").c_str(),
                                             MT_INVLIST,
    ...
    if (you.inv[you.equip[hand_used]].cursed())
    {
        if (announce)
        {
            mprf(jtrans("%s is stuck to you!").c_str(),
    ...
    mprf(jtrans("You remove %s").c_str(), you.inv[ring_wear_2].name(DESC_YOUR).c_str());
#ifdef USE_TILE_LOCAL
    const unsigned int old_talents = your_talents(false).size();
#endif
...
static bool _dont_use_invis()
{
    ...
    {
        mpr(jtrans("You can't turn invisible."));
        return true;
    }
...
void zap_wand(int slot)
{
    ...
    {
        item_slot = prompt_invent_item(jtrans("Zap which item?").c_str(),
                                       MT_INVLIST,
                                       OBJ_WANDS,
                                       true, true, true, 0, -1, NULL,
                                       OPER_ZAP);
    ...
    if (!alreadyknown && (beam.obvious_effect || type_zapped == ZAP_FIREBALL))
    {
        ...
        if (wand.sub_type == WAND_RANDOM_EFFECTS)
            mpr(jtrans("You feel that this wand is rather unreliable."));
            ...
            || you.skill(SK_EVOCATIONS, 10) > 50 + random2(141)))
    {
        ...
        {
            mpr(jtrans("Your skill with magical items lets you calculate the power of this device..."));
        }

        mprf(jtrans("This wand has %d charge left.").c_str(),
             wand.plus);
...
void prompt_inscribe_item()
{
    ...
    int item_slot = prompt_invent_item(jtrans("Inscribe which item?").c_str(),
                                       MT_INVLIST, OSEL_ANY);
...
static void _vampire_corpse_help()
{
    ...
    if (check_blood_corpses_on_ground() || count_corpses_in_pack(true) > 0)
        mpr(jtrans("If it's blood you're after, try <w>e</w>."));
}
...
void drink(int slot)
{
    if (you_foodless(true))
    {
        if (you.form == TRAN_TREE)
            mpr(jtrans("It'd take too long for a potion to reach your roots."));
        else
                      ...
                      40, true, alreadyknown))
    {
        ...
        mprf("It was %s.",potion.name(DESC_QUALNAME).c_str());
    }
...
static bool _drink_fountain()
{
    ...
    if (feat == DNGN_FOUNTAIN_BLUE)
    {
        ...
        mpr(jtrans("You drink the pure, clear water."));
    }
    else if (feat == DNGN_FOUNTAIN_BLOOD)
    {
        ...
        mpr(jtrans("You drink the blood."));
    ...
    else
    {
        ...
        mpr(jtrans("You drink the sparkling water."));
    ...
    if (gone_dry)
    {
        mpr(jtrans("The fountain dries up!"));
...
static void _brand_weapon(bool alreadyknown, item_def &wpn)
{
    ...
    {
        alert_nearby_monsters();
        mprf(jtrans("%s emits a brilliant flash of light!").c_str(),
    ...
    case SPWPN_EVASION:
        ...
        {
            alert_nearby_monsters();
            mprf(jtrans("%s emits a brilliant flash of light!").c_str(),itname.c_str());
        }
        else // should be VORPAL only
        {
            if (get_vorpal_type(wpn) != DVORP_CRUSHING)
                mprf(jtrans("%s's sharpness seems more permanent.").c_str(), itname.c_str());
            else
                mprf(jtrans("%s's heaviness feels very stable.").c_str(), itname.c_str());
        }
          break;
  
      case SPWPN_FLAME:
      case SPWPN_FLAMING:
        mprf(jtrans("%s is engulfed in an explosion of fire!").c_str(), itname.c_str());
        immolation(10, IMMOLATION_AFFIX, alreadyknown);
          break;
  
      case SPWPN_FROST:
      case SPWPN_FREEZING:
        mprf(jtrans("%s is covered with a thin layer of ice!").c_str(), itname.c_str());
    ...
    case SPWPN_VAMPIRICISM:
          mprf(jtrans("%s thirsts for the lives of mortals!").c_str(), itname.c_str());
        drain_exp(true, 100);
          break;
  
      case SPWPN_VENOM:
        if (rebranded)
            mprf(jtrans("%s drips with poison.").c_str(), itname.c_str());
          else
              mprf(jtrans("%s seems more permanently poisoned.").c_str(), itname.c_str());
        toxic_radiance_effect(&you, 1);
          break;
  
      case SPWPN_ELECTROCUTION:
          mprf(jtrans("%s releases a massive orb of lightning.").c_str(), itname.c_str());
        _explosion(you.pos(), &you, BEAM_ELECTRICITY, LIGHTCYAN, "electricity",
                     "electrocution affixation");
          break;

    case SPWPN_CHAOS:
        mprf(jtrans("%s erupts in a glittering mayhem of all colours.").c_str(), itname.c_str());
    ...
    case SPWPN_PAIN:
        ...
        mprf(jtrans("%s shrieks out in agony!").c_str(), itname.c_str());
    ...
    case SPWPN_DISTORTION:
        ...
        mprf(jtrans("%s twongs alarmingly.").c_str(), itname.c_str());
    ...
    case SPWPN_ANTIMAGIC:
        mprf(jtrans("%s repels your magic.").c_str(), itname.c_str());
        drain_mp(you.species == SP_DJINNI ? 100 : you.magic_points);
          success = false;
          break;
  
      case SPWPN_HOLY_WRATH:
          mprf(jtrans("%s emits a blast of cleansing flame.").c_str(), itname.c_str());
...
bool enchant_weapon(item_def &wpn, int acc, int dam, const char *colour)
{
    ...
    if (is_weapon(wpn))
    {
        if (!is_artefact(wpn) && wpn.base_type == OBJ_WEAPONS)
        {
            ...
            if (success && colour)
                mprf(jtrans("%s glow %s for a moment.").c_str(), iname.c_str(), colour);
        }
        if (wpn.cursed())
        {
            if (!success && colour)
            {
                if (const char *space = strchr(colour, ' '))
                    colour = space + 1;
                mprf(jtrans("%s glow silvery %s for a moment.").c_str(), iname.c_str(), s, colour);
            }
    ...
    if (!success && colour)
    {
        ...
        mprf(jtrans("%s very briefly gain a %s sheen.").c_str(), iname.c_str(), colour);
    }
...
bool enchant_armour(int &ac_change, bool quiet, item_def &arm)
{
    ...
    if (armour_is_hide(arm, false))
    {
        if (!quiet)
        {
            mprf(jtrans("%s glows purple and changes!").c_str(),
    ...
    if (!is_enchantable_armour(arm, false))
    {
        if (is_cursed)
        {
            if (!quiet)
            {
                mprf(jtrans("%s glows silver for a moment.").c_str(),
                     arm.name(DESC_YOUR).c_str());
            }
    ...
    if (!quiet)
    {
        mprf(jtrans("%s glows green for a moment.").c_str(),
             arm.name(DESC_YOUR).c_str());
    }
...
static int _handle_enchant_armour(int item_slot, string *pre_msg)
{
    do
    {
        if (item_slot == -1)
        {
            item_slot = prompt_invent_item(jtrans("Enchant which item?").c_str(), MT_INVLIST,
        ...
        {
            mpr(jtrans("Choose some type of armour to enchant, or Esc to abort."));
            if (Options.auto_list)
                more();
...
static void _handle_read_book(int item_slot)
{
    ...
    {
        if (silenced(you.pos()))
            mpr(jtrans("This book does not work if you cannot read it aloud!"));
        else
...
retry:
    do
    {
        ...
        item_slot = prompt_invent_item(jtrans("Use on which item? (\\ to view known items)").c_str(),
        ...
        if (item_slot == PROMPT_ABORT
            && yesno(jtrans("Really abort (and waste the scroll)?").c_str()))
        {
            canned_msg(MSG_OK);
            return false;
        }
    ...
    {
        mpr(jtrans("This item is melded into your body!"));
        if (Options.auto_list)
            more();
        goto retry;
    }
...
void read_scroll(int slot)
{
    ...
    {
        mpr(jtrans("You cannot read scrolls while unable to breathe!"));
        return;
    }
    ...
    int item_slot = (slot != -1) ? slot
                                 : prompt_invent_item(jtrans("Read which item?").c_str(),
                                                       MT_INVLIST,
                                                       OBJ_SCROLLS,
                                                       true, true, true, 0, -1,
                                                       NULL, OPER_READ);
    ...
    {
        mpr(jtrans("You can't read that!"));
    ...
    {
        crawl_state.zero_turns_taken();
        return mpr(jtrans("You have no means to unroll scrolls."));
    }

    if (silenced(you.pos()))
    {
        mpr(jtrans("Magic scrolls do not work when you're silenced!"));
        crawl_state.zero_turns_taken();
        return;
    }
    ...
    if (alreadyknown)
    {
        ...
        case SCR_ENCHANT_WEAPON_III:
            ...
            {
                mpr(jtrans("This weapon cannot be enchanted."));
                return;
            }
            else if (!you.weapon() || !is_weapon(*you.weapon()))
            {
                mpr(jtrans("You are not wielding a weapon."));
                return;
            }
            break;
    ...
    if (you.stat_zero[STAT_INT] && !one_chance_in(5))
    {
        ...
        mpr(jtrans("You almost manage to decipher the scroll, but fail in this attempt."));
        return;
    }
        ...
        && x_chance_in_y(player_mutation_level(MUT_BLURRY_VISION), 5))
    {
        mpr((player_mutation_level(MUT_BLURRY_VISION) == 3 && one_chance_in(3))
                        ? jtrans("This scroll appears to be blank.")
                        : jtrans("The writing blurs in front of your eyes."));
        return;
    }
    ...
    string pre_succ_msg =
            make_stringf(jtrans("As you read the %s, it crumbles to dust.").c_str(),
    ...
    case SCR_IMMOLATION:
    {
        ...
        if (had_effect)
            mpr(jtrans("The creatures around you are filled with an inner flame!").c_str());
        else
            mpr(jtrans("The air around you briefly surges with heat, but it dissipates.").c_str());
    ...
    case SCR_BRAND_WEAPON:
          ...
          {
            mpr(pre_succ_msg.c_str());
            mpr(jtrans("It is a scroll of brand weapon.").c_str());
...
{
    int item_slot = prompt_invent_item(jtrans("Examine which item?").c_str(),
                                        MT_INVLIST, -1,
                                        true, true, true, 0, -1, NULL,
                                        OPER_EXAMINE);
    if (prompt_failed(item_slot))
...
static bool _prompt_eat_bad_food(const item_def food)
{
    ...
    const string qualifier = colour
                             + (is_poisonous(food)      ? jtrans("poisonous").c_str() :
                                is_mutagenic(food)      ? jtrans("mutagenic").c_str() :
                                causes_rot(food)        ? jtrans("rot-inducing").c_str() :
                                is_forbidden_food(food) ? jtrans("forbidden").c_str() : "")
                             + colour_off;

    string prompt  = jtrans("Reallyitem").c_str();
           prompt += jtrans("thisitem").c_str() + qualifier;
           prompt += (food.base_type == OBJ_CORPSES ? jtrans("corpse").c_str()
                                                    : jtrans("chunk of meat").c_str());
