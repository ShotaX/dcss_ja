        ...
        || victim->is_stationary())
      {
        if (!mons)
            canned_msg(MSG_YOU_UNAFFECTED);
              ...
              simple_monster_message(mons, jtrans("is unaffected.").c_str());
          return 0;
      }
...
bool backlight_monsters(coord_def where, int pow, int garbage)
{
    ...
    if (lvl == 0)
        simple_monster_message(mons, jtrans("is outlined in light.").c_str());
    else if (lvl == 4)
        simple_monster_message(mons, jtrans("glows brighter for a moment.").c_str());
    else
        simple_monster_message(mons, jtrans("glows brighter.").c_str());
...
bool do_slow_monster(monster* mon, const actor* agent, int dur)
{
        ...
        && mon->add_ench(mon_enchant(ENCH_SLOW, 0, agent, dur)))
    {
        if (!mon->paralysed() && !mon->petrified()
            && simple_monster_message(mon, jtrans("seems to slow down.").c_str()))
        {
            return true;
        }
    }
