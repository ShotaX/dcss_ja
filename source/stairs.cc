...
bool check_annotation_exclusion_warning()
{
    ...
    {
        mpr(jtrans("Within the labyrinth you'll only be able to teleport away from the exit!"));
        if (!yesno(jtrans("Continue anyway?").c_str(), false, 'N', true, false))
        {
            canned_msg(MSG_OK);
    ...
    {
        mpr(jtrans("Warning, next level annotated:") +
    ...
    {
        mpr(jtrans("This staircase is marked as excluded!"), MSGCH_WARN);
        might_be_dangerous = true;
    }

    if (might_be_dangerous
        && !yesno(jtrans("Enter next level anyway?").c_str(), true, 'n', true, false))
    {
        canned_msg(MSG_OK);
        interrupt_activity(AI_FORCE_INTERRUPT);
        crawl_state.level_annotation_shown = false;
...
static bool _stair_moves_pre(dungeon_feature_type stair)
{
    ...
    mprf(jtrans("%s moves away as you attempt to %s it!").c_str(), verb.c_str(), stair_str.c_str());
                           ...
                           branch_type old_branch)
{
    ...
    else if (feat_is_escape_hatch(stair))
    {
        ...
        {
            mprf(jtrans("You %s downwards.").c_str(),
                 you.flight_mode() ? jtrans("fly").c_str() : jtrans("slide").c_str());
        }
    }
    else if (feat_is_gate(stair))
    {
        mprf(jtrans("You %s %s through the gate.").c_str(),
             going_up ? jtrans("up").c_str() : jtrans("down").c_str(),
             you.flight_mode() ? jtrans("fly").c_str() : jtrans("go").c_str());
    }
    else
    {
        mprf(jtrans("You %s %swards").c_str(),
			 going_up ? jtrans("up").c_str() : jtrans("down").c_str(),
             you.flight_mode() ? jtrans("fly").c_str() : jtrans("climb").c_str());
...
void up_stairs(dungeon_feature_type force_stair)
{
        ...
        && coinflip())
    {
        ...
        mprf(jtrans("In your confused state, you trip and fall back %s.").c_str(), fall_where);
        if (!feat_is_staircase(stair_find))
    ...
    {
        mprf(jtrans("Welcome back to %s!").c_str(),
    ...
    if (!allow_control_teleport(true))
        mpr(jtrans("You sense a powerful magical force warping space."), MSGCH_WARN);
...
void down_stairs(dungeon_feature_type force_stair)
{
    ...
    if (shaft)
    {
        ...
        if (shaft_dest == level_id::current())
        {
            if (known_shaft)
            {
                ...
                mpr(jtrans("The strain on the space-time continuum destroys the shaft!"));
            }
            _maybe_destroy_trap(you.pos());
            return;
        }
        ...
        if (!you.flight_mode() || force_stair)
            mpr(jtrans("You fall through a shaft!"));
        if (you.flight_mode() && !force_stair)
            mpr(jtrans("You dive down through the shaft."));
    ...
    if (stair_find == DNGN_ENTER_ZOT && !you.opened_zot)
    {
        ...
        if (runes.size() < NUMBER_OF_RUNES_NEEDED)
        {
            ...
            default:
                mprf(jtrans("You need at least %d runes to enter this place.").c_str(),
        ...
        shuffle_array(runes);
        mprf(jtrans("You insert the %s rune into the lock.").c_str(), rune_type_name(runes[0]));
#ifdef USE_TILE_LOCAL
...
#endif
        mpr(jtrans("The lock glows an eerie green colour!"));
        more();

        mprf(jtrans("You insert the %s rune into the lock.").c_str(), rune_type_name(runes[1]));
        big_cloud(CLOUD_BLUE_SMOKE, &you, you.pos(), 20, 7 + random2(7));
        viewwindow();
        mpr(jtrans("Heavy smoke blows from the lock!"));
        more();

        mprf(jtrans("You insert the %s rune into the lock.").c_str(), rune_type_name(runes[2]));
    ...
    {
        mprf(jtrans("Welcome %sto %s!").c_str(),branches[you.where_are_you].longname,
             you.char_direction == GDT_GAME_START ? "" : jtrans("back").c_str());
    }
        ...
        && coinflip())
    {
        ...
        mprf(jtrans("In your confused state, you trip and fall %s.").c_str(), fall_where);
    ...
    case BRANCH_LABYRINTH:
        ...
        // Are these too long?
        mpr(jtrans("As you enter the labyrinth, previously moving walls settle noisily into place."));
        mpr(jtrans("You hear the metallic echo of a distant snort before it fades into the rock."));
        break;

    case BRANCH_ABYSS:
        if (old_level.branch == BRANCH_ABYSS)
        {
            mpr(jtrans("You plunge deeper into the Abyss."), MSGCH_BANISHMENT);
            break;
        }
        if (!force_stair)
        ...
        {
            mpr(jtrans("You feel Cheibriados slowing down the madness of this place."),
    ...
    case BRANCH_PANDEMONIUM:
        ...
        {
            mpr(jtrans("You enter the halls of Pandemonium!"));
            mpr(jtrans("To return, you must find a gate leading back."));
        }
        break;
    ...
    if (entered_branch)
    {
        ...
        if (branches[branch].entry_message)
            ...
            mprf(jtrans("Welcome to %s!").c_str(), branches[branch].longname);
        enter_branch(branch, old_level);
    }

    if (stair_find == DNGN_ENTER_HELL)
    {
        mpr(jtrans("Welcome to Hell!"));
        mpr(jtrans("Please enjoy your stay."));
