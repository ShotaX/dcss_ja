...
static void _change_skill_level(skill_type exsk, int n)
{
    ...
    {
        mprf(MSGCH_INTRINSIC_GAIN, jtrans("You have mastered %s!").c_str(), skill_name(exsk));
    ...
    {
        mprf(MSGCH_INTRINSIC_GAIN, jtrans("Your %s skill %s to level %d!").c_str(),
             skill_name(exsk), (n > 0) ? jtrans("increases").c_str() : jtrans("decreases").c_str(),
             you.skills[exsk]);
    }
    else if (you.num_turns)
    {
        mprf(MSGCH_INTRINSIC_GAIN, jtrans("Your %s skill %d levels %s and is now at level %d!").c_str(), skill_name(exsk), abs(n), (n > 0) ? jtrans("gained").c_str() : jtrans("lost").c_str(),
        	you.skills[exsk]);
    }
...
static void _check_start_train()
{
    ...
    if (!skills.empty())
        mpr(jtrans("You resume training") + skill_names(skills));
...
static void _check_stop_train()
{
    ...
    if (!skills.empty())
    {
        mpr(jtrans("You stop training") + skill_names(skills));
