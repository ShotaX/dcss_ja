...
{
    if (reason.empty())
        reason = jtrans("Can't repeat that command.");
...
{
    if (reason.empty())
        reason = jtrans("Can't redo that command.");
                           ...
                           const activity_interrupt_data &at)
{
    ...
    {
        if (ai == AI_FULL_MP)
            crawl_state.cancel_cmd_repeat(jtrans("Magic restored.").c_str());
        else if (ai == AI_FULL_HP)
        {
            string health = (you.species == SP_DJINNI ? "EP" : "HP");

            crawl_state.cancel_cmd_repeat(health + jtrans("restored").c_str());
        }
        else
            crawl_state.cancel_cmd_repeat(jtrans("Command repetition interrupted.").c_str());
