...
spret_type cast_sublimation_of_blood(int pow, bool fail)
{
    ...
    if (wielded != -1)
    {
            ...
            && you.inv[wielded].sub_type == FOOD_CHUNK)
        {
            ...
            mpr(jtrans("The chunk of flesh you are holding crumbles to dust."));

            mpr(jtrans("A flood of magical energy pours into your mind!"));
                 ...
                 && item_type_known(you.inv[wielded]))
        {
            ...
            mprf(jtrans("The blood within %s froths and boils.").c_str(),
                 you.inv[wielded].quantity > 1 ? jtrans("one of your flasks").c_str()
                                               : jtrans("the flask you are holding").c_str());
...
static bool _try_recall(mid_t mid)
{
    ...
    else if (mons->alive())
    {
        ...
        else
        {
            ...
            {
                recall_orders(mons);
                simple_monster_message(mons, jtrans("is recalled.").c_str());
                return true;
            }
        }
    }
...
spret_type cast_passwall(const coord_def& delta, int pow, bool fail)
{
    ...
    else
    {
        ...
        if (grd(dest) == DNGN_DEEP_WATER)
            msg = jtrans("You sense a large body of water on the other side of the rock.");
        else if (grd(dest) == DNGN_LAVA)
            msg = jtrans("You sense an intense heat on the other side of the rock.");
...
{
    mpr(jtrans("Your icy shield evaporates."), MSGCH_DURATION);
...
spret_type cast_stoneskin(int pow, bool fail)
{
    ...
    {
        mpr(jtrans("This spell conflicts with another spell still in effect."));
        return SPRET_ABORT;
    }
    ...
    if (you.species == SP_LAVA_ORC)
    {
        ...
        // mprf(%s, jtrans("Your skin is naturally stony.").c_str());
          return SPRET_ABORT;
    }
...
spret_type cast_darkness(int pow, bool fail)
{
    ...
    if (you.duration[DUR_DARKNESS])
        mprf(MSGCH_DURATION,"%s",jtrans("It gets a bit darker.").c_str());
    else
        mprf(MSGCH_DURATION,"%s",jtrans("It gets dark.").c_str());
