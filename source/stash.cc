...
void LevelStashes::no_stash(int x, int y)
{
    ...
    mpr(en? jtrans("I'll no longer ignore what I see on this square.")
          : jtrans("Ok, I'll ignore what I see on this square."));
}
...
void StashTracker::search_stashes()
{
    ...
    {
        mpr(jtrans("Your search expression is invalid."), MSGCH_PLAIN);
        lastsearch = help;
        return ;
    }
    ...
    if (results.empty())
    {
        mpr(jtrans("Can't find anything matching that."), MSGCH_PLAIN);
        return;
    }

    if (results.size() > SEARCH_SPAM_THRESHOLD)
    {
        mpr(jtrans("Too many matches; use a more specific search."), MSGCH_PLAIN);
        return;
    }
