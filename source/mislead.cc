...
void mons_cast_mislead(monster* mons)
{
    ...
    if (you.clarity())
    {
        mpr(jtrans("Your vision blurs momentarily."));
    ...
    if (you.duration[DUR_MISLED] > old_value)
    {
        ...
        if (old_value <= 0)
        {
            mpr(jtrans("But for a moment, strange images dance in front of your eyes."), MSGCH_WARN);
#ifdef USE_TILE_LOCAL
            ...
            mpr(jtrans("You are even more misled!"), MSGCH_WARN);
