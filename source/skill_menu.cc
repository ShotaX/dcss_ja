...
string SkillMenuSwitch::get_help()
{
    ...
    case SKM_MODE_MANUAL:
        return jtrans("In manual mode, experience is spread evenly across all activated skills.");
    case SKM_DO_PRACTISE:
        if (skm.is_set(SKMF_SIMPLE))
            return hints_skills_info();
        else
        {
            return jtrans("Press the letter");
        }
    case SKM_DO_FOCUS:
        return jtrans("focusedskill");
    case SKM_LEVEL_ENHANCED:
        if (skm.is_set(SKMF_ENHANCED))
        {
            return make_stringf(jtrans("Skills enhanced by the power of %s are in <blue>blue</blue>.").c_str(),
...
//Fixme_ja?
            ...
            return comma_separated_line(causes.begin(), causes.end())
                   + jtrans("Skills reduced by are in <magenta>magenta</magenta>.");
        }
    case SKM_VIEW_TRAINING:
        ...
        {
            return jtrans("The percentage of the experience") +"\n";
        }
    case SKM_VIEW_PROGRESS:
        return jtrans("The percentage of the progress") +"\n";
    case SKM_VIEW_TRANSFER:
        return jtrans("The progress of the knowledge transfer") +"\n";
    default: return "";
    }
}
...
string SkillMenuSwitch::get_name(skill_menu_state state)
{
    switch (state)
    {
    case SKM_MODE_AUTO:      return jtrans("auto");
    case SKM_MODE_MANUAL:    return jtrans("manual");
    case SKM_DO_PRACTISE:    return "practise";
    case SKM_DO_FOCUS:       return "focus";
    case SKM_SHOW_KNOWN:     return "known";
    case SKM_SHOW_DEFAULT:   return jtrans("default");
    case SKM_SHOW_ALL:       return jtrans("all");
    case SKM_LEVEL_ENHANCED:
    ...
    case SKM_VIEW_TRAINING:  return jtrans("training");
    case SKM_VIEW_PROGRESS:  return jtrans("progress");
    case SKM_VIEW_TRANSFER:  return "transfer";
    case SKM_VIEW_POINTS:    return jtrans("points");
...
bool SkillMenu::exit()
{
    ...
    {
        set_help(jtrans("You need to enable at least one skill."));
        return false;
    }
...
void SkillMenu::set_default_help()
{
    ...
    if (is_set(SKMF_RESKILL_FROM))
    {
        text = jtrans("SKMF_RESKILL_FROM");
    }
    else if (is_set(SKMF_RESKILL_TO))
    {
        text = jtrans("SKMF_RESKILL_TO");
    }
    else if (is_set(SKMF_EXPERIENCE))
    {
        text = jtrans("SKMF_EXPERIENCE");
    }
    ...
    else
    {
        ...
        if (is_set(SKMF_CROSSTRAIN))
            text += jtrans("Crosstraining is in <green>green</green>.");
        if (is_set(SKMF_ANTITRAIN))
            text += jtrans("Antitraining is in <red>red</red>.");

        if (is_set(SKMF_CROSSTRAIN) && is_set(SKMF_ANTITRAIN))
        {
            text += jtrans("The skill responsible for the bonus or malus is marked with '*'.");
        }
        else if (is_set(SKMF_CROSSTRAIN))
        {
            text += jtrans("The skill responsible for the bonus is marked with '<green>*</green>'.");
        }
        else if (is_set(SKMF_ANTITRAIN))
        {
            text += jtrans("The skill responsible for the malus is marked with '<red>*</red>'.");
        }
    }
