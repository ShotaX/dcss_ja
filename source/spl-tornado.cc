...
spret_type cast_tornado(int powc, bool fail)
{
    ...
    if (friendlies
        && !yesno(jtrans("There are friendlies around, are you sure you want to hurt them?").c_str(),
                  true, 'n'))
    {
        canned_msg(MSG_OK);
        return SPRET_ABORT;
    ...
    mprf(jtrans("A great vortex of raging winds %s.").c_str(),
         (you.airborne() || you.form == TRAN_TREE) ?
         jtrans("appears around you").c_str()
		 : jtrans("appears and lifts you up").c_str());
