...
bool attribute_increase()
{
    ...
    pop->push_entry(me);
#else
    mpr(jtrans("Your experience leads to an increase in your attributes!"),
        MSGCH_INTRINSIC_GAIN);
    learned_something_new(HINT_CHOOSE_STAT);
    mpr(jtrans("Increase (S)trength, (I)ntelligence, or (D)exterity?"), MSGCH_PROMPT);
#endif
...
void jiyva_stat_action()
{
    ...
    if (choices)
    {
        simple_god_message(jtrans("'s power touches on your attributes.").c_str());
                 ...
                 const char *cause, bool see_source)
{
    ...
    if (!suppress_msg)
    {
        mprf((amount > 0) ? MSGCH_INTRINSIC_GAIN : MSGCH_WARN,
             jtrans("You feel %s2.").c_str(),
             jtrans(stat_desc(which_stat, (amount > 0) ? SD_INCREASE : SD_DECREASE)).c_str());
    }
               ...
               const char *cause, bool see_source)
{
    ...
    if (!force)
    {
        ...
        {
            mprf(jtrans("Your divine stamina protects you from %s loss.").c_str(),
    ...
    mprf(stat_loss > 0 ? MSGCH_WARN : MSGCH_PLAIN,
         jtrans("You feel %s%s.").c_str(),
		 stat_loss > 0 ? "" : jtrans("for a moment").c_str(),
         jtrans(stat_desc(which_stat, SD_LOSS)).c_str());
                  ...
                  bool suppress_msg, bool recovery)
{
    ...
    if (!suppress_msg)
    {
        mprf(recovery ? MSGCH_RECOVERY : MSGCH_PLAIN,
             jtrans("You feel your %s returning.").c_str(),
             jtrans(_stat_name(which_stat).c_str()).c_str());
    }
...
static void _handle_stat_change(stat_type stat, const char* cause, bool see_source)
{
    ASSERT_RANGE(stat, 0, NUM_STATS);
      ...
      if (you.stat(stat) <= 0 && you.stat_zero[stat] == 0)
      {
        ...
        mprf(MSGCH_WARN, jtrans("You have lost your %s.").c_str(), stat_desc(stat, SD_NAME));
...
void update_stat_zero()
{
    for (int i = 0; i < NUM_STATS; ++i)
    {
        ...
        else if (you.stat_zero[s])
        {
            ...
            {
                mprf(jtrans("Your %s has recovered.").c_str(), stat_desc(s, SD_NAME));
