...
void monster::equip_weapon(item_def &item, int near, bool msg)
{
    ...
    if (msg)
    {
        snprintf(info, INFO_SIZE, jtrans("wields %s.").c_str(),
    ...
    if (msg)
    {
        ...
        case SPWPN_FLAMING:
            ...
            break;
        case SPWPN_FREEZING:
            mpr(jtrans("It glows with a cold blue light!"));
            break;
        case SPWPN_HOLY_WRATH:
            mpr(jtrans("It softly glows with a divine radiance!"));
            break;
        case SPWPN_ELECTROCUTION:
            mpr(jtrans("You hear the crackle of electricity."), MSGCH_SOUND);
            break;
        case SPWPN_VENOM:
        ...
        case SPWPN_FLAME:
            ...
            break;
        case SPWPN_FROST:
            mpr(jtrans("It is covered in frost."));
            break;
        case SPWPN_RETURNING:
            mpr(jtrans("It wiggles slightly."));
            break;
        case SPWPN_DISTORTION:
            mpr(jtrans("Its appearance distorts for a moment."));
            break;
        case SPWPN_CHAOS:
            mpr(jtrans("It is briefly surrounded by a scintillating aura of random colours."));
            break;
        case SPWPN_PENETRATION:
            mprf(jtrans("%s %s briefly pass through it before %s manages to get a firm grip on it.").c_str(),
...
void monster::equip_armour(item_def &item, int near)
{
    ...
    {
        snprintf(info, INFO_SIZE, jtrans("wears %s.").c_str(),
...
void monster::equip_jewellery(item_def &item, int near)
{
    ...
    if (need_message(near))
    {
        snprintf(info, INFO_SIZE, jtrans("puts on %s.").c_str(),
                 item.name(DESC_A).c_str());
        simple_monster_message(this, info);
    }
...
void monster::unequip_weapon(item_def &item, int near, bool msg)
{
    ...
    if (msg)
    {
        snprintf(info, INFO_SIZE, jtrans("unwields %s.").c_str(),
                             item.name(DESC_A, false, false, true, false,
                             ISFLAG_CURSED).c_str());
        msg = simple_monster_message(this, info);
    }
...
void monster::unequip_armour(item_def &item, int near)
{
    if (need_message(near))
    {
        snprintf(info, INFO_SIZE, jtrans("takes off %s.").c_str(),
                 item.name(DESC_A).c_str());
        simple_monster_message(this, info);
    }
...
void monster::pickup_message(const item_def &item, int near)
{
    if (need_message(near))
    {
        ...
        mprf(jtrans("%s picks up %s.").c_str(),
             name(DESC_THE).c_str(),
             item.base_type == OBJ_GOLD ? jtrans("some gold").c_str()
                                        : item.name(DESC_A).c_str());
    }
}
...
bool monster::pickup(item_def &item, int slot, int near, bool force_merge)
{
    ...
    if (other_mon != NULL)
    {
        if (other_mon == this)
        {
            ...
            {
                mprf(MSGCH_DIAGNOSTICS, jtrans("Monster %s already holding item %s.").c_str(),
            ...
            {
                mprf(MSGCH_DIAGNOSTICS, jtrans("Item %s thinks it's already held by monster %s.").c_str(),
                     item.name(DESC_PLAIN, false, true).c_str(),
                     name(DESC_PLAIN, true).c_str());
            }
        }
        ...
        {
            mprf(MSGCH_DIAGNOSTICS, jtrans("Item %s, held by dead monster, being picked up by monster %s.").c_str(),
                 item.name(DESC_PLAIN, false, true).c_str(),
                 name(DESC_PLAIN, true).c_str());
        }
        else
        {
            mprf(MSGCH_DIAGNOSTICS, jtrans("Item %s, held by monster %s, being picked up by monster %s.").c_str(),
                 item.name(DESC_PLAIN, false, true).c_str(),
...
bool monster::drop_item(int eslot, int near)
{
    ...
    if (pitem->flags & ISFLAG_SUMMONED)
    {
        if (need_message(near))
        {
            mprf(jtrans("%s %s as %s drops %s!").c_str(),
                 name(DESC_THE).c_str(),
                 summoned_poof_msg(this, *pitem).c_str(),
                 pitem->name(DESC_THE).c_str(),
                 pitem->quantity > 1 ? jtrans("them").c_str() : jtrans("it").c_str());
        }
    ...
    else
    {
        if (need_message(near))
        {
            mprf(jtrans("%s drops %s.").c_str(), name(DESC_THE).c_str(),
...
string monster::conj_verb(const string &verb) const
{
    ...
    if (verb == "snap closed at")
        return jtrans("snaps closed at");

    if (verb == "pounce on")
        return jtrans("pounces on");

    if (ends_with(verb, "f") || ends_with(verb, "")
        || ends_with(verb, ""))
    {
        return (verb + "");
    }
    ...
    if (floundering() && one_chance_in(4))
    {
        if (verbose)
        {
            if (you.can_see(this))
            {
                mpr(name(DESC_THE)
                    + (liquefied(pos())
                       ? jtrans("becomes momentarily stuck in the liquid earth.")
                       : jtrans("splashes around in the water.")));
            }
            else if (player_can_hear(pos(), LOS_RADIUS))
                mpr(jtrans("You hear a splashing noise."), MSGCH_SOUND);
        }
...
bool monster::go_frenzy(actor *source)
{
    ...
    if (has_ench(ENCH_SLOW))
    {
        ...
        simple_monster_message(this,
            make_stringf(jtrans("shakes off %s lethargy.").c_str(),
    ...
    if (simple_monster_message(this, jtrans("flies into a frenzy!").c_str()))
...
void monster::banish(actor *agent, const string &)
{
    ...
    if (mons_is_projectile(type))
        return;
    simple_monster_message(this, jtrans("is devoured by a tear in reality.").c_str(),
                           MSGCH_BANISHMENT);
...
bool monster::drain_exp(actor *agent, bool quiet, int pow)
{
    ...
    if (!quiet && you.can_see(this))
        mprf(jtrans("%s is drained!").c_str(), name(DESC_THE).c_str());
...
bool monster::rot(actor *agent, int amount, int immediate, bool quiet)
{
    ...
    if (!quiet && you.can_see(this))
    {
        mprf(jtrans("%s %s!").c_str(), name(DESC_THE).c_str(),
...
bool monster::fully_petrify(actor *atk, bool quiet)
{
    bool msg = !quiet && simple_monster_message(this, mons_is_immotile(this) ?
                         jtrans("turns to stone!").c_str() : jtrans("stops moving altogether!").c_str());
...
bool monster::sicken(int amount, bool unused, bool quiet)
{
    ...
    if (!has_ench(ENCH_SICK) && you.can_see(this) && !quiet)
    {
        ...
        mprf(jtrans("%s looks sick.").c_str(), name(DESC_THE).c_str());
    }
...
bool monster::bleed(const actor* agent, int amount, int degree)
{
    ...
    {
        mprf(jtrans("%s begins to bleed from %s wounds!").c_str(), name(DESC_THE).c_str(),
             pronoun(PRONOUN_POSSESSIVE).c_str());
    }
...
bool monster::malmutate(const string &reason)
{
    ...
    simple_monster_message(this, jtrans("twists and deforms.").c_str());
                                     ...
                                     int killernum)
{
    ...
    if (alive() && has_ench(ENCH_AQUATIC_LAND))
    {
        if (!feat_is_watery(grd(pos())))
            simple_monster_message(this, jtrans("flops around on dry land!").c_str());
        else if (!feat_is_watery(grd(oldpos)))
        {
            simple_monster_message(this, jtrans("dives back into the water!").c_str());
    ...
    if (prop & FPROP_BLOODY)
    {
        ...
        if (genus == MONS_JELLY || genus == MONS_GIANT_SLUG)
        {
            ...
            if (you.see_cell(pos()) && !visible_to(&you))
            {
                ...
                mprf(jtrans("The bloodstain on %s disappears!").c_str(), desc.c_str());
            }
        }
    }
}
...
bool monster::do_shaft()
{
    ...
    if (get_trap_type(pos()) != TRAP_SHAFT)
    {
        ...
        if (!ground_level() || total_weight() == 0)
        {
            if (mons_near(this))
            {
                if (visible_to(&you))
                {
                    mprf(jtrans("A shaft briefly opens up underneath %s!").c_str(),
                         name(DESC_THE).c_str());
                }
                else
                    mpr(jtrans("A shaft briefly opens up in the floor!"));
            }
    ...
    const bool reveal =
        simple_monster_message(this, jtrans("falls through a shaft!").c_str());
...
{
    simple_monster_message(this, jtrans("drinks a potion.").c_str());
    ...
    {
        if (heal(5 + random2(7)))
            simple_monster_message(this, jtrans("is healed!").c_str());
                               ...
                               beam_type flavour)
{
    if (type == MONS_SIXFIRHY && flavour == BEAM_ELECTRICITY)
    {
        if (!alive()) // overcharging is deadly
            simple_monster_message(this, jtrans("explodes in a shower of sparks!").c_str());
        else if (heal(damage*2, false))
            simple_monster_message(this, jtrans("seems to be charged up!").c_str());
        return;
    }
    ...
    else if (type == MONS_SPRIGGAN_RIDER)
    {
        ...
        if (damage > 0 && x_chance_in_y(damage, damage + hit_points))
        {
            ...
            if (fly_died && !is_habitable(pos()))
            {
                hit_points = 0;
                if (observable())
                    mprf(jtrans("As %s mount dies, %s plunges down into %s!").c_str(),
                         pronoun(PRONOUN_POSSESSIVE).c_str(),
                         name(DESC_THE).c_str(),
                         grd(pos()) == DNGN_LAVA ?
                             jtrans("lava and is incinerated").c_str() :
                             jtrans("deep water and drowns").c_str());
            }
            else if (fly_died && observable())
            {
                mprf(jtrans("%s jumps down from %s now dead mount.").c_str(),
                     name(DESC_THE).c_str(),
                     pronoun(PRONOUN_POSSESSIVE).c_str());
            }
        }
...
void monster::steal_item_from_player()
{
    ...
    if (steal_what == -1 || you.gold > 0 && one_chance_in(10))
    {
        ...
        mprf(jtrans("%s steals %s your gold!").c_str(),
             name(DESC_THE).c_str(),
             stolen_amount == you.gold ? jtrans("all").c_str() : jtrans("some of").c_str());
...
bool monster::shove(const char* feat_name)
{
    for (distance_iterator di(pos()); di; ++di)
        if (monster_space_valid(this, *di, false))
        {
            ...
            mgrd(*di) = mindex();
            simple_monster_message(this,
                make_stringf(jtrans("is pushed out of the %s.").c_str(), feat_name).c_str());
...
bool monster::check_clarity(bool silent) const
{
    ...
    {
        simple_monster_message(this, jtrans("seems unimpeded by the mental distress.").c_str());
...
bool monster::check_stasis(bool silent, bool calc_unid) const
{
    ...
    if (!silent && you.can_see(this) && !mons_is_lurking(this))
    {
        simple_monster_message(this, jtrans("looks uneasy for a moment.").c_str());
