...
bool monster::blink_to(const coord_def& dest, bool quiet, bool jump)
{
    ...
    const string verb = (jump ? jtrans("leapshite").c_str() : jtrans("blinkshite").c_str());
    ...
    if (is_constricted())
    {
        ...
        if (!attempt_escape(2))
        {
            if (!quiet)
            {
                string message = verb + jtrans("free from constriction.").c_str() + jtrans("struggles to").c_str();
    ...
    if (!quiet)
    {
        string message = " " + conj_verb(verb)
                         + (was_constricted ? jtrans("free!!!!").c_str() : "!");
        simple_monster_message(this, message.c_str());
    }
