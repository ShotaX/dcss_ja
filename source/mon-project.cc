                     ...
                     int foe, bool fail)
{
    ...
    if (!mon)
    {
        mpr(jtrans("Failed to spawn projectile."), MSGCH_ERROR);
        return SPRET_ABORT;
    }
...
static void _iood_stop(monster& mon, bool msg = true)
{
    ...
    if (msg)
        simple_monster_message(&mon, jtrans("dissipates.").c_str());
...
move_again:
    ...
    if (cell_is_solid(pos) || victim)
    {
        if (cell_is_solid(pos))
        {
            if (you.see_cell(pos))
            {
                mprf(jtrans("%s hits %s").c_str(), mon.name(DESC_THE, true).c_str(),
        ...
        if (mons && iood && mons_is_projectile(victim->type))
        {
            if (mon.observable())
                ...
                noisy(40, pos, jtrans("You hear a loud magical explosion!").c_str());
        ...
        if (mons && mons_is_boulder(&mon) && mons_is_boulder(mons))
        {
            if (mon.observable())
                ...
                noisy(20, pos, jtrans("You hear a loud crashing sound!").c_str());
        ...
        if (victim && _iood_shielded(mon, *victim))
        {
            ...
            {
                if (victim->is_player())
                    mprf(jtrans("You block %s.").c_str(), mon.name(DESC_THE, true).c_str());
                else
                {
            ...
            if (victim->is_player())
            {
                mprf(jtrans("Your %s reflects %s!").c_str(),
            ...
            else if (you.see_cell(pos))
            {
                ...
                {
                    mprf(jtrans("%s reflects %s with %s %s!").c_str(),
                ...
                {
                    mprf(jtrans("%s bounces off thin air!").c_str(),
                        mon.name(DESC_THE, true).c_str());
                }
            }
            victim->shield_block_succeeded(&mon);
    ...
    {
        mprf(jtrans("%s screeches to a halt.").c_str(), mon.name(DESC_THE, true).c_str());
