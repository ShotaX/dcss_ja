...
spret_type brand_weapon(brand_type which_brand, int power, bool fail)
{
    ...
    {
        mpr(jtrans("This weapon is already enchanted."));
        return SPRET_ABORT;
    }
    ...
    case SPWPN_FLAME:
    case SPWPN_FLAMING:
        msg += jtrans("bursts into flame!");
        duration_affected = 7;
        break;

    case SPWPN_FROST:
        msg += jtrans("frosts over!");
        duration_affected = 7;
        break;

    case SPWPN_FREEZING:
        msg += jtrans("glows blue.");
        duration_affected = 7;
        break;

    case SPWPN_VENOM:
        msg += jtrans("starts dripping with poison.");
        duration_affected = 15;
        break;

    case SPWPN_DRAINING:
        msg += jtrans("crackles with unholy energy.");
        duration_affected = 12;
        break;

    case SPWPN_VORPAL:
        msg += jtrans("glows silver and looks extremely sharp.");
    ...
    case SPWPN_PAIN:
        ...
        if (silenced(you.pos()))
            msg += jtrans("writhes in agony.");
        else
        {
            msg += jtrans("shrieks in agony.");
            noisy(15, you.pos());
        }
        duration_affected = 8;
    ...
    case SPWPN_HOLY_WRATH:
        msg += jtrans("shines with holy light.");
        break;

    case SPWPN_ELECTROCUTION:
        msg += jtrans("starts to spark.");
        break;

    case SPWPN_ANTIMAGIC:
        msg += jtrans("depletes magic around it.");
        break;

    case SPWPN_CHAOS:
        msg += jtrans("glistens with random hues.");
        break;

    case SPWPN_RETURNING:
        msg += jtrans("wiggles in your") + you.hand_name(false) + ".";
        duration_affected = 5;
        break;
    ...
    if (emit_special_message)
        ...
        mprf(jtrans("%s flashes.").c_str(), weapon.name(DESC_YOUR).c_str());
