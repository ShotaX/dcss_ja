                        ...
                        formatted_string *fs)
{
    ...
    for (j = 0; j < SPELLBOOK_SIZE; j++)
    {
        ...
        out.cprintf("%s", chop_string(jtrans(spell_title(stype)).c_str(), 29).c_str());
        ...
        for (int j = 0; j < SPELLBOOK_SIZE; ++j)
        {
            ...
            if (flags & (SPFLAG_MONSTER | SPFLAG_TESTING))
            {
                ...
                item.sub_type  = i;
                    ...
                    jtrans(spell_title(spell)).c_str());
            }
#endif
                          ...
                          spell_type current_spell = SPELL_NO_SPELL)
{
    ...
    {
        if (!just_check)
            mpr(jtrans("You can't handle any books in this form."), MSGCH_PROMPT);
        return false;
    }
    ...
    if (num_books == 0)
    {
        if (!just_check)
        {
            if (num_unknown > 1)
            {
                mpr(jtrans("You must pick up those books before reading them."),
                    MSGCH_PROMPT);
            }
            else if (num_unknown == 1)
            {
                mpr(jtrans("You must pick up this book before reading it."),
                    MSGCH_PROMPT);
            }
            else
            {
                mpr(jtrans("You aren't carrying or standing over any spellbooks."),
                    MSGCH_PROMPT);
            }
        }
        return false;
    ...
    else if (num_unreadable == num_books)
    {
        if (!just_check)
        {
            mprf(MSGCH_PROMPT, jtrans("All of the spellbooks%s are beyond your current level of comprehension.").c_str(),
                 num_on_ground == 0 ? jtrans("you're carrying").c_str() : "");
        }
        return false;
    }
    else if (book_hash.empty())
    {
        if (!just_check)
        {
            mpr(jtrans("None of the spellbooks you are carrying contain any spells."),
                MSGCH_PROMPT);
        }
        return false;
    }
    ...
    if (num_known == total)
        mpr(jtrans("You already know all available spells."), MSGCH_PROMPT);
    else if (num_race == total || (num_known + num_race) == total)
    {
        ...
        mprf(MSGCH_PROMPT,
             jtrans("You cannot memorise any of the available spells because you are %s.").c_str(), lichform ? jtrans("in Lich form").c_str()
    ...
    if (num_unreadable)
    {
        mprf(MSGCH_PROMPT, jtrans("Additionally, %u of your spellbooks are beyond your current level of understanding, and thus none of the spells in them are available to you.").c_str(), num_unreadable);
    }
...
static bool _sort_mem_spells(spell_type a, spell_type b)
{
    ...
    return (strcasecmp(jtrans(spell_title(a)).c_str(), jtrans(spell_title(b)).c_str()) < 0);
}
                                    ...
                                    unsigned int num_race)
{
    ...
    spell_menu.set_title(
        new MenuEntry(jtrans("Your Spells - Memorisation  (toggle to descriptions with '!')").c_str(),
            MEL_TITLE));

    spell_menu.set_title(
        new MenuEntry(jtrans("Your Spells - Descriptions  (toggle to memorisation with '!')").c_str(),
            MEL_TITLE), false);
    ...
    string more_str = make_stringf(jtrans("<lightgreen>%d spell level%s left<lightgreen>").c_str(),
                                   player_spell_levels());

    if (num_unreadable > 0)
    {
        more_str += make_stringf(jtrans(", <lightmagenta>%u overly difficult spellbook</lightmagenta>").c_str(),
                                 num_unreadable);
    }

    if (num_race > 0)
    {
        more_str += make_stringf(jtrans(", <lightred>%u spell%s unmemorisable</lightred>").c_str(),
                                 num_race);
    }
    ...
    for (unsigned int i = 0; i < spells.size(); i++)
    {
        ...
        desc << left;
        desc << chop_string(jtrans(spell_title(spell)).c_str(), 30);
...
static bool _learn_spell_checks(spell_type specspell)
{
    ...
    {
        mpr(jtrans("This spell is too difficult to memorise!"));
        return false;
    }
...
bool learn_spell(spell_type specspell)
{
    ...
    if (chance >= 0.025)
        mpr(jtrans("This spell is very dangerous to cast!"), MSGCH_WARN);
    else if (chance >= 0.005)
        mpr(jtrans("This spell is quite dangerous to cast!"), MSGCH_WARN);
    else if (chance >= 0.001)
        mpr(jtrans("This spell is slightly dangerous to cast."));

    snprintf(info, INFO_SIZE,
             jtrans("Memorise %s, consuming %d spell level and leaving %d?").c_str(),
             jtrans(spell_title(specspell)).c_str(), spell_levels_required(specspell),
...
bool forget_spell_from_book(spell_type spell, const item_def* book)
{
    ...
    mprf(jtrans("As you tear out the page describing %s, the book crumbles to dust.").c_str(),
        jtrans(spell_title(spell)).c_str());
    ...
    else
    {
        ...
        mprf("A bug prevents you from forgetting %s.", jtrans(spell_title(spell)).c_str());
        return false;
    }
}
...
int rod_spell(int rod)
{
    ...
    {
        mprf(MSGCH_PROMPT,
             jtrans("Evoke which spell from the rod ([a-%c] spell [?*] list)?").c_str(),
             'a' + num_spells - 1);
...
static bool _compare_spells(spell_type a, spell_type b)
{
    ...
    return (strcmp(jtrans(spell_title(a)).c_str(), jtrans(spell_title(b)).c_str()) < 0);
}
                             ...
                             string owner)
{
    ...
    if (spells.empty())
    {
        ...
        {
            snprintf(buf, sizeof(buf), jtrans("%s disliked all level %d spells").c_str(),
                    god_name(god).c_str(), level);
        }
        else if (god_discard == 0 && uncastable_discard > 0)
            sprintf(buf, jtrans("No level %d spells can be cast by you").c_str(), level);
        else if (god_discard > 0 && uncastable_discard > 0)
        {
            snprintf(buf, sizeof(buf),
                         jtrans("All level %d spells are either disliked by %s or cannot be cast by you.").c_str(),
                    level, god_name(god).c_str());
        }
        else
            sprintf(buf, jtrans("No level %d spells?!?!?!").c_str(), level);

        mprf(MSGCH_ERROR, jtrans("Could not create fixed level randart spellbook: %s").c_str(),
             buf);
