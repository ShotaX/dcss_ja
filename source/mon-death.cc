...
void pikel_band_neutralise()
{
    ...
    for (monster_iterator mi; mi; ++mi)
    {
     ...
     string final_msg;
    if (visible_slaves == 1)
        final_msg = jtrans("With Pikel's spell broken, the former slave thanks you for freedom.");
    else if (visible_slaves > 1)
        final_msg = jtrans("With Pikel's spell broken, the former slaves thank you for their freedom.");
...
void hogs_to_humans()
{
    ...
    {
        if (any == human)
            final_msg = jtrans("No longer under Kirke's spell, the hog turns into a human!");
        else
            final_msg = jtrans("No longer under Kirke's spell, the hog returns to its original form!");
    }
    else if (any > 1)
    {
        if (any == human)
            final_msg = jtrans("No longer under Kirke's spell, the hogs revert to their human forms!");
        else
            final_msg = jtrans("No longer under Kirke's spell, the hogs revert to their original forms!");
    }
...
void elven_twin_died(monster* twin, bool in_transit, killer_type killer, int killer_index)
{
    ...
    else
    {
        ...
        if (mons->observable())
        {
            ...
            simple_monster_message(mons, jtrans("seems to find hidden reserves of power!").c_str());
        }
        else
...
void elven_twins_pacify(monster* twin)
{
    ...
    if (mons_near(mons))
        simple_monster_message(mons, jtrans("likewise turns neutral.").c_str());
...
void timeout_phoenix_markers(int duration)
{
    ...
    for (int i = 0, size = markers.size(); i < size; ++i)
    {
        ...
        if (mmark->duration < 0)
        {
            ...
            if (from_inventory)
                simple_monster_message(mons, jtrans("is reborn from your pack in a blaze of fire!").c_str());
            else if (you.can_see(mons))
                simple_monster_message(mons, jtrans("is reborn in a blaze of fire!").c_str());
...
void shedu_do_resurrection(const monster* mons)
{
    ...
    if (you.can_see(my_pair))
        simple_monster_message(my_pair, jtrans("ceases action and prepares to resurrect its fallen mate.").c_str());
...
void shedu_do_actual_resurrection(monster* mons)
{
    ...
    if (from_inventory)
        simple_monster_message(mons, jtrans("resurrects its mate from your pack!").c_str());
    else if (you.can_see(mons))
        simple_monster_message(mons, jtrans("resurrects its mate from the grave!").c_str());
    else if (you.can_see(my_pair))
        simple_monster_message(mons, jtrans("rises from the grave!").c_str());
}
