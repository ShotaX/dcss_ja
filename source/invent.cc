...
void InvMenu::set_title(const string &s)
{
    ...
    if (stitle.empty())
    {
        ...
        stitle = make_stringf(
            jtrans("Inventory: %.0f/%.0f aum (%d%%, %d/52 slots)").c_str(),
...
static string _no_selectables_message(int item_selector)
{
    ...
    {
    case OSEL_ANY:
        return jtrans("You aren't carrying anything.").c_str();
    case OSEL_SCROLL_TARGET:
        return jtrans("You aren't carrying anything you could use a scroll on.").c_str();
    case OSEL_WIELD:
    case OBJ_WEAPONS:
        return jtrans("You aren't carrying any weapons.").c_str();
    case OBJ_ARMOUR:
    {
        if (_has_melded_armour())
            return jtrans("Your armour is currently melded into you.").c_str();
        else if (_has_tran_unwearable_armour())
            return jtrans("You aren't carrying any armour you can wear in your current form.").c_str();
        else
            return jtrans("You aren't carrying any armour.").c_str();
    }
    case OSEL_UNIDENT:
        return jtrans("You don't have any unidentified items.").c_str();
    case OSEL_RECHARGE:
        return jtrans("You aren't carrying any rechargeable items.").c_str();
    case OSEL_ENCH_ARM:
        return jtrans("You aren't carrying any armour which can be enchanted further.").c_str();
    case OBJ_CORPSES:
    case OSEL_VAMP_EAT:
        return jtrans("You aren't carrying any corpses which you can drain.").c_str();
    case OSEL_DRAW_DECK:
        return jtrans("You aren't carrying any decks from which to draw.").c_str();
    case OBJ_FOOD:
        return jtrans("You aren't carrying any food.").c_str();
    case OBJ_POTIONS:
        return jtrans("You aren't carrying any potions.").c_str();
    case OBJ_SCROLLS:
    case OBJ_BOOKS:
        return jtrans("You aren't carrying any spellbooks or scrolls.").c_str();
    case OBJ_WANDS:
        return jtrans("You aren't carrying any wands.").c_str();
    case OBJ_JEWELLERY:
        return jtrans("You aren't carrying any pieces of jewellery.").c_str();
    case OSEL_THROWABLE:
        return jtrans("You aren't carrying any items that might be thrown or fired.").c_str();
    case OSEL_BUTCHERY:
        return jtrans("You aren't carrying any sharp implements.").c_str();
    case OSEL_EVOKABLE:
        return jtrans("You aren't carrying any items that can be evoked.").c_str();
    case OSEL_FRUIT:
        return jtrans("You aren't carrying any fruit.").c_str();
    case OSEL_CURSED_WORN:
        return jtrans("None of your equipped items are cursed.").c_str();
    case OSEL_UNCURSED_WORN_ARMOUR:
        return jtrans("You aren't wearing any piece of uncursed armour.").c_str();
    case OSEL_UNCURSED_WORN_JEWELLERY:
        return jtrans("You aren't wearing any piece of uncursed jewellery.").c_str();
    case OSEL_BRANDABLE_WEAPON:
        return jtrans("You aren't carrying any weapons that can be branded.").c_str();
    }

    return jtrans("You aren't carrying any such object.").c_str();
}
                      ...
                      bool all_wands, bool msg, bool equip)
{
    ...
    case OBJ_WANDS:
        ...
        {
            if (msg)
                mpr(jtrans("You cannot grasp it well enough."));
            return false;
        }
