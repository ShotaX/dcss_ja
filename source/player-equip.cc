...
#define unknown_proprt(prop) (proprt[(prop)] && !known[(prop)])
    ...
    if (proprt[ARTP_AC])
    {
        ...
        if (!known[ARTP_AC])
        {
            if (msg)
            {
                mprf(jtrans("You feel %s.").c_str(), proprt[ARTP_AC] > 0?
                     jtrans("well-protected").c_str() :
					 jtrans("more vulnerable").c_str());
            }
    ...
    if (proprt[ARTP_EVASION])
    {
        ...
        if (!known[ARTP_EVASION])
        {
            if (msg)
            {
                mprf(jtrans("You feel somewhat %s.").c_str(), proprt[ARTP_EVASION] > 0?
                     jtrans("nimbler").c_str() : 
				 	 jtrans("more awkward").c_str());
            }
        ...
        && !items_give_ability(item.link, ARTP_FLY))
    {
        ...
        {
            if (you.airborne())
                mpr(jtrans("You feel vaguely more buoyant than before."));
            else
                mpr(jtrans("You feel buoyant."));
        }
    ...
    {
        if (msg)
            mpr(jtrans("You feel a brief urge to hack something to bits."));
                                     ...
                                     bool *show_msgs, bool meld)
{
    ...
    if (proprt[ARTP_AC])
    {
        ...
        {
            mprf(jtrans("You feel less %s.").c_str(),
                 proprt[ARTP_AC] > 0? jtrans("well-protected").c_str() :
				 						jtrans("vulnerable").c_str());
        }
    }
    ...
    if (proprt[ARTP_EVASION])
    {
        ...
        {
            mprf(jtrans("You feel less %s.").c_str(),
                 proprt[ARTP_EVASION] > 0? jtrans("nimble").c_str() :
											jtrans("awkward").c_str());
        }
    }
...
static void _wield_cursed(item_def& item, bool known_cursed, bool unmeld)
{
    if (!item.cursed() || unmeld)
        return;
    mprf(jtrans("It sticks to your %s!").c_str(), you.hand_name(false).c_str());
    int amusement = 16;
    if (!known_cursed)
    {
        amusement *= 2;
    ...
    case OBJ_WEAPONS:
    {
        ...
        if (special != SPWPN_NORMAL)
        {
            ...
            if (showMsgs)
            {
                ...
                case SPWPN_DRAGON_SLAYING:
                    mpr(player_genus(GENPC_DRACONIAN)
                        || you.form == TRAN_DRAGON
                            ? jtrans("You feel a sudden desire to commit suicide.")
                            : jtrans("You feel a sudden desire to slay dragons!"));
                    break;
                ...
                case SPWPN_SPEED:
                    mprf(jtrans("Your %s tingle!").c_str(),
                ...
                case SPWPN_PAIN:
                {
                    ...
                    if (you.skill(SK_NECROMANCY) == 0)
                        ...
                        mprf(jtrans("Pain shudders through your %s!").c_str(), your_arm);
                    else
                        mprf(jtrans("A searing pain shoots up your %s!").c_str(), your_arm);
                    break;
                }
                ...
                case SPWPN_PENETRATION:
                    mprf(jtrans("Your %s briefly pass through it before you manage to get a firm grip on it.").c_str(),
                         you.hand_name(true).c_str());
                    break;
...
static void _unequip_weapon_effect(item_def& item, bool showMsgs, bool meld)
{
    ...
    else if (item.base_type == OBJ_WEAPONS)
    {
        ...
        if (brand != SPWPN_NORMAL)
        {
            ...
            case SPWPN_FLAMING:
                if (showMsgs)
                    mprf(jtrans("%s stops flaming.").c_str(), msg.c_str());
                break;

            case SPWPN_FREEZING:
            case SPWPN_HOLY_WRATH:
                if (showMsgs)
                    mprf(jtrans("%s stops glowing.").c_str(), msg.c_str());
                break;

            case SPWPN_ELECTROCUTION:
                if (showMsgs)
                    mprf(jtrans("%s stops crackling.").c_str(), msg.c_str());
                break;

            case SPWPN_VENOM:
                if (showMsgs)
                    mprf(jtrans("%s stops dripping with poison.").c_str(), msg.c_str());
                break;
            ...
            if (you.duration[DUR_WEAPON_BRAND])
            {
                ...
                mpr(jtrans("Your branding evaporates."));
            }
        }
    }
...
static void _equip_armour_effect(item_def& arm, bool unmeld)
{
    ...
    if (ego != SPARM_NORMAL)
    {
        ...
        case SPARM_POSITIVE_ENERGY:
            mpr(jtrans("Your life force is being protected."));
            break;
...
static void _unequip_armour_effect(item_def& item, bool meld)
{
    ...
    case SPARM_FIRE_RESISTANCE:
        mpr(jtrans("\"Was it this warm in here before?\""));
        break;

    case SPARM_COLD_RESISTANCE:
        mpr(jtrans("You catch a bit of a chill."));
        break;

    case SPARM_POISON_RESISTANCE:
        if (player_res_poison() <= 0)
            mpr(jtrans("You feel less healthy."));
        break;
    ...
    case SPARM_POSITIVE_ENERGY:
        mpr(jtrans("You feel vulnerable."));
        break;
...
static void _remove_amulet_of_faith(item_def &item)
{
    if (!you_worship(GOD_NO_GOD)
        && !you_worship(GOD_XOM))
    {
        simple_god_message(jtrans("seems less interested in you.").c_str());
        ...
        {
            mprf(MSGCH_GOD,
                 jtrans("%s leaches power out of you as you remove it.").c_str(),
...
static void _equip_jewellery_effect(item_def &item, bool unmeld)
{
    ...
    case RING_INVISIBILITY:
        mprf(jtrans("You become %stransparent for a moment.").c_str(),
    ...
    case RING_FLIGHT:
        mprf(jtrans("You feel %sbuoyant.").c_str(), you.airborne() ? jtrans("more").c_str() : "");
        fake_rap = ARTP_FLY;
        ident = ID_KNOWN_TYPE;
        break;
    ...
    case RING_TELEPORTATION:
        if (crawl_state.game_is_sprint())
            ...
            mprf(jtrans("You feel slightly %sjumpy.").c_str(),
                 (player_teleport(false) > 8) ? jtrans("more").c_str() : "");
        ident = ID_KNOWN_TYPE;
        break;
    ...
    case AMU_FAITH:
        if (!you_worship(GOD_NO_GOD))
        {
            mpr(jtrans("You feel a surge of divine interest."), MSGCH_GOD);
            ident = ID_KNOWN_TYPE;
        }
        break;
    ...
    case AMU_STASIS:
        ...
        if (amount)
        {
            mprf(jtrans("The amulet engulfs you in a%s magical discharge!").c_str(),
                 (amount > 250) ? jtrans("massive").c_str() :
                 (amount >  50) ? jtrans("violent").c_str() :
                                  "");
            ident = ID_KNOWN_TYPE;
            ...
            if (you.duration[DUR_SLOW])
                dir--;
            if (dir > 0)
                mpr(jtrans("You abruptly slow down."), MSGCH_DURATION);
            else if (dir < 0)
                mpr(jtrans("Your slowness suddenly goes away."), MSGCH_DURATION);
            if (you.duration[DUR_TELEPORT])
                mpr(jtrans("You feel strangely stable."), MSGCH_DURATION);
            if (you.duration[DUR_BERSERK])
                mpr(jtrans("You violently calm down."), MSGCH_DURATION);
            // my thesaurus says this usage is correct
            if (you.duration[DUR_FINESSE])
                mpr(jtrans("Your hands get arrested."), MSGCH_DURATION);
    ...
    if (item.cursed())
    {
        mprf(jtrans("Oops, that %s feels deathly cold.").c_str(),
             jewellery_is_amulet(item)? jtrans("amulet").c_str() :
										jtrans("ring").c_str());
        learned_something_new(HINT_YOU_CURSED);
