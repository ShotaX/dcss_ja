...
void seen_monsters_react()
{
    ...
    for (monster_iterator mi(you.get_los()); mi; ++mi)
    {
        ...
        if (mi->props.exists("dowan_upgrade"))
        {
            ...
            simple_monster_message(*mi, jtrans("seems to find hidden reserves of power!").c_str());
        }
    }
}
...
void update_monsters_in_view()
{
    ...
    if (!msgs.empty())
    {
        ...
        bool warning = false;
        string warning_msg = (jtrans("View2")); //"Ashenzari warns you:";
        warning_msg += " ";
        for (unsigned int i = 0; i < size; ++i)
        {
                   ...
                   coord_def pos)
{
    ...
    {
        if (did_map)
            mpr(jtrans("View3")); //("You feel aware of your surroundings.");
        else
            canned_msg(MSG_DISORIENTED);
        ...
        if (!sensed.empty())
            mpr_comma_separated_list((jtrans("View4")), sensed);
    }
...
void toggle_show_terrain()
{
    ...
    if (_show_terrain)
        mpr(jtrans("View5")); //("Showing terrain only.");
    else
        mpr(jtrans("View6")); //("Returning to normal view.");
}
