...
void monster_caught_in_net(monster* mon, bolt &pbolt)
{
    ...
    if (mons_class_is_stationary(mon->type))
    {
        if (you.see_cell(mon->pos()))
        {
            ...
            {
                mprf(jtrans("The net is caught on %s!").c_str(),
    ...
    if (mon->is_insubstantial())
    {
        if (you.can_see(mon))
        {
            mprf(jtrans("The net passes right through %s!").c_str(),
                 mon->name(DESC_THE).c_str());
        }
        return;
    }
    ...
    {
        simple_monster_message(mon, jtrans("darts out from under the net!").c_str());
        return;
    }

    if (mon->type == MONS_OOZE || mon->type == MONS_PULSATING_LUMP)
    {
        simple_monster_message(mon, jtrans("oozes right through the net!").c_str());
        return;
    }
    ...
    if (!mon->caught() && mon->add_ench(ENCH_HELD))
    {
        if (mons_near(mon) && !mon->visible_to(&you))
            ...
            simple_monster_message(mon, jtrans("is caught in the net!").c_str());

        if (mon->flight_mode() == FL_WINGED)
        {
            simple_monster_message(mon, jtrans("falls like a stone!").c_str());
...
bool player_caught_in_net()
{
    ...
    {
        mpr(jtrans("You dart out from under the net!"));
        return false;
    }
    ...
    if (!you.attribute[ATTR_HELD])
    {
        ...
        {
            mpr(jtrans("You fall like a stone!"));
             ...
             || mons->type == MONS_PULSATING_LUMP)
    {
        ...
        if (mons->is_insubstantial())
        {
            simple_monster_message(mons,
                                   jtrans("drifts right through the net!").c_str());
        }
        else
        {
            simple_monster_message(mons,
                                   jtrans("oozes right through the net!").c_str());
        }
    }
    else
...
void trap_def::trigger(actor& triggerer, bool flat_footed)
{
    ...
    if (type == TRAP_SHAFT && m)
    {
            ...
            || trig_knows && !mons_is_fleeing(m) && !m->pacified())
        {
            ...
            if (you_know && triggerer.ground_level())
                simple_monster_message(m, jtrans("carefully avoids the shaft.").c_str());
            return;
        }
    }
    ...
    {
        simple_monster_message(m,jtrans("carefully avoids a trap.").c_str());
        return;
    }
                                  ...
                                  && category() != DNGN_TRAP_WEB)
    {
        if (you_know && m && triggerer.airborne())
            simple_monster_message(m, jtrans("flies safely over a trap.").c_str());
        return;
    }
    ...
    case TRAP_GOLUBRIA:
    {
        ...
        if (_find_other_passage_side(to))
        {
            if (you_trigger)
                ...
                simple_monster_message(m, jtrans("enters the passage of Golubria.").c_str());
    ...
    case TRAP_TELEPORT:
        ...
        if (!you_trigger && !you_know && !in_sight)
            hide();
        if (you_trigger)
            mpr(jtrans("You enter a teleport trap!"));
        if (ammo_qty > 0 && !--ammo_qty)
        {
            ...
            {
                env.map_knowledge(pos).set_feature(DNGN_FLOOR);
                mpr(jtrans("The teleport trap disappears."));
            }
            disarm();
        }
    ...
    case TRAP_ALARM:
        ...
        if (silenced(pos))
        {
            if (you_know && in_sight)
                mpr(jtrans("The alarm trap vibrates slightly, failing to make a sound."));
        }
        else
        {
            string msg;
            if (you_trigger)
                msg = jtrans("The alarm trap emits a blaring wail!");
            else
            {
    ...
    case TRAP_BLADE:
        ...
        else if (m)
        {
            if (one_chance_in(5) || (trig_knows && coinflip()))
            {
                ...
                if (you_know)
                {
                    simple_monster_message(m,
                                           jtrans("fails to trigger a blade trap.").c_str());
                }
                else
                    hide();
            }
            else if (random2(m->ev) > 8 || (trig_knows && random2(m->ev) > 8))
            {
                if (in_sight
                    && !simple_monster_message(m,
                                            jtrans("avoids a huge, swinging blade.").c_str()))
                {
    ...
    case TRAP_NET:
        ...
        else if (m)
        {
            ...
            if (one_chance_in(3) || (trig_knows && coinflip()))
            {
                ...
                triggered = false;
                if (you_know)
                    simple_monster_message(m, jtrans("fails to trigger a net trap.").c_str());
                else
                    hide();
            }
            else if (random2(m->ev) > 8 || (trig_knows && random2(m->ev) > 8))
            {
                ...
                if (in_sight)
                {
                    if (!simple_monster_message(m,
                                                jtrans("nimbly jumps out of the way of a falling net.").c_str()))
                    {
            ...
            else
            {
                ...
                if (in_sight)
                {
                    if (m->visible_to(&you))
                    {
                        mprf(jtrans("A large net falls down onto %s!").c_str(),
    ...
    case TRAP_WEB:
        if (triggerer.body_size(PSIZE_BODY) >= SIZE_GIANT)
        {
            trap_destroyed = true;
            if (you_trigger)
                mprf(jtrans("You tear through %s web.").c_str(), you_know ? "" : "");
            else if (m)
                simple_monster_message(m, jtrans("tears through a web.").c_str());
            break;
        }
        ...
        if (triggerer.is_web_immune())
        {
            if (m)
            {
                if (m->is_insubstantial())
                    simple_monster_message(m, jtrans("passes through a web.").c_str());
                else if (mons_genus(m->type) == MONS_JELLY)
                    simple_monster_message(m, jtrans("oozes through a web.").c_str());
        ...
        else if (m)
        {
            if (one_chance_in(3) || (trig_knows && coinflip()))
            {
                // Not triggered, trap stays.
                if (you_know)
                    simple_monster_message(m, jtrans("evades a web.").c_str());
                else
                    hide();
            }
            else
            {
                ...
                {
                    if (m->visible_to(&you))
                        simple_monster_message(m, jtrans("is caught in a web!").c_str());
                    else
    ...
    case TRAP_ZOT:
        if (you_trigger)
        {
            mpr((trig_knows) ? jtrans("You enter the Zot trap.")
                             : jtrans("Oh no! You have blundered into a Zot trap!"));
            if (!trig_knows)
        ...
        else if (m)
        {
            ...
            {
                mprf(MSGCH_SOUND, jtrans("You hear a %s \"Zot\"!").c_str(),
                     in_sight ? jtrans("loud").c_str() : jtrans("distant_zot").c_str());
            }
            ...
            if (targ)
            {
                if (in_sight)
                {
                    mprf(jtrans("The power of Zot is invoked against %s!").c_str(),
...
void disarm_trap(const coord_def& where)
{
    ...
    case DNGN_TRAP_MAGICAL:
        ...
        {
            mpr(jtrans("You can't disarm that trap."));
            return;
        }
        break;
    case DNGN_TRAP_NATURAL:
        // Only shafts for now.
        mpr(jtrans("You can't disarm a shaft."));
        return;
    default:
        break;
    }
    ...
    {
        string prompt = make_stringf(jtrans("Really try disarming that %s?").c_str(),
    ...
    {
        mpr(jtrans("You failed to disarm the trap."));
        if (random2(you.dex()) <= 5 + random2(5 + trap.difficulty()))
        {
                ...
                && trap.pos != you.pos())
            {
                if (coinflip())
                {
                    mpr(jtrans("You stumble into the trap!"));
    ...
    {
        mpr(jtrans("You have disarmed the trap."));
        trap.disarm();
    }
}
...
void free_self_from_net()
{
    ...
    if (do_what <= 0) // You try to destroy the net
    {
        ...
        if (hold < -7)
        {
            mprf(jtrans("You %s the net and break free!").c_str(),
                 can_slice ? (damage >= 4? jtrans("slicen").c_str() : jtrans("cutn").c_str()) :
                             (damage >= 4? jtrans("shredn").c_str() : jtrans("ripn").c_str()));
        ...
        if (damage >= 4)
        {
            mprf(jtrans("You %s into the net.").c_str(),
                 can_slice? jtrans("slicenn").c_str() 
							: jtrans("tear a large gash").c_str());
        }
        else
...
void trap_def::shoot_ammo(actor& act, bool was_known)
{
    ...
    if (act.is_player())
    {
        ...
        {
            mprf(jtrans("You avoid triggering %s trap.").c_str(), name(DESC_A).c_str());
    ...
    else if (!force_hit && one_chance_in(5))
    {
        if (was_known && you.see_cell(pos) && you.can_see(&act))
            mprf(jtrans("%s avoids triggering %s trap.").c_str(), act.name(DESC_THE).c_str(),
                 name(DESC_A).c_str());
        return;
    }
    ...
    if (!force_hit && trap_hit < act.melee_evasion(NULL))
    {
        if (act.is_player())
        {
            mprf(jtrans("%s shoots out and misses you.").c_str(), shot.name(DESC_A).c_str());
            practise(EX_DODGE_TRAP);
        }
        else if (you.see_cell(act.pos()))
        {
            mprf(jtrans("%s misses %s!").c_str(), shot.name(DESC_A).c_str(),
             ...
             && you.see_cell(act.pos()))
    {
        string owner;
        if (act.is_player())
            owner = jtrans("your").c_str();
        else if (you.can_see(&act))
            owner = apostrophise(act.name(DESC_THE));
        else // "its" sounds abysmal; animals don't use shields
            owner = jtrans("someone's").c_str();
        mprf(jtrans("%s shoots out and hits %s shield.").c_str(), shot.name(DESC_A).c_str(),
             owner.c_str());
    ...
    else // OK, we've been hit.
    {
        ...
        if (act.is_player())
        {
            mprf(jtrans("%s shoots out and hits you!").c_str(), shot.name(DESC_A).c_str());
        ...
        else
        {
            ...
            {
                mprf(jtrans("%s hits %s%s!").c_str(),
                     shot.name(DESC_A).c_str(),
                     act.name(DESC_THE).c_str(),
                     (damage_taken == 0 && !poison) ?
                         jtrans(", but does no damage").c_str() : "");
            }
...
bool maybe_destroy_web(actor *oaf)
{
    ...
    {
        if (oaf->is_monster())
            simple_monster_message(oaf->as_monster(), jtrans("pulls away from the web.").c_str());
        else
    ...
    if (oaf->is_monster())
        simple_monster_message(oaf->as_monster(), jtrans("tears the web.").c_str());
    else
...
bool ensnare(actor *fly)
{
    ...
    if (fly->body_size() >= SIZE_GIANT)
    {
        ...
        {
            if (fly->is_player())
                mpr(jtrans("A web splats on you, sticky and dirtying but otherwise harmless."));
            else
                mprf(jtrans("A web harmlessly splats on %s.").c_str(), fly->name(DESC_THE).c_str());
        }
        return false;
    }
    ...
    {
        simple_monster_message(fly->as_monster(), jtrans("is caught in a web!").c_str());
