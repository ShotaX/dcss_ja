...
static int _cull_items(void)
{
    ...
    mpr(jtrans("Too many items on level, removing some."), MSGCH_WARN);
...
void item_check(bool verbose)
{
    ...
    {
        if (verbose)
            strm << jtrans("There are no items here") << endl;
        return;
    }
    ...
    if (items.size() == 1)
    {
        ...
        strm << jtrans("You see") << name << jtrans("here") << endl;
    ...
    {
        if (!done_init_line)
            mprnojoin(jtrans("item-there"), MSGCH_FLOOR_ITEMS);
        ...
        strm << jtrans("item-many_items") << endl;
...
#ifdef TOUCH_UI
    string prompt = jtrans("Pick up what? (<Enter> or tap header to pick up)");
#else
    string prompt = jtrans("Pick up what? (_ for help)");
#endif
        ...
        for (int j = item_link; j != NON_ITEM; j = mitm[j].link)
        {
            if (&mitm[j] == selected[i].item)
            {
                ...
                if (result == 0 || result == -1)
                {
                    n_tried_pickup++;
                    if (result == 0)
                        pickup_warning = jtrans("You can't carry that much weight.");
                    else
...
{
    return string("found ") + item.name(DESC_A) + "."; //---ノートへの記入？----//
}
...
static void _milestone_check(const item_def &item)
{
    if (item_is_rune(item))
        ...
        mark_milestone("orb", "found the Orb of Zot!");  //---ノートへの記入？----//
}
...
bool pickup_single_item(int link, int qty)
{
    ...
    {
        mpr(jtrans("You can't carry that much weight."));
        learned_something_new(HINT_HEAVY_LOAD);
        return false;
    }
...
void pickup(bool partial_quantity)
{
    ...
    else
    {
        ...
        while (o != NON_ITEM)
        {
            ...
            if (keyin != 'a')
            {
                string prompt = jtrans("Pick up %s? ((y)es/(n)o/(a)ll/(m)enu/*?g,/q)").c_str();
...
static void _got_gold(item_def& item, int quant, bool quiet)
{
    ...
    if (!quiet)
    {
        const string gain = quant != you.gold
                            ? make_stringf(jtrans("(gained %d)").c_str(), quant)
                            : "";
                        ...
                        bool ignore_burden)
{
    ...
    {
        mpr(jtrans("You cannot pick up the net that holds you!"));
        ...
        for (int m = 0; m < ENDOFPACK; m++)
        {
            if (items_stack(you.inv[m], it))
            {
                if (!quiet && partial_pickup)
                    mpr(jtrans("You can only carry some of what is here."));
                if (!quiet)
                    _fish(it, quant_got);
        ...
        && you.char_direction == GDT_DESCENDING)
    {
        ...
        mpr(jtrans("The lords of Pandemonium are not amused. Beware!"), MSGCH_WARN);

        if (you_worship(GOD_CHEIBRIADOS))
            simple_god_message(jtrans("tells them not to hurry.").c_str());

        mpr(jtrans("Now all you have to do is get back out of the dungeon!"), MSGCH_ORB);
...
bool drop_item(int item_dropped, int quant_drop)
{
    ...
    {
        mpr(jtrans("You don't feel like leaving the orb behind!"));
        return false;
    }
    ...
    {
        mprf(jtrans("%s is stuck to you!").c_str(), you.inv[item_dropped].name(DESC_THE).c_str());
        return false;
    }
...
void drop_last()
{
    ...
    if (items_to_drop.empty())
        mprf("%s",jtrans("No item to drop.").c_str());
    else
    {
        you.last_pickup.clear();
...
#ifdef TOUCH_UI
    tmp_items = prompt_invent_items(jtrans("Drop what? (<Enter> or tap header to drop)").c_str(),
                                    MT_DROP,
#else
    tmp_items = prompt_invent_items(jtrans("Drop what? (_ for help)").c_str(), MT_DROP,
#endif
        ...
        if (item_needs_autopickup(mitm[o]))
        {
            ...
            if (Options.autopickup_no_burden && unit_mass != 0)
            {
                ...
                if (num_can_take < num_to_take)
                {
                    if (!n_tried_pickup)
                    {
                        mpr(jtrans("You can't pick everything up without burdening yourself."));
                    }
                    n_tried_pickup++;
            ...
            if (result == 0 || result == -1)
            {
                n_tried_pickup++;
                if (result == 0)
                    pickup_warning = jtrans("You can't carry any more.");
                else
...
static void _rune_from_specs(const char* _specs, item_def &item)
{
    ...
    while (true)
    {
        ...
        mpr(jtrans("Which rune (ESC to exit)?"), MSGCH_PROMPT);
...
static void _deck_from_specs(const char* _specs, item_def &item)
{
    ...
    if (item.sub_type == NUM_MISCELLANY)
    {
        ...
        {
            mpr(
jtrans("[a] escape     [b] destruction [c] dungeons [d] summoning [e] wonders"),
                MSGCH_PROMPT);
            mpr(
jtrans("[f] punishment [g] war         [h] changes  [i] defence"),
                MSGCH_PROMPT);
    ...
    if (rarity_val == -1)
    {
        while (true)
        {
            mpr(jtrans("[a] plain [b] ornate [c] legendary? (ESC to exit)"),
                MSGCH_PROMPT);
...
void corrode_item(item_def &item, actor *holder)
{
    ...
    {
        mprf(jtrans("The acid corrodes %s!").c_str(), item.name(DESC_YOUR).c_str());
    ...
    else if (holder && you.see_cell(holder->pos()))
    {
        ...
        {
            mprf(jtrans("The acid corrodes %s!").c_str(),
                 holder->name(DESC_THE).c_str());
        }
        else
        {
            mprf(jtrans("The acid corrodes %s %s!").c_str(),
