          ...
          string *pre_msg)
{
             ...
             && !wizard_blink)
    {
        if (pre_msg)
            mpr(pre_msg->c_str());
        mpr(jtrans("The power of the Abyss keeps you in your place!"));
    }
    ...
    else if (orb_haloed(you.pos()) && !wizard_blink)
    {
        if (pre_msg)
            mpr(pre_msg->c_str());
        mpr(jtrans("The orb interferes with your control of the blink!"), MSGCH_ORB);
    ...
    else if (!allow_control_teleport(true) && !wizard_blink)
    {
        if (pre_msg)
            mpr(pre_msg->c_str());
        mpr(jtrans("A powerful magic interferes with your control of the blink."));
    ...
    else
    {
        ...
        while (1)
        {
            ...
            {
                if (!wizard_blink
                    && !yesno(jtrans("Are you sure you want to cancel this blink?").c_str(),
                              false, 'n'))
                {
                    mesclr();
                    continue;
            ...
            {
                mprf(jtrans("You cannot blink away from %s!").c_str(),
            ...
            {
                mprf(jtrans("You cannot blink closer to %s!").c_str(),
            ...
            {
                mesclr();
                mpr(jtrans("You can't blink into the sea!"));
            }
            else if (grd(beam.target) == DNGN_LAVA_SEA)
            {
                mesclr();
                mpr(jtrans("You can't blink into the sea of lava!"));
            }
            ...
            {
                mesclr();
                mpr(jtrans("You can only blink to visible locations."));
            }
        }
        ...
        {
            mpr(jtrans("Oops! Maybe something was there already."));
            random_blink(false);
        }
...
void random_blink(bool allow_partial_control, bool override_abyss, bool override_stasis)
{
    ...
    {
        mpr(jtrans("You may select the general direction of your translocation."));
...
bool allow_control_teleport(bool quiet)
{
    ...
    if (!quiet && !retval && player_control_teleport())
    {
        if (orb_haloed(you.pos()))
            mpr(jtrans("The orb prevents control of your teleportation!"), MSGCH_ORB);
        else if (you.beheld())
            mpr(jtrans("It is impossible to concentrate on your destination whilst mesmerised."));
        else
            mpr(jtrans("A powerful magic prevents control of your teleportation."));
    }
...
void you_teleport(void)
{
    ...
    else
    {
        ...
        {
            mpr(jtrans("You feel the orb delaying this translocation!"), MSGCH_ORB);
                             ...
                             bool wizard_tele, int range)
{
    ...
    if (is_controlled)
    {
        ...
        if (!wizard_tele)
        {
            mpr(jtrans("You may choose your destination (press '.' or delete to select)."));
            mpr(jtrans("Expect minor deviation."));
            more();
        }
        ...
        while (true)
        {
            ...
            if (crawl_state.seen_hups)
            {
                mpr("Controlled teleport interrupted by HUP signal, cancelling teleport.", MSGCH_ERROR);
                if (!wizard_tele)
                    contaminate_player(1000, true);
                return false;
            }
            ...
            if (!chose || pos == you.pos())
            {
                if (!wizard_tele)
                {
                    if (!yesno(jtrans("Are you sure you want to cancel this teleport?").c_str(),
                               true, 'n'))
                    {
                        continue;
                    }
            ...
            {
                mprf(jtrans("You cannot teleport away from %s!").c_str(),
                     beholder->name(DESC_THE, true).c_str());
                mpr(jtrans("Choose another destination (press '.' or delete to select)."));
                more();
                continue;
            }
            ...
            {
                mprf(jtrans("You cannot teleport closer to %s!").c_str(),
                     fearmonger->name(DESC_THE, true).c_str());
                mpr(jtrans("Choose another destination (press '.' or delete to select)."));
                more();
                continue;
        ...
        if (!in_bounds(pos))
        {
            mpr(jtrans("Nearby solid objects disrupt your rematerialisation!"));
            is_controlled = false;
        }
        ...
        if (is_controlled)
        {
            ...
            if (_cell_vetoes_teleport(pos, true, wizard_tele))
            {
                if (wizard_tele)
                {
                    mpr(jtrans("Even you can't go there right now. Sorry!"), MSGCH_WARN);
                    return false;
                }
            ...
            else if (testbits(env.pgrid(pos), FPROP_NO_CTELE_INTO) && !wizard_tele)
            {
                is_controlled = false;
                large_change = false;
                mpr(jtrans("A strong magical force throws you back!"), MSGCH_WARN);
            }
            else
            {
                // Leave a purple cloud.
...
spret_type cast_portal_projectile(int pow, bool fail)
{
    ...
    {
        mpr(jtrans("You can't shoot at gazebos."));
        return SPRET_ABORT;
    }
...
spret_type cast_apportation(int pow, bolt& beam, bool fail)
{
    ...
    if (item_is_orb(item)
      ...
      {
        mpr(jtrans("You cannot apport the Orb!"));
        return SPRET_ABORT;
    }
    ...
    if (max_units <= 0)
    {
        ...
        mpr(jtrans("The mass is resisting your pull."));
    ...
    if (item_is_orb(item))
    {
        ...
        {
            orb_pickup_noise(where, 30,
                jtrans("The orb shrieks and becomes a dead weight against your magic!").c_str(),
                jtrans("The orb lets out a furious burst of light and becomes a dead weight against your magic!").c_str());
            return SPRET_SUCCESS;
        }
        else // Otherwise it's just a noisy little shiny thing
        {
            orb_pickup_noise(where, 30,
                jtrans("The orb shrieks as your magic touches it!").c_str(),
                jtrans("The orb lets out a furious burst of light as your magic touches it!").c_str());
        }
    }
    ...
    mprf(jtrans("Yoink! You pull the item%s towards yourself.").c_str(),
         (item.quantity > 1) ? "" : "");
    ...
    if (max_units < item.quantity)
    {
        ...
        {
            // Always >1 item.
            mpr(jtrans("They abruptly stop in place!"));
            // Too late to abort.
            return SPRET_SUCCESS;
        }
...
spret_type cast_semi_controlled_blink(int pow, bool cheap_cancel, bool end_ctele, bool fail)
{
    ...
    while (1)
    {
        mpr(jtrans("Which direction? [ESC to cancel]"), MSGCH_PROMPT);
        ...
        if (cheap_cancel
            || yesno(jtrans("Are you sure you want to cancel this blink?").c_str(), false ,'n'))
        {
            canned_msg(MSG_OK);
            return SPRET_ABORT;
        }
...
spret_type cast_golubrias_passage(const coord_def& where, bool fail)
{
    ...
    if (tries >= 100 || tries2 >= 100)
    {
        if (you.trans_wall_blocking(randomized_where))
            ...
            mpr(jtrans("Creating passages of Golubria requires sufficient empty space."));
        return SPRET_ABORT;
    }
    ...
    {
        mpr(jtrans("A powerful magic interferes with the creation of the passage."));
        return SPRET_ABORT;
    }
